...
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
  ...
  <title><?php
  if (CHtml::encode($this->pageTitle)<>'') {echo CHtml::encode($this->pageTitle);}else{ echo 'Dashboard - Insectram CRM';} ?></title>
...
$menu=Generalsettings::model()->find(array(
	...
	if(((isset($aa) &&  !is_array($aa)) || !isset($aa)) && empty($menu))
	{
	...
	?>
		<body class="vertical-layout vertical-menu-modern 2-columns fixed-navbar pace-done  menu-expanded" data-open="click" data-menu="vertical-menu-modern" data-col="2-columns">
			<div id="backgroundLoading" class="backgroundLoading loadingDisplay"></div>
			  ...
			  <div class="dropdown-menu" aria-labelledby="dropdown-flag">
			   <?php $tranlateslanguage= Yii::app()->getModule('translate')->language->getlanguages(); ?>
				...
				$adres = 'https://'.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'];
				 ...
				 foreach($tranlateslanguage as $tlanguage){?>
					 <a class="dropdown-item" href="<?=$adres.(!empty($_GET) && strstr($_SERVER['REQUEST_URI'], "?") ?'&':'?');?>language=<?=$tlanguage->name?>"><i class="flag-icon flag-icon-<?=$tlanguage->flag;?>"></i><?=t($tlanguage->title);?></a>
				<?php }?>
              <?php /*  <a class="dropdown-item" href="#"><i class="flag-icon flag-icon-fr"></i> French</a>
                <a class="dropdown-item" href="#"><i class="flag-icon flag-icon-cn"></i> Chinese</a>
                <a class="dropdown-item" href="#"><i class="flag-icon flag-icon-de"></i> German</a> */ ?>
