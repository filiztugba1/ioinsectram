...
class CWebUser extends CApplicationComponent implements IWebUser
{
	...
	public function checkAccess($operation,$params=array(),$allowCaching=true)
	{
		...
		//superadmin vs authassignmentste bulunan yetkiyi de cache e ekle
		//var_dump($operation);
		$perms=array();
		if (is_array(Yii::app()->params['myauthassignments']))
		{
			$perms=Yii::app()->params['myauthassignments'];
		}else
		{
			...
			if ($items)
			{
				...
				{
					array_push($perms, $ite->itemname);
					if($ite->itemname=='Superadmin'){ return true;}
					//Yii::app()->params['myauthassignments']=$ite->itemname;					
				}
				Yii::app()->params['myauthassignments']=$perms;
			}else
			{
				return false;
			}
		}
		$arrok=Yii::app()->params['checkaccok'];
		$arrno=Yii::app()->params['checkaccno'];
		if (is_array($arrok))
		{
			if(array_search($operation, $arrok)>=1)
			{
				return true;
			}
			else
			{
				if(!array_search($operation, $arrno)>=1)
				{
		...
		{
			Yii::app()->params['checkaccok']=array();
			Yii::app()->params['checkaccno']=array();
		}
		if($allowCaching && $params===array() && isset($this->_access[$operation]))
		{
			...
			{
				if ($return)
				{		if(array_search($operation, $perms)>=1){
						
						array_push(Yii::app()->params['checkaccok'],$operation);
						return true;
						}
						array_push(Yii::app()->params['checkaccok'],$operation);
				}
				...
				{
						array_push(Yii::app()->params['checkaccno'],$operation);
				}
			}
			return $return;
		}
		...
		foreach($perms as $p)
		{
			$access=AuthItemChild::model()->find(array('condition'=>'parent="'.$p.'" and child="'.$operation.'"'));			
			if ($access)
			{
				break;
			}
