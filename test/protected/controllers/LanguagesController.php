...
class LanguagesController extends Controller
{
		...
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','deleteall'
                        ,'languageslist','languagescreateupdate','languagesdelete','languagesdetail','languagesdeleteall'),
				'users'=>array('*'),
			),
  ...
  public function actionLanguageslist()
  {
    ...
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->languageslist($_POST);
   ...
   public function actionLanguagescreateupdate()
  {
    $yetki=AuthAssignment::model()->accesstoken();
   
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->languagescreateupdate($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
   ...
   public function actionLanguagesdelete()
  {
    ...
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->languagesdelete($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
     ...
     public function actionLanguagesdetail()
  {
    ...
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->languagesdetail($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
    ...
    public function actionLanguagesdeleteall()
  {
    ...
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->languagesdeleteall($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
	...
	public function actionCreate()
	{
		...
		// $this->performAjaxValidation($model);
			$language=Languages::model()->findAll(array(
								   'condition'=>'name=:name','params'=>array('name'=>$_POST['Languages']['name'])
							   ));

		if(isset($_POST['Languages']) && count($language)==0)
		{
	...
	{
		$deleteall=explode(',',$_POST['Languages']['id']);
		...
		Yii::app()->SetFlashes->add($model,t('Selected languages deleted!'),array('index'));
	}
