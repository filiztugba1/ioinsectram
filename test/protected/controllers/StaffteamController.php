...
class StaffteamController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','teamleader','color','deleteall'
                        ,'staffteamlist','staffteamcreateupdate','staffteamdelete','staffteamdetail','staffteamdeleteall','teamuserlist','staffteamisactive'),
				'users'=>array('*'),
			),
     ...
     ///////////. yeni api başlangıç /////////
  public function actionStaffteamlist()
  {
     $yetki=AuthAssignment::model()->accesstoken();
   
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->staffteamlist($_POST);
   ...
   public function actionStaffteamcreateupdate()
  {
    $yetki=AuthAssignment::model()->accesstoken();
   
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->staffteamcreateupdate($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
   ...
   public function actionStaffteamdelete()
  {
    ...
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->staffteamdelete($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
     ...
     public function actionStaffteamdetail()
  {
    ...
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->staffteamdetail($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
    ...
    public function actionStaffteamdeleteall()
  {
    ...
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->staffteamdeleteall($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
  ...
  public function actionTeamuserlist()
  {
    $yetki=AuthAssignment::model()->accesstoken();
   
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->teamuserlist($_GET);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
   ...
   public function actionStaffteamisactive()
  {
    ...
    if($yetki['status'])
    {
      $response=NewParamsModel::model()->staffteamisactive($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
	...
	public function actionTeamleader()
	{
		...
		if($ax->firmid!=0)
		{
				...
				if($firm->package=='Packagelite')
				{
					...
					?> <option value="<?=$ax->id;?>"><?=$userlike->name.' '.$userlike->surname;?></option><?php					}
