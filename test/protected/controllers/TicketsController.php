...
//////////////////////////////////////
			...
			if($model->save())
			{
				...
				$mail->Subject = 'Yeni bir destek talebi.!';
				$mail->AltBody = Yii::app()->getBaseUrl(true).'/tickets';
				$mail->MsgHTML('<h3>'.$user->name.''.$user->surname.'</h3>
								 Tarafından bir destek talebi açılmıştır.
								<p>Görmek için lütfen <a href="'.Yii::app()->getBaseUrl(true).'/tickets">Buraya tıklayın </a> </p>');
		...
		if ($type==0)
		 {
            ...
            api_response( 'Status : <span style="color:white;padding:5px 10px;background-color:#1e73be;border-radius:10px;">'.t('New').' </span>');	
			 echo 'Status : <span style="color:white;padding:5px 10px;background-color:#1e73be;border-radius:10px;">'.t('New').' </span>';
		 }
		 else if($type==1){
            api_response( 'Status : <span style="color:white;padding:5px 10px ;background-color:#ffc107;border-radius:10px;">'.t('Open').' </span>');	
	
			  echo 'Status : <span style="color:white;padding:5px 10px ;background-color:#ffc107;border-radius:10px;">'.t('Open').' </span>';
		  }
		 else if ($type==2){
          api_response( 'Status : <span style="color:white;padding:5px 10px;background-color:#5cb85c;border-radius:10px;">'.t('Closed').'  </span>');	
