...
class ConformityController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('print','reports','index','view','client','department','updatediv','img','activity','index1','gunsonuemail','conformitytype','conformitystatus','deleteall','conformitystatusbutton','closed','assign','assigncreate','returnassign','uygunsuzlukListesi','newConformityList',
                        'index2','conformitylist','conformitycreateupdate','conformitydetail','conformitydelete','activity','conformityassignusers','conformityassigncreate','conformityreturnassign','deadlinecreate','conformityok','conformitynok','conformityetkinliktanimi','conformitykapat'),
				'users'=>array('*'),
			),
  ...
  public function actionIndex2()
  {
    ...
    public function actionConformitylist() // admin girişinde firm listesini çekmek için kullanılır.
  {
    ...
    {
      $req=$_POST;
      if(isset($_GET) && !empty($_GET) && $_GET['status'])
      {
        $req=$_GET;
      }
      $response=NewConformityModel::model()->conformitylist($req);
   ...
   public function actionConformitycreateupdate()
  {
    ...
    if($yetki['status'])
    {
      $response=NewConformityModel::model()->conformitycreateupdate($_POST,$_FILES);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
   ...
   public function actionConformitydelete()
  {
    ...
    if($yetki['status'])
    {
      $response=NewConformityModel::model()->conformitydelete($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
     ...
     public function actionConformitydetail()
  {
    ...
    if($yetki['status'])
    {
      $response=NewConformityModel::model()->conformitydetail($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
  ...
  public function actionConformityassignusers($id)
  {
    ...
    if($yetki['status'])
    {
      $response=NewConformityModel::model()->conformityAssignUsers($id);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
  ...
  public function actionConformityassigncreate()
	{
		$yetki=AuthAssignment::model()->accesstoken();
   
    if($yetki['status'])
    {
      $response=NewConformityModel::model()->assignCreate($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
   ...
   public function actionConformityreturnassign()
	{
		$yetki=AuthAssignment::model()->accesstoken();
   
    if($yetki['status'])
    {
      $response=NewConformityModel::model()->returnassign($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
  ...
  public function actionDeadlinecreate()
	{
		$yetki=AuthAssignment::model()->accesstoken();
   
    if($yetki['status'])
    {
      $response=NewConformityModel::model()->deadlineCreate($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
  ...
  public function actionConformityok()
	{
		$yetki=AuthAssignment::model()->accesstoken();
   
    if($yetki['status'])
    {
      $response=NewConformityModel::model()->Conformityok($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
  ...
  public function actionConformitynok()
	{
		$yetki=AuthAssignment::model()->accesstoken();
   
    if($yetki['status'])
    {
      $response=NewConformityModel::model()->Conformitynok($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
   ...
   public function actionConformityetkinliktanimi()
	{
		$yetki=AuthAssignment::model()->accesstoken();
   
    if($yetki['status'])
    {
      $response=NewConformityModel::model()->conformityetkinliktanimi($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
  ...
  public function actionConformitykapat()
	{
		$yetki=AuthAssignment::model()->accesstoken();
   
    if($yetki['status'])
    {
      $response=NewConformityModel::model()->conformitykapat($_POST);
      echo json_encode($response);
      exit;
    }
    json_encode($yetki);
...
if(!(isset($_GET['depType']) && $_GET['depType']=='multiselect')){
					...
					<label for="basicSelect"><?=t('Department');?></label>
                        <fieldset class="form-group">
                          ...
                          <select class="custom-select block" style="width:100%" id="selectdepartment2" onchange="myFunctionDepartment2()" name="Conformity[departmentid]">
								<?php
								foreach($department as $departmentx){?>
								<option value="<?=$departmentx->id;?>" <?php if(isset($_GET['departmentid']) && $_GET['departmentid']==$departmentx->id){echo "selected";}?>><?=$departmentx->name;?></option>
							<?php }?>
							</select>
                        </fieldset>
                    </div>

		<?php $subdepartment=Departments::model()->findAll(array(
								   'order'=>'name ASC',
					...
					<label for="basicSelect"><?=t('Sub-Department');?></label>
                        <fieldset class="form-group">
                          ...
                          <select class="custom-select block" style="width:100%" id="modalsubdepartmentid" name="Conformity[subdepartmentid]">
								...
								<option value="<?=$subdepartmentx->id;?>" <?php if(isset($_GET['subdepartmentid']) && $_GET['subdepartmentid']==$subdepartmentx->id){echo "selected";}?>><?=$subdepartmentx->name;?></option>
							<?php }?>
							</select>
                        </fieldset>
                    </div>
	...
	public function actionAssign()
	{
		...
		?>
			...
			<?php				$model=Conformity::model()->find(array('condition'=>'id='.$_GET['id']));
				...
				$usermodel=User::model()->find(array('condition'=>'id=:id','params'=>array('id'=>$model->userid)));
				...
				if($usermodel->clientid==0) //eger conformityi acan kişi admin-firm veya branch ise goncerilecek email
				{
					...
					foreach($ass as $assx)
					{
						...
						<option  value="<?=$userx->id;?>"><?php if($userx->name=='' && $userx->surname==''){echo $userx->username;}else{echo $userx->name.' '.$userx->surname;}?></option>
					<?php }}
			...
			<?php
				...
				<tr <?php if($status->id==0){echo 'style="background-color: #c8d2f9;"';}?>  <?php if (Yii::app()->user->checkAccess('nonconformitymanagement.activity.view')){?> onclick="window.open('<?=Yii::app()->baseUrl?>/conformity/activity/<?=$conformityx['id'];?>', '_blank')" <?php }?>>
					...
					<td>
					 ...
					 <td><?=$subdep?></td>
					 <td>
					 <?php					 if( gmdate('m',$conformityx['date'])==2)
						{
