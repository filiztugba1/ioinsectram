	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','tabs','fileandtable','create2','create3','documentlist','documentmenu','subview','subfile','documentlist2','update3',"documandetay",
                        'index2','categorylist'
                        ),
				'users'=>array('*'),
			),
            ...
            <div class="card-body">
				<table class="table table-striped table-bordered dataex-html5-export table-responsive">
                    ...
                    <tbody>
					 ...
					 <?php $document= Yii::app()->db->createCommand('SELECT documents.id as id,documents.categoryid as categoryid,documents.name as name,documents.fileurl as url,documents.type as type,documents.firmid as dfirmid,documents.branchid as dbranchid,documents.clientid as dclientid,documents.clientbranchid as dclientbranchid,documents.createdtime as createdtime,documentviewfirm.createdtime as vcreatedtime FROM documents INNER JOIN documentviewfirm ON documents.id = documentviewfirm.documentid where documents.categoryid='.$_GET['id'].' and documentviewfirm.firmid='.$ax->firmid.' and documentviewfirm.branchid='.$ax->branchid.' and documentviewfirm.clientid='.$ax->clientid.' and documentviewfirm.clientbranchid='.$ax->clientbranchid.' GROUP BY documentviewfirm.documentid')->queryAll();
        ...
        if (isset($photos) && count($photos) > 0)
		{
			...
			foreach ($photos as $image => $pic)
			{
				...
				if($ax->firmid==0 || $package->maxfile==-1 || $size>=0)
				{
					...
					for($n=0;$n<$countt;$n++)
					{
						$dname=$n==0?$type[$n]:$dname.'_'.$type[$n];
					}
          if ($pic->saveAs(Yii::getPathOfAlias('webroot').'/uploads/'.$firm->username.'/'.$dname.'_'.date('Y-m-d H:i:s').'.'.$type[$countt]))
					{

                        $img_add = new Documents();


						$img_add->fileurl='uploads/'.$firm->username.'/'.$dname.'_'.date('Y-m-d H:i:s').'.'.$type[$countt];
						$img_add->categoryid=$_POST['Documents']['categoryid'];
                        ...
                        if($viewer!=-1 && count($_POST['Documents']['viewer'])!=0)
						{
							// echo 'sdf';
	...
	if (isset($_POST['Documents']))
	{
		...
		else
		{
			...
			if(isset($_POST['Documents']['id']))
			{
				...
				if(isset($img_add) && $viewer!=-1 && count($_POST['Documents']['viewer'])!=0)
				{
