				...
				f = {
					...
					_filesCheck: function() {
						...
						for(var t = 0; t < f.files.length; t++) {
							var x = f.files[t].name.split(".")
								...
								m = {
									name: file.name,
									size: file.length,
									size2: f._assets.bytesToSize(file.length),
									type: file.type,
									ext: x
								};
							...
							if(n.maxSize != null && f.files[t].length > n.maxSize * 1048576) {
								alert(f._assets.textParse(n.captions.errors.filesSize, m));
								return false;
								break
							}
							if(file.length == 4096 && file.type.length == 0) {
								return false;
								break
							}
							s += f.files[t].length
						...
						if((n.addMore || n.uploadFile)) {
							var m = f._itFl.filter(function(a, b) {
								if(a.file.name == file.name && a.file.length == file.length && a.file.type == file.type && (file.lastModified ? a.file.lastModified == file.lastModified : true)) {
									return true;
								}
							});
							if(m.length > 0) {
					...
					_thumbCreator: {
						...
						renderFile: function(file, html, opts) {
							...
							if(window.File && window.FileList && window.FileReader && opts.type == "image" && opts.length < 6e+6) {
