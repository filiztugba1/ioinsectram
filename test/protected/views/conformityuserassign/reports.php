            ...
            <div class="col-12">
			  ...
			  <div class='card-header'>
			   <div class="col-xl-12 col-lg-12 col-md-12 mb-1" style='background:#e0ebff;padding:12px 10px 12px 0px;    border-radius: 5px;'>
							 <form id="conformity-form" action=""  method="post" enctype="multipart/form-data">
             ...
             foreach($userss as $userssx)
								...
								<div class="col-md-3">
									...
									<label for="basicSelect"><?=t('Start Date');?></label>
									  <input type="date"  class="form-control"  placeholder="<?=t('Start Date');?>" name="startdate" value="<?php if(isset($_POST['startdate'])){echo $_POST['startdate'];}else{echo '2019-01-01';}?>">
									</fieldset>
								</div>
								...
								<div class="col-md-3">
									...
									<label for="basicSelect"><?=t('Finish Date');?></label>
									  <input type="date"  class="form-control"  placeholder="<?=t('Finish Date');?>" name="finishdate" value="<?php if(isset($_POST['startdate'])){echo $_POST['finishdate'];}else{echo date('Y-m-d');}?>">
									</fieldset>
								</div>
							...
							<div class="card">
								<div class="card-header">
									<div class="row" style="border-bottom: 1px solid #e3ebf3;">
										 ...
										 <h4 class="card-title">Açık ve kapalı uygunsuzluk raporu</h4>
										</div>
									</div>
								</div>
											...
											<table  class="table table-striped table-bordered dataex-html5-export2 table-responsive">
												<thead>
													<tr>
														...
														<th><?=t('AÇIK UYGUNSUZLUK');?></th>
														<th><?=t('KAPALI UYGUNSUZLUK');?></th>
														...
														if($conformityuserassign)
														{
															...
															foreach($conformityuserassign as $conformityuserassignx)
															{
																...
																if((!$gerigonderme && !$deadlineverme) || (!$gerigonderme && $deadlineverme))
																{
																	...
																	?>
																	<tr>
																	<td><?php if($userssx->name=='' && $userssx->surname==''){echo $userssx->username;}else{echo $userssx->name.' '.$userssx->surname;}?></td>
																	<td><?=$conformityname->numberid;?></td>
																	<td><?php																		$depart=Departments::model()->find(array('condition'=>'id='.$conformityname->departmentid));
																		if($depart)
																		{
																			echo $depart->name;
																		}
																	...
																	?></td>
																	<td>
																		<?php																			$subdepart=Departments::model()->find(array('condition'=>'id='.$conformityname->subdepartmentid));
																			if($subdepart)
																			{
																				echo $subdepart->name;
																			}
																	...
																	<td><?=$conformityname->definition;?></td>
																	<td><?=$conformityname->suggestion;?></td>
																	<td><?=$conformityname->priority.' '.t('Degree');?></td>
																	<td><?=date('d-m-Y',$conformityuserassignx->sendtime);?></td>
																	<?php $conformityuserassign=Conformityuserassign::model()->findAll(array("condition"=>"parentid=".$conformityuserassignx->id));?>
																	<td>
																	<?php																	if($conformityuserassign)
																	{
																		echo 0;
																	}
																	else {
																	...
																	?></td>
																	<td><?php																	if($conformityuserassign)
																	{
																		echo 1;
																	}
																	else {
