...
$monitors=MobileworkordermonitorsView::model()->findAll(array("condition"=>$sql." checkdate!=0 and isdelete=0 and clientbranchid=".$_POST['Report']['clientid'].' and monitortype='.$_POST['Monitoring']['mtypeid'],"order"=>"monitorid asc"));
...
?>
            ...
            <?=t('Regional Trend Analysis')?>
              <?=$_POST["Monitoring"]["date"];?> /
                <?=$_POST["Monitoring"]["date1"];?> <br>
                  <?=$monitortype->name.' - '.$monitortype->detailed;?>
          </h4>
          <br>
          <?=t('Rapor Kriterleri')?>:
            <?=$kriterler?>
  ...
  <?php $subid=$_POST["Monitoring"]["subid"][0];?>
