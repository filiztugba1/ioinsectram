...
foreach ($departments as $department) {
  ...
  $parent= Yii::app()->db->createCommand('SELECT *,sum(value) as total FROM mobileworkorderdata_view where '.$parentsql.'='.$department->id.' and clientbranchid='.$_POST['Report']['clientid'].' and monitortype='.$_POST['Monitoring']['mtypeid'].' and isproduct=0 and (createdtime between '.$midnight.' and '.$midnight2.') '.$sqlnew)->queryAll();
                ...
                <div class="card-header">
					<div class="row" style="border-bottom: 1px solid #e3ebf3;">
					   <div class="col-xl-9 col-lg-9 col-md-9 mb-1">
						  <h4 class="card-title"><?=t('Department Reports')?>   <?=$_POST["Monitoring"]["date"];?> / <?=$_POST["Monitoring"]["date1"];?> <br>     <?=$monitortype->name.' - '.t($monitortype->detailed);?></h4>
							<?php
...
<?php }else{ ?>
	...
	myChart.on('click', function (params) {
		...
		var name= "../client/subdepidgetir?id=" + params.name+"&clientid="+<?=$_POST["Report"]["clientid"]?>+"&departmentid="+<?=$_POST["Report"]["dapartmentid"]->id?>;
		$.get( name, function( data ) {
			var veri=jQuery.parseJSON(data);
