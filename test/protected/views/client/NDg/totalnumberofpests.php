...
$reports=MobileworkorderdataView::model()->findAll(array('condition'=>$sql.' clientbranchid='.$_POST['Report']['clientid'].' and isproduct=0   and createdtime!=0 and monitortype='.$_POST['Monitoring']['mtypeid'],'order'=>'createdtime asc, petid asc'));
	...
	/*	$tarihbasla=strtotime($tarih);
		...
		*/
		...
		$parent= Yii::app()->db->createCommand('SELECT *,sum(value) as total FROM mobileworkorderdata_view where  '.$sql.' clientbranchid='.$_POST['Report']['clientid'].' and monitortype='.$_POST['Monitoring']['mtypeid'].' and isproduct=0 and (createdtime between '.$x1.' and '.$x2.') group by petid')->queryAll();
                ...
                <div class="card-header">
					<div class="row" style="border-bottom: 1px solid #e3ebf3;">
					   <div class="col-xl-9 col-lg-9 col-md-9 mb-1">
						  <h4 class="card-title"><?=t('Total Number of Pests Based on Date');?>   <?=$_POST["Monitoring"]["date"];?> / <?=$_POST["Monitoring"]["date1"];?> <br>   <?=$monitortype->name.' - '.$monitortype->detailed;?>	</h4> <br> <?=t('Rapor Kriterleri')?>:<?=$kriterler?>
...
// specify chart configuration item and data
        option = {
            ...
            color: ["#F2490C","#e003fb","#7b1fa2","#ff7588","#65BFBF","#5C5A61","#F2B90C","#0018c5","#7d0404"],
            calculable : true,
            xAxis : [
                {
                    type : 'category',
			...
			text:'<?=t('PDF');?>',
			  ...
			  customize: function(doc) {
				doc.content.splice(0, 1, {
				 ...
				 {
					text: '<?=t("Total Number of Pests Based on Date");?>   <?=$_POST["Monitoring"]["date"];?> / <?=$_POST["Monitoring"]["date1"];?>  <?=$monitortype->name.' - '.$monitortype->detailed;?>	<?=t("Rapor Kriterleri")?>:<?=$kriterler?>',
					bold: true,
					fontSize: 12,
						alignment: 'center'
				  },
