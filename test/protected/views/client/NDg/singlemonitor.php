...
if(!isset($_POST["Monitoring"]["subid"]) || count($_POST['Monitoring']['subid'])==0){  //sub departman yoksa girsin
  if($_POST['Report']['dapartmentid'][0] <> 0)
  {
    ...
    if(is_countable($_POST['Report']['dapartmentid']) && count($_POST['Report']['dapartmentid'])>1)
    {
...
$reports=MobileworkordermonitorsView::model()->findAll(array('select'=>'*,DATE_FORMAT(FROM_UNIXTIME(`checkdate`), "%Y-%m-%d") AS "timestamp"','condition'=>$sql.'checkdate!=0 and isdelete=0 and clientbranchid='.$_POST['Report']['clientid'].' and monitortype='.$_POST['Monitoring']['mtypeid'].'','order'=>'timestamp asc, monitorid asc'));
 ...
 // exit;
	...
	/*	$tarihbasla=strtotime($tarih);
		...
		*/
		if($secilimi==true)
		{
			$parent= Yii::app()->db->createCommand('SELECT *,sum(value) as total FROM mobileworkorderdata_view where  '.$sqldata.' clientbranchid='.$_POST['Report']['clientid'].' and monitortype='.$_POST['Monitoring']['mtypeid'].' and isproduct=0 and (createdtime between '.$x1.' and '.$x2.') and monitorid=(SELECT id from monitoring where id='.$monitornn.' and clientid='.$_POST['Report']['clientid'].') ')->queryAll();
			...
			$parent= Yii::app()->db->createCommand('SELECT *,sum(value) as total FROM mobileworkorderdata_view where  clientbranchid='.$_POST['Report']['clientid'].' and monitortype='.$_POST['Monitoring']['mtypeid'].' and isproduct=0 and (createdtime between '.$x1.' and '.$x2.') and monitorid=(SELECT id from monitoring where id='.$monitornn.' and clientid='.$_POST['Report']['clientid'].')')->queryAll();
