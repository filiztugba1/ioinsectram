...
<?php if (Yii::app()->user->checkAccess('sector.view')){ ?>
<!-- Sayfada neredeyiz -->
<?=User::model()->geturl('Sector','',0,'sector');?>
<?php if (Yii::app()->user->checkAccess('sector.create')){ ?>
			...
			<div class="card">
				 <div class="card-header">
						 <div class="row" style="padding-bottom: 10px;border-bottom: 1px solid #f8f8f9;">
							 <div class="col-md-6">
								  <h4  class="card-title"><?=t('New Sector Create');?></h4>
									</div>
									 <div class="col-md-6">
				...
				<form id="sector-form" action="/sector/create" method="post">
				<div class="card-content">
					...
					<label for="basicSelect"><?=t('Name');?></label>
                        <fieldset class="form-group">
                          <input type="text" class="form-control" id="basicInput" placeholder="<?=t('Name');?>" name="Sector[name]">
                        </fieldset>
                    </div>

					<input type="hidden" value='1' name="Sector[active]">
					...
					<label for="basicSelect"><?=t('Parent');?></label>
                       <fieldset class="form-group">
                          <select class="custom-select block" id="customSelect" name="Sector[parentid]">
						  <option value="0"><?=t('Select');?></option>
						  <?php foreach($sector as $sectorx){?>
                            <option value="<?=$sectorx->id;?>"><?=t($sectorx->name);?></option>
							<?php }?>
                            </select>
                        </fieldset>
                    </div>
                ...
                <div class="card-header">
					<div class="row" style="border-bottom: 1px solid #e3ebf3;">
					   <div class="col-xl-9 col-lg-9 col-md-9 mb-1">
						 <h4 class="card-title"><?=t('SECTOR LIST');?></h4>
						</div>

						<?php if (Yii::app()->user->checkAccess('sector.create')){ ?>
						<div class="col-xl-3 col-lg-3 col-md-3 mb-1">
								<div class="input-group-append" id="button-addon2" style="float: right; text-align: right;">
									<button class="btn btn-info" id="createbutton" type="submit"><?=t('Add Sector');?> <i class="fa fa-plus"></i></button>
								</div>
                      ...
                      <table class="table table-striped table-bordered dataex-html5-export">
                        ...
                        <tbody>
             				<?php foreach($sector as $sectors):?>
                                <tr>
								<td><input type="checkbox" name="Sector[id][]" class='sec' value="<?=$sectors->id;?>"></td>
                                    <td><?=t($sectors->name);?></td>


									  <?php if($sectors->parentid==0){?>
										<td>-</td>
									  <?php }else{
										  $sectorparent=Sector::model()->find(array(
												   #'select'=>'title',
													'condition'=>'id=:id',
													'params'=>array(':id'=>$sectors->parentid),
												));?>

									 <td><?=t($sectorparent->name);?></td>
									 <?php }?>

										<td>
									<div class="form-group pb-1">
										<input type="checkbox" id="switchery"  data-size="sm" class="switchery" data-id="<?=$sectors->id;?>"  <?php if($sectors->active==1){echo "checked";}?> <?php if (Yii::app()->user->checkAccess('sector.list.active')==0){?>disabled<?php }?> />
									</div>
								</td>


									<td>
									<?php if (Yii::app()->user->checkAccess('sector.update')){ ?>
										 <a  class="btn btn-warning btn-sm" onclick="openmodal(this)" data-id="<?=$sectors->id;?>" data-name="<?=$sectors->name;?>" data-active="<?=$sectors->active;?>" data-parentid="<?=$sectors->parentid;?>"
										 data-toggle="tooltip" data-placement="top" title="" data-original-title="<?=t('Update');?>"
										 ><i style="color:#fff;" class="fa fa-edit"></i></a>
									<?php }?>
									<?php if (Yii::app()->user->checkAccess('sector.delete')){ ?>
										<a class="btn btn-danger btn-sm" onclick="openmodalsil(this)" data-id="<?=$sectors->id;?>"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-warning white">
                            <h4 class="modal-title" id="myModalLabel8"><?=t('Sector Update');?></h4>
					...
					<!--form baslangıç-->
						<form id="sector-form" action="/sector/update/0" method="post">
                            <div class="modal-body">
								<input type="hidden" class="form-control" id="modalsectorid" name="Sector[id]" value="0">
								<div class="col-xl-12 col-lg-12 col-md-12 mb-1">
								<label for="basicSelect"><?=t('Name');?></label>
									<fieldset class="form-group">
										<input type="text" class="form-control" id="modalsectorname" placeholder="<?=t('Name');?>" name="Sector[name]" value="">
									</fieldset>
								</div>
								...
								<label for="basicSelect"><?=t('Active');?></label>
									<fieldset class="form-group">
										 <select class="custom-select block" id="modalsectoractive" name="Sector[active]">
									...
									<label for="basicSelect"><?=t('Parent');?></label>
									<fieldset class="form-group">
										<select class="custom-select block" id="modalsectorparentid" name="Sector[parentid]">
										<option value="0"><?=t('Select');?></option>
											<?php foreach($sector as $sectorx){?>
											<option value="<?=$sectorx->id;?>"><?=t($sectorx->name);?></option>
											<?php }?>
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="sil" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
                ...
                <div class="modal-dialog" role="document">
					...
					<!--form baslangıç-->
						<form id="sector-form" action="/sector/delete/0" method="post">

						<input type="hidden" class="form-control" id="modalsectorid2" name="Sector[id]" value="0">
	...
	<!--delelete all start-->
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="deleteall" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
				...
				<!--form baslangıç-->
						<form action="/sector/deleteall" method="post">

						<input type="hidden" class="form-control" id="modalid3" name="Sector[id]" value="0">
...
{
	$('#modalsectorid').val($(obj).data('id'));
	$('#modalsectorname').val($(obj).data('name'));
	$('#modalsectoractive').val($(obj).data('active'));
	$('#modalsectorparentid').val($(obj).data('parentid'));
	$('#duzenle').modal('show');
...
{
	$('#modalsectorid2').val($(obj).data('id'));
	$('#sil').modal('show');
...
{
$.post( "?", { sectorid: data, active: permission })
