											...
											if($who->type==2){
												$tclient=Client::model()->findAll(array('order'=>'name ASC','condition'=>'isdelete=0 and  firmid='.$who->id.' and mainclientid!=0 and mainfirmid!=firmid group by mainclientid'));
														foreach($tclient as $tclientx)
														{
															...
															<?php $tclientbranchs=Client::model()->findAll(array('condition'=>'isdelete=0 and  parentid='.$tclientsx->id.' and firmid='.$who->id));
...
$.post( "/documents/documentlist?id="+document.getElementById("categoriid").value+'&&firmid=<?=$who->id;?>&&firmtype=<?=$who->type;?>').done(function( list ) {
 ...
 $("#documents-form-create").on('submit',(function(e) {
   ...
   url:"/documents/create3",
	...
	// alert(data);
      	$.post( "/documents/documentlist?id="+document.getElementById("categoriid").value+'&&firmid=<?=$who->id;?>&&firmtype=<?=$who->type;?>&status='+arsivStatus).done(function( list )
		{
		   	$.unblockUI();
			$('#documentlist').html(list);
			$('[data-toggle="tooltip"]').tooltip({
...
if($(obj).data('status'))
{
	...
	$.post( "/documents/documentlist?id="+$(obj).data('id')+'&&firmid=<?=$who->id;?>&&firmtype=<?=$who->type;?>&status='+status).done(function( data ) {
