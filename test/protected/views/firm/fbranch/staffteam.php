...
$user=User::model()->findAll(array(
 ...
 {
	$userisactive=' and active=1';
 }
else if(isset($_GET['status']) && $_GET['status']==2)
{
	$userisactive=' and active=0';
}
else
{
	$userisactive='';
...
$staffteams= Staffteam::model()->findAll(array(
									'condition'=>'firmid='.$_GET['id'].$userisactive,
							   ));
				...
				<div class="card-header" style="">
					<ul class="nav nav-tabs">
						...
						<?php if($type=="branch" && $ax->branchid==0){?>
						   <li class="nav-item">
							<a class="nav-link active"  href="/firm/staffteam?type=<?=$type;?>&&id=<?=$_GET['id'];?>"><span class="btn-effect2" style="font-size: 15px;"><?php echo count( $staffteams);?></span><?=t('Staffteam');?></a></a>
						  </li>
						 <?php }?>
...
<div class="row" id="createpage" >
	<div class="col-xl-12 col-lg-12 col-md-12">
			...
			<div class="card">
				...
				<form id="staffcreate-form" action="/staffteam/create" method="post">
				<div class="card-content">
					<div class="card-body">

					 <input type="hidden" class="form-control" id="basicInput" name="Staffteam[firmid]" value="<?=$_GET['id'];?>">
					...
					<div class="col-xl-5 col-lg-5 col-md-5 mb-1">
                        ...
                        <fieldset class="form-group">
                          <input type="text" class="form-control" id="teamname" placeholder="<?=t('Team Name');?>" name="Staffteam[teamname]" required>
                        </fieldset>
                    </div>
					...
					<label for="basicSelect"><?=t('TEAM LEADER');?></label>
                        <fieldset class="form-group">
                          ...
                          <select class="select2" style="width:100%" id="leader" onchange="staff()" name="Staffteam[leaderid]" required>
             			...
             			<?php foreach($staffteams as $staffteam):?>
								...
								<td>
									<div class="form-group pb-1">
										<input type="checkbox" id="switchery" data-size="sm"  class="switchery" data-id="<?=$staffteam->id;?>"  <?php if($staffteam->active==1){echo "checked";}?>  <?php if($staffteam->active==1){echo "checked";}?> <?php if (Yii::app()->user->checkAccess('staffteam.update')==0){?>disabled<?php }?> />
									</div>
								</td>
									...
									<a  class="btn btn-warning btn-sm" onclick="openmodal(this)" style='margin-right:2px'
							...
							data-id="<?=$staffteam->id;?>"><i style="color:#fff;" class="fa fa-trash"></i></a>
