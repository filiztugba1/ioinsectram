        ...
        <div class="card-header" style="">
            <ul class="nav nav-tabs">
					...
					<?php if (Yii::app()->user->checkAccess('firm.staff.view')){ ?>
                      <li class="nav-item">
                 		 <a class="nav-link"  href="/firm/staff?type=<?=$type;?>&&id=<?=$_GET['firmid'];?>" ><span class="btn-effect2" style="font-size: 15px;">
							<?=	count($say);?>
							</span> <?=t('Staff');?></a>
					 ...
					 <?php if($type=="branch" && $ax->branchid==0){?>
						   <li class="nav-item">
							<a class="nav-link"  href="/firm/staffteam?type=<?=$type;?>&&id=<?=$_GET['firmid'];?>"><span class="btn-effect2" style="font-size: 15px;"><?php echo count( $staffteams);?></span><?=t('Staffteam');?></a></a>
						  </li>
						 <?php }?>
					...
					<?php if($type=="branch" && $ax->branchid==0){?>
                       <li class="nav-item">
                        <a class="nav-link "  href="/firm/client?type=<?=$type;?>&&id=<?=$_GET['firmid'];?>"><span class="btn-effect2" style="font-size: 15px;"><?php echo count( $client)+count( $tclient);?></span><?=t('Client');?></a></a>
                      </li>
					 <?php }?>

             <li class="nav-item">
                        <a class="nav-link active"  href="<?=Yii::app()->baseUrl?>/firm/reports?id=<?=$_GET['firmid'];?>&firmid=<?=$ax->branchid;?>" ><span class="btn-effect" style="font-size: 15px;"><i class="fa fa-bar-chart-o" style="font-size: 15px;"></i></span><?=t('Reports');?></a></a>
                    </li>
            </ul>
        </div>
    ...
    <div class="card">
        ...
        <form id="reports-form" action="/client/reportcreate" method="post" target="_blank">
            <div class="card-content">
                ...
                <input type="hidden" class="form-control" id="basicInput"  name="Report[firm]" value="<?=$_GET['firmid'];?>">
                ...
                <select class="select2" style="width:100%" id="client" multiple="multiple" name="Report[clientid][]"  required onchange="myFunctionClient()">
                  ...
                  $clientbranchs=Client::model()->findAll(array('condition'=>'isdelete=0 and parentid='.$client->id));
									foreach($clientbranchs as $clientbranch)
									{
										...
										{
										?>
										<option <?php if($clientbranch->id==$workorder->clientid){echo "selected";}?> value="<?=$clientbranch->id;?>"><?=$client->name;?> -> <?=$clientbranch->name;?></option>
