...
$visittypesx=Visittype::model()->findAll(array('order'=>'name ASC','condition'=>$where));
//$visittypes=Visittype::model()->findAll(array('order'=>'name ASC'));?>


<?php if (Yii::app()->user->checkAccess('visittype.view')){ ?>
<!-- Sayfada neredeyiz -->
<?=User::model()->geturl('Visit Type','',0,'visittype');?>
<?php if (Yii::app()->user->checkAccess('visittype.create')){ ?>
			...
			<div class="card">
				    <div class="card-header">
						 <div class="row" style="padding-bottom: 10px;border-bottom: 1px solid #f8f8f9;">
							 <div class="col-md-6">
								  <h4  class="card-title"><?=t('Visit Type Create');?></h4>
									</div>
									 <div class="col-md-6">
				...
				<form id="visittype-form" action="/visittype/create" method="post">
				<div class="card-content">
					...
					<div class="row">
						...
						<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
							...
							<fieldset class="form-group">
								<select class="select2" style="width:100%" id="firm" name="Visittype[firmid]" onchange="myfirm()" requred>
						...
						<?php }else{?>
							<input type="hidden" class="form-control" id="firm" name="Visittype[firmid]" value="<?=$ax->firmid;?>" requred>
						<?php }?>
						...
						<label for="basicSelect"><?=t('Branch');?></label>
							<fieldset class="form-group">
								<select class="select2" style="width:100%" id="branch" name="Visittype[branchid]" disabled requred>
									<option value="0"><?=t('Please Chose');?></option>
						...
						<?php }else{?>
							<input type="hidden" class="form-control" id="branch" name="Visittype[branchid]" value="<?=$ax->branchid;?>" requred>
						<?php }?>
					...
					<label for="basicSelect"><?=t('Name');?></label>
                        <fieldset class="form-group">
                          <input type="text" class="form-control" id="basicInput" placeholder="<?=t('Name');?>" name="Visittype[name]" requred>
                        </fieldset>
                    </div>
					...
					<label for="basicSelect"><?=t('Is Active');?></label>
                       <fieldset class="form-group">
                          <select class="custom-select block" id="customSelect" name="Visittype[isactive]">
                ...
                <div class="card-header">
					<div class="row" style="border-bottom: 1px solid #e3ebf3;">
					   <div class="col-xl-9 col-lg-9 col-md-9 mb-1">
						 <h4 class="card-title"><?=t('VISIT TYPE LIST');?></h4>
						</div>

						<?php if (Yii::app()->user->checkAccess('visittype.create')){ ?>
						<div class="col-xl-3 col-lg-3 col-md-3 mb-1">
								<div class="input-group-append" id="button-addon2" style="float: right; text-align: right;">
									<button class="btn btn-info" id="createbutton" type="submit"><?=t('Add Visit Type');?> <i class="fa fa-plus"></i></button>
								</div>
						</div>
						<?php }?>
					</div>
                ...
                <div class="card-content collapse show">
                  <div class="card-body card-dashboard">
                      ...
                      <table class="table table-striped table-bordered dataex-html5-export">
                        ...
                        <tbody>
             			<?php foreach($visittypesx as $visittypes):?>
										<?php										$deleteUpdateActive=0;
										if($visittypes->firmid!=0 && ($ax->firmid==$visittypes->firmid || $ax->firmid==0))
										{
											$deleteUpdateActive=1;
										}
										?>
                                <tr>
								<td>
										<?php if (Yii::app()->user->checkAccess('visittype.delete') && $deleteUpdateActive){ ?>
											<input type="checkbox" name="Visittype[id][]" class='sec' value="<?=$visittypes->id;?>">
										<?php }?></td>
                                    <td><?=t($visittypes->name);?></td>

								<?php if($ax->firmid==0){?>
								<td><?=Firm::model()->find(array('condition'=>'id='.$visittypes->firmid))->name;?></td>
								<?php }?>

								<?php if($ax->branchid==0){?>
								<td><?=Firm::model()->find(array('condition'=>'id='.$visittypes->branchid))->name;?></td>
								<?php }?>


								<td>
									<?php if (Yii::app()->user->checkAccess('visittype.update') && $deleteUpdateActive){ ?>
									<div class="form-group pb-1">
										<input type="checkbox" id="switchery" data-size="sm"  class="switchery" data-id="<?=$visittypes->id;?>"  <?php if($visittypes->isactive==1){echo "checked";}?>  <?php if (Yii::app()->user->checkAccess('visittype.update')==0){?>disabled<?php }?> />
									</div>
									<?php }?>
								</td>
									...
									<td>
									<?php if (Yii::app()->user->checkAccess('visittype.update') && $deleteUpdateActive){ ?>
										<a  class="btn btn-warning btn-sm" onclick="openmodal(this)" data-id="<?=$visittypes->id;?>" data-name="<?=$visittypes->name;?>" data-active="<?=$visittypes->isactive;?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?=t('Update');?>"><i style="color:#fff;" class="fa fa-edit"></i></a>
									<?php }?>
									<?php if (Yii::app()->user->checkAccess('visittype.delete') && $deleteUpdateActive){ ?>

										<?//echo $ax->firmid;
										//echo $visittypes->firmid;

										//echo $ax->branchid;
										// echo $visittypes->branchid;


										?>
									<?php if($ax->firmid==0 || ($visittypes->firmid==$ax->firmid)|| (($visittypes->firmid==$ax->firmid) && ($visittypes->branchid==$ax->branchid))){?>
									<a class="btn btn-danger btn-sm" onclick="openmodalsil(this)" data-id="<?=$visittypes->id;?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?=t('Delete');?>"><i style="color:#fff;" class="fa fa-trash"></i></a>
                        ...
                        <tfoot>
                          <tr>
						   <th style='width:1px;'>
						   <?php if (Yii::app()->user->checkAccess('visittype.delete')){ ?>
								<div class="input-group-append" id="button-addon2" style="float: right; text-align: right;">
...
<?php if (Yii::app()->user->checkAccess('visittype.update')){ ?>
<!-- G�NCELLEME BA�LANGI�-->
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-warning white">
                            <h4 class="modal-title" id="myModalLabel8"><?=t('Visit Type Update');?></h4>
					...
					<!--form baslang��-->
						<form id="visittype-form" action="/visittype/update/0" method="post">
                            <div class="modal-body">
								<input type="hidden" class="form-control" id="modaltypeid" name="Visittype[id]" value="0">
								<input type="hidden" class="form-control" id="basicInput" placeholder="<?=t('Name');?>" name="Visittype[firmid]" value="<?=$ax->firmid;?>">
								<div class="col-xl-12 col-lg-12 col-md-12 mb-1">
								<label for="basicSelect"><?=t('Name');?></label>
									<fieldset class="form-group">
										<input type="text" class="form-control" id="modaltypename" placeholder="<?=t('Name');?>" name="Visittype[name]" value="">
									</fieldset>
								</div>
								...
								<label for="basicSelect"><?=t('Is Active');?></label>
									<fieldset class="form-group">
										 <select class="custom-select block" id="modaltypeisactive" name="Visittype[isactive]">
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-danger white">
                            <h4 class="modal-title" id="myModalLabel8"><?=t('Visit Type Delete');?></h4>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
                            </button>
                        </div>
					...
					<!--form baslang��-->
						<form id="visittype-form" action="/visittype/delete/0" method="post">

						<input type="hidden" class="form-control" id="modaltypeid2" name="Visittype[id]" value="0">
	...
	<!--delelete all start-->
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="deleteall" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
				...
				<!--form baslang��-->
						<form action="/visittype/deleteall" method="post">

						<input type="hidden" class="form-control" id="modalid3" name="Visittype[id]" value="0">
...
function openmodal(obj)
{
	...
	$('#modaltypeactive').val($(obj).data('isactive'));
	$('#duzenle').modal('show');
