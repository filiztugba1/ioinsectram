...
<?php
/*
Body Content Footer Ba�lang�c�
## Bu k�s�mda g�vdeye yaz�lan  contente ait toparlay�c� divleri kapat�yoruz
*/
...
<?php
/*
Body Content Footer Biti�i
##
*/
?>
<?php /*
  <div class="customizer border-left-blue-grey border-left-lighten-4 d-none d-xl-block"><a class="customizer-close" href="#"><i class="ft-x font-medium-3"></i></a><a class="customizer-toggle bg-danger" href="#"><i class="ft-settings font-medium-3 fa-spin fa fa-spin fa-fw white"></i></a><div class="customizer-content p-2 ps-container ps-theme-dark" data-ps-id="a74c2828-70e4-f851-978a-e9742f8cb6b4">
  ...
  */ ?>
  ...
  <footer class="footer footer-static footer-light navbar-border">
    <p class="clearfix blue-grey lighten-2 text-sm-center mb-0 px-2">
      <span class="float-md-left d-block d-md-inline-block">Copyright &copy; 2018 <a class="text-bold-800 grey darken-2" href="http://www.datahan.com.tr"
        target="_blank">DATAHAN </a>, All rights reserved. </span>
      <span class="float-md-right d-block d-md-inline-block d-none d-lg-block">InsectramCRM<i class="ft-heart pink"></i></span>
    </p>
  </footer>
  <!-- BEGIN VENDOR JS-->
  ...
  <script src="<?=Yii::app()->theme->baseUrl.'/'?>app-assets/vendors/js/extensions/unslider-min.js" type="text/javascript"></script>
  <script src="<?=Yii::app()->theme->baseUrl.'/'?>app-assets/vendors/js/timeline/horizontal-timeline.js" type="text/javascript"></script>
  <!-- END PAGE VENDOR JS-->
  <!-- BEGIN STACK JS-->
...
function menu(obj)
{
	$.post( "/generalsettings/menu").done(function( data ) {
		if(data==1)
		{
...
function openmodaldurum(obj)
{
	$.post( "/notifications/getn")
	 .done(function( data ) {
...
setInterval(function(){
		$.post( "/notifications/getn/1").done(function( data ) {
		as=data;
		result = as.split('|');
		result2='<center>'+result[1]+'</center>';
		console.log(x);

		if(x<result[0])
		{
			x=as.trim();
			toastr.success(result2,"<center><a style='color:#fff' href='/notifications'><?=t('New Notification')?></a></center>" , {
...
<script>
	...
	var x=document.getElementById("livesearch2").value;
	  if (x=="") {
		$('#livesearch').html('');
	  }
	  else
	  {
		  $.post( "/client/livesearch?q="+x).done(function( data ) {
			  $('#livesearch').show();
  ...
  xmlhttp.open("GET","/client/livesearch?q="+str,true);
  xmlhttp.send();
...
$(document).ready(function(){
	$(".vertical-scroll").perfectScrollbar({suppressScrollX:!0,theme:"dark",wheelPropagation:!0});
});
...
/*
$funtion(){
$('.fc-basic-view > table').addClass('table-responsive');
$('.fc-widget-header > table').addClass('table-responsive');
})
*/
</script>
 ...
 <?php
	$scriptlist=explode(';',Yii::app()->params['scripts']);
	foreach($scriptlist as $item)
	{
		if (strlen($item)>=3)
			{
			echo '  <script src="'.$item.'?1" type="text/javascript"></script>'."\r\n";
			}
	}
  ?>
  <!-- END PAGE LEVEL JS-->
...
if ($flashMessages) {
    ...
    foreach($flashMessages as $key => $message) {
		...
		?>
		toastr.<?=$key?>("<center><?=str_replace('"','-',$message)?></center>", "<center><?=str_replace('"','-',$keytitle)?>!</center>", {
				positionClass: "toast-top-right",
				containerId: "toast-top-right"
		});
  <?php  }
