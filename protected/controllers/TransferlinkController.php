...
class TransferlinkController extends Controller
{
	...
	public function actionCreate()
	{
      ...
      //// uygunsuzluklar transfer edilecek
      $conformitys=Conformity::model()->findAll(array('condition'=>'clientid='.$tclient->id));
      foreach($conformitys as $conformity)
      {
        $cmodel=Conformity::model()->find(array('condition'=>'id='.$conformity->id));
        $cmodel->firmbranchid=$tclient->firmid;
        $cmodel->branchid=$tclient->firmid;
        $cmodel->save();
      }
