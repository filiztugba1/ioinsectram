...
class MobileController extends Controller
{
    ...
    //UPDATE `mobileworkordermonitors` SET `checkdate` = '1651905361' WHERE `mobileworkordermonitors`.`workorderid` = 1593271; `workorderid` = 24714 
    //UPDATE `mobileworkorderdata` SET `openedtimeend` = '1651748641' WHERE `mobileworkorderdata`.`workorderid` = 9088856;
		...
		if($model->save())
		{
      if(isset($_POST["acikMonitorler"]) && intval($_POST["acikMonitorler"])==1)
      {
        $modelMWM=Mobileworkordermonitors::model()->findAll(array('condition'=>'workorderid='.$_POST["id"]));
       
         
        foreach($modelMWM as $modelMWMx)
        {
       
          if($modelMWMx->checkdate==0 || $modelMWMx->checkdate==null)
          {
          
            $modelMWMGuncelle=Mobileworkordermonitors::model()->findByPk($modelMWMx->id);
            $modelMWMGuncelle['checkdate']=$timestamp2;
            $modelMWMGuncelle->save();
          }
        }
      }
      else  if(isset($_POST["acikMonitorler"]) && intval($_POST["acikMonitorler"])==2)
      {
         $modelMWM=Mobileworkorderdata::model()->findAll(array('condition'=>'workorderid='.$_POST["id"]));
        foreach($modelMWM as $modelMWMx)
        {
           $modelD=Mobileworkorderdata::model()->findByPk($modelMWMx['id']);
     
          if($modelMWMx['createdtime']==0 || $modelMWMx['createdtime']==null)
          {
            ...
            $modelM['checkdate']=$timestamp2;
              $modelM->saverid=intval($ax->id);
              $modelM->update();
              ///
              $modelD->createdtime=$yenitarih;
              $modelD->saverid=intval($ax->id);
      ...
      } else  if(isset($_POST["acikMonitorler"]) && intval($_POST["acikMonitorler"])==3)
      {
...
$wo->update();
	...
	public function actionUpdatekntrledilmedi()
	{
		...
		if(isset($_POST['id']))
		{
			...
			$check=Mobileworkorderdata::model()->find(array('condition'=>'petid=49 and workorderid='.$modelD->workorderid.'  and monitorid='.$modelD->monitorid));
			if($check)
			{
     
				$check->pettype=0;	
				$check->petid=49;
				$check->value=$_POST['tur']; // 0-Normal 1- Lost 2- Broken 3- Unreacheble
        $check->save();
				echo "success";
			}
			else
	...
	public function actionUpdatedata()
    {
		...
		foreach ($mobileworkordatas as $mobileworkordata){
			...
			$veriler =$veriler.  'data-id'.$say.'="'.$mobileworkordata->id.'"
						  ...
						  data-value'.$say.'="'.$val.'"';
			$goster= $goster .' '. t($pet->name). ' : '.$mobileworkordata->value.' <br>';
		}
