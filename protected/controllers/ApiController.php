...
class ApiController extends Controller
{
   ...
   public function actionMonitorTypes(){
       if (isset($_GET['key']) && $_GET['key']=="123demo123"){
                    $list_type=[];
           $types=Monitoringtype::model()->findAll(array());
           foreach($types as $item){
             $list_type[$item->id]['id']=$item->id;
             $list_type[$item->id]['name']=strtolower($item->name);
             $list_type[$item->id]['detailed']=$item->detailed;
           }
       
       
            $list=[];
            $monitors=Monitoring::model()->findAll(array('condition'=>'clientid=133'));
            $sira=0;
   ...
   public function actionMonitors(){
     if (isset($_GET['key']) && $_GET['key']=="123demo123"){
      ...
      // $list['mtypes']=$list_type;
           foreach($monitors as $item){
       
             
              $monitor_data = Mobileworkorderdata::model()->find(array(
      'select'=>'id, SUM(value) as value',
      'condition'=>'id>13085084 and workorderid=35895 and monitorid='.$item->id)
    );

$monitor_data=$monitor_data->value;
             
           
             /*$list['all'][$sira]['monitor_id']=$item->id;
             $list['all'][$sira]['monitor_no']=$item->mno;
             $list['all'][$sira]['monitor_value']=$monitor_data;
             $list['all'][$sira]['monitor_type_id']=$item->mtypeid;
             $list['all'][$sira]['monitor_type_name']=$list_type[$item->mtypeid]['name'];
             $list['all'][$sira]['monitor_type_detail']=$list_type[$item->mtypeid]['detailed'];
             $list['all'][$sira]['mnitor_location_text']=$item->definationlocation;
             */
             $listtemp=[];
             $listtemp['monitor_id']=$item->id;
             $listtemp['monitor_no']=$item->mno;
             $listtemp['monitor_value']=$monitor_data;
             $listtemp['monitor_type_id']=$item->mtypeid;
             $listtemp['monitor_type_name']=$list_type[$item->mtypeid]['name'];
             $listtemp['monitor_type_detail']=$list_type[$item->mtypeid]['detailed'];
             $listtemp['mnitor_location_text']=$item->definationlocation;
             $list[$list_type[$item->mtypeid]['name']][]=$listtemp;
             $sira++;
           }
...
public function actionInsectrampackage1(){
  $user=$this->autoauth();
...
*/
              ...
              $pet=Pets::model()->findByPk($workorderdata->petid);
               ...
               $title='';
                ...
                $monitype=Monitoringtype::model()->findByPk($workordermonitor->monitortype);
          ...
          array_push($listworkorder,array(
              ...
              'start_time'=>$workorder->start_time,
              'finish_time'=>$workorder->finish_time,
              'service_report_ok'=> $rprt,//$modelServiceR != null,
              'visittypename'=>$visittype->name,
              'clientname'=>$workclient->name,
              'todo'=>$workorder->todo,
              'branchid'=>$workorder->clientid,
...
$clientler=array();
 ...
 // $listfirms=$this->actionMobileclients($arrayClientsids);
	...
	$str=json_encode(array('Firms'=>$listfirms,'Workorders'=>$listworkorder));
	//$varmiencrypt=Superlogs::model()->find(array('condition'=>'encrypted="'.md5($str).'" and userid!=0'));

	$filenamex='apilogs_v2/'.time().'-cache.txt';
...
public function actionPackage()
{
    ...
    { // WorkOrders
		...
		{
	//	if($workorders[$i]->customer_service_report->signer_name <> "null" && $workorders[$i]->customer_service_report->riskreview != "null")
		if($workorders[$i]->customer_service_report->signer_name <> "null")
        {
        ...
        if (isset($workorders[$i]->customer_service_report->service_report_picture)){
            ...
            rename('c.txt', $path3);
   			...
   			$modelS->pr_name_surname='';
                      ...
                      if (isset($workorders[$i]->customer_service_report->pr_name_surname)){
                        echo $workorders[$i]->customer_service_report->pr_name_surname;
				                $modelS->pr_name_surname=$workorders[$i]->customer_service_report->pr_name_surname;
                        }
                      if (isset($workorders[$i]->customer_service_report->signature_date)){
                        echo $workorders[$i]->customer_service_report->signature_date;
				                $modelS->signature_date=$workorders[$i]->customer_service_report->signature_date;
                        }
        
        
                      if (isset($workorders[$i]->customer_service_report->treatment_and_observations)){
                        echo $workorders[$i]->customer_service_report->treatment_and_observations;
				                $modelS->treatment_and_observations=$workorders[$i]->customer_service_report->treatment_and_observations;
                        }
       
                      if (isset($workorders[$i]->customer_service_report->housekeeping)){
                        echo $workorders[$i]->customer_service_report->housekeeping;
				                $modelS->housekeeping=$workorders[$i]->customer_service_report->housekeeping;
                        }
        
                      if (isset($workorders[$i]->customer_service_report->proofing)){
                        echo $workorders[$i]->customer_service_report->proofing;
				                $modelS->proofing=$workorders[$i]->customer_service_report->proofing;
                        }
                      if (isset($workorders[$i]->customer_service_report->visit_report_summary)){
                        echo $workorders[$i]->customer_service_report->visit_report_summary;
				                $modelS->visit_report_summary=$workorders[$i]->customer_service_report->visit_report_summary;
                        }
        
        
                      if (isset($workorders[$i]->customer_service_report->team_leader)){
                        echo $workorders[$i]->customer_service_report->team_leader;
				                $modelS->team_leader=$workorders[$i]->customer_service_report->team_leader;
                               
                       

                        }
        
        if (isset($workorders[$i]->customer_service_report->follow_up_date) && $workorders[$i]->customer_service_report->follow_up_date<>'') {
                         $duplicate=Workorder::model()->find('id=:ID',array(':ID'=>$workorders[$i]->id));
                        
          if($workorders[$i]->customer_service_report->follow_up_start_time==''){
            $workorders[$i]->customer_service_report->follow_up_start_time='08:00';
            $workorders[$i]->customer_service_report->follow_up_end_time='09:00';
          }
          
          if($workorders[$i]->customer_service_report->follow_up_end_time==''){
            $workorders[$i]->customer_service_report->follow_up_start_time='08:00';
            $workorders[$i]->customer_service_report->follow_up_end_time='09:00';
          }
...
$modelwork->date=$workorders[$i]->customer_service_report->follow_up_date;
$modelwork->start_time=$workorders[$i]->customer_service_report->follow_up_start_time;
$modelwork->finish_time=$workorders[$i]->customer_service_report->follow_up_end_time;
...
$modelwork->save();
Workorder::model()->addmonitortotifollow($modelwork->id);
                       }
                    
                      if (isset($workorders[$i]->customer_service_report->pest_types)){
                        echo $workorders[$i]->customer_service_report->pest_types;
				                $modelS->pest_types=$workorders[$i]->customer_service_report->pest_types;
                        }
                      if (isset($workorders[$i]->customer_service_report->workplace_and_soon)){
                        echo $workorders[$i]->customer_service_report->workplace_and_soon;
				                $modelS->workplace_and_soon=$workorders[$i]->customer_service_report->workplace_and_soon;
                        }
                    
                      if (isset($workorders[$i]->customer_service_report->number_of_flats)){
                        echo $workorders[$i]->customer_service_report->number_of_flats;
				                $modelS->number_of_flats=$workorders[$i]->customer_service_report->number_of_flats;
                        }
                      if (isset($workorders[$i]->customer_service_report->pr_area_ground)){
                        echo $workorders[$i]->customer_service_report->pr_area_ground;
				                $modelS->pr_area_ground=$workorders[$i]->customer_service_report->pr_area_ground;
                        }
                      if (isset($workorders[$i]->customer_service_report->security_precautions)){
                        echo $workorders[$i]->customer_service_report->security_precautions;
				                $modelS->security_precautions=$workorders[$i]->customer_service_report->security_precautions;
                        }
        foreach ($servicereports as $servicereport){
                          $modelA=new Activeingredients;
                          $modelA->workorderid=$workorders[$i]->id;
...
public function actionPackage3()
{
	$sure_baslangici = microtime(true);
	if(!isset($_REQUEST['deviceid'])){
		$_REQUEST['deviceid']='0';
      ...
      $modelM=Mobileworkordermonitors::model()->findByPk($workorders[$i]->workordermonitors[$j]->id);
		...
		//"new_barcode_value": "null",
            if($workorders[$i]->workordermonitors[$j]->new_barcode_value!="null") // Monitor Barcode Numarası Değiştir
            {
    			...
    			if($workorders[$i]->workordermonitors[$j]->checkdate!=0)
    			{
            //var_dump($workorders[$i]->workordermonitors[$j]);
    				$modelM->checkdate=substr($workorders[$i]->workordermonitors[$j]->checkdate,0,10);
           ...
           echo 'gg';
					...
					{
						if($workorders[$i]->workordermonitors[$j]->checkdate!=0 && $workorders[$i]->workordermonitors[$j]->monitorData[$k]->value != 0)
						{
             	$modelD->value=$workorders[$i]->workordermonitors[$j]->monitorData[$k]->value;
					echo 'gir';
							$modelD->saverid=$userid;
	...
	*/
    ...
    public function autoauth()
    {
        if (isset($_SERVER['HTTP_AUTHORIZATION'])){
           ...
           if (is_numeric($user->userlgid)){
                     	$ddil="tr";

                 	if ($user->userlgid*1=='1') //dil dosyasını arıyoruz.
                {	
                    echo $user->userlgid;
                    	$ddil="tr";
                  }
             
                 	if ($user->userlgid*1=='2') //dil dosyasını arıyoruz.
                {	
                    	$ddil="en";
                  }
    ...
    {
        $str=file_get_contents('php://input');
      	$filenamex='apilogs-conform/'.time().'-'.rand(1,1000).'-conformities.txt';
        file_put_contents($filenamex, $str);
              ...
              definition like '%".$gelen->definition."%' and
            	...
            	subdepartmentid=".$gelen->subdepartmentid));
  				if(!$varmi){
  					...
  					{
  						mkdir(Yii::getPathOfAlias('webroot').'/uploads/'.$firm->username.'/');
  						$imagename=rand(1,10000).'-'.time().".png";
  						$path=Yii::getPathOfAlias('webroot').'/uploads/'.$firm->username.'/'.$imagename;
  						rename('a.txt', $path);
  					}
  					else{
    ...
    public function actionGetmeds2()
    {
		$user = $this->autoauth();
        $listfirms=array();
        $meds=array();
    ...
    public function actionGetmeds()
    {
        ...
        $medfirms=Stokkimyasalkullanim::model()->findAll(array('condition'=>'isactive=1 and branchid='.$user->branchid.' or firmid='.$user->firmid));
		if(!$medfirms)
		{
			 $medfirms=Stokkimyasalkullanim::model()->findAll(array('condition'=>'isactive=1 and branchid=0 and firmid=0'));
		}
        foreach ($medfirms as $medfirm)
        {
            ...
            array_push($meds,array(
                    ...
                    'unit'=>($medfirm->urunAmbajBirimi==0?t('Adet'):
	($medfirm->urunAmbajBirimi==1?'gr':
	($medfirm->urunAmbajBirimi==2?'ml':
	($medfirm->urunAmbajBirimi==3?'gr':
	($medfirm->urunAmbajBirimi==4?'ml':'')
	)
	)
	))
                ));
