...
class ConformityController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('print','reports','index','view','client','department','updatediv','img','activity','index1','gunsonuemail','conformitytype','conformitystatus','deleteall','conformitystatusbutton','closed','assign'
				,'assigncreate','returnassign','uygunsuzlukListesi','newConformityList','filedelete','fileupdate'),
				'users'=>array('*'),
			),
	...
	public function actionNewConformityList()
	{
   ...
   // $mpdf->showImageErrors = true;
	...
	public function actionCreate()
	{
		...
		if(count($conformity)==0)
		{
			...
			/// tamamlama fotoğrafı
			$image2=CUploadedFile::getInstance($model,'filesf2');

			$model->numberid=$number;
			if(isset($image2))
			{
				$type=explode('.',$image2->name);
				$image2->saveas($path.'/'.$temTime.'_comp.'.$type[1]);
				$model->filesf2='/uploads/'.$firm->username.'/'.$temTime.'_comp.'.$type[1];
			}
			else
			{
				$model->filesf2="";
			...
			$model->date=strtotime($_POST['Conformity']['date'].' '.date('H:i:s'));
			//guncel number
			...
			$say=count($clix)+1;
			$de=date("y",strtotime($_POST['Conformity']['date'].' '.date('H:i:s')));
	 ...
	 public function actionFileupdate($id)
	{
		...
		if($_POST['Conformity']['filetype']=='filesf2')
		{
			if($_FILES['Conformity']["size"]["filesf2"]>0)
			{
				$image2=CUploadedFile::getInstance($model,'filesf2');
				if(isset($image2))
				{
					
						if(file_exists(Yii::getPathOfAlias('webroot').'/'.$_POST['Conformity']['filesfx2']) && $_POST['Conformity']['filesfx2']!=''){
							unlink(Yii::getPathOfAlias('webroot').'/'.$_POST['Conformity']['filesfx2']);
						}
					$type=explode('.',$image2->name);
					$image2->saveas($path.'/'.$temTime.'_comp.'.$type[1]);
					$model->filesf2='/uploads/'.$firm->username.'/'.$temTime.'_comp.'.$type[1];
				...
				{
					$model->filesf2=$_POST['Conformity']['filesfx2'];
				}
			}
			else
			{
				Yii::app()->user->setFlash('error', t('Öncelikle resim yüklemelisiniz.'));
				$this->redirect('/conformity/activity/'.$_POST['Conformity']['id']);
			}
		...
		{
			if($_FILES['Conformity']["size"]["filesf"]>0)
			{
				$image=CUploadedFile::getInstance($model,'filesf');
				if(isset($image))
				{
					if(file_exists(Yii::getPathOfAlias('webroot').'/'.$_POST['Conformity']['filesfx']) && $_POST['Conformity']['filesfx']!=''){
						unlink(Yii::getPathOfAlias('webroot').'/'.$_POST['Conformity']['filesfx']);
					}
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Conformity']))
		{
			...
			else
			{
			 ...
			 {
				$model->closedtime=strtotime($_POST['Conformity']['closedtime'].' '.date('H:i:s'));//+10800 utc kurtulmak için +11+1sn
			...
			if($_POST['Conformity']['statusid']==6)
			{
				...
				$user=User::model()->findAll(array('condition'=>'firmid=:firmid and branchid=:branchid and clientid='.$cbranch->parentid.' and type=22','params'=>array('firmid'=>$model->firmid,'branchid'=>$model->branchid)));
		...
		$model=Conformity::model()->find(array('condition'=>'id=:id','params'=>array('id'=>$_GET['id'])));
			...
			if(isset($_GET['status']) && ($_GET['status']==6 || $_GET['status']==1)	)
			{
				...
				*/
				...
				{
					$model->statusid=6;
					$model->closedtime=strtotime($_POST['Conformityactivity']['nokdate'].' '.date('H:i:s'));
			...
			if(isset($_GET['status']) && 	$_GET['status']==2)
			{
				...
				{
					 $model->closedtime=strtotime($_POST['Conformityactivity']['okdate'].' '.date('H:i:s'));
	 ...
	 public function actionFiledelete($id)
	{
		...
		if($_POST['Conformity']['filetype']=='filesf2')
		{
			...
			if(file_exists(Yii::getPathOfAlias('webroot').'/'.$model->filesf2) && $model->filesf2!=''){
				unlink(Yii::getPathOfAlias('webroot').'/'.$model->filesf2);
				$model->filesf2='';
			}
		}
		else
		{
				if(file_exists(Yii::getPathOfAlias('webroot').'/'.$model->filesf) && $model->filesf!=''){
				unlink(Yii::getPathOfAlias('webroot').'/'.$model->filesf);
				$model->filesf='';
			}
		}
		$model->save();
		...
		if(!isset($_GET['ajax']))
		{
			...
			/* Hataları sadece alttaki setflashes classı ile ayıklıyoruz!!! :)))*/
				...
				Yii::app()->SetFlashes->add($model,t('Delete Success!'),array('conformity/activity/'.$id));
				$this->redirect(array('conformity/activity/'.$id));
...
if(!(isset($_GET['depType']) && $_GET['depType']=='multiselect')){
					?>
	...
	public function actionClosed()
	{
		...
		$time->closedtime=strtotime($_POST['Conformity']['closedtime'].' '.date('H:i:s'));
		$time->save();
	...
	public function actionAssign()
	{
		...
		?>
			...
			<?
				...
				$usermodel=User::model()->find(array('condition'=>'id=:id and active=1','params'=>array('id'=>$model->userid)));
				...
				if($usermodel->clientid==0) //eger conformityi acan kişi admin-firm veya branch ise goncerilecek email
				{
					...
					foreach($ass as $assx)
					{
						$userx=User::model()->find(array('condition'=>'active=1 and id='.$assx->userid));
					...
					{
						$userx=User::model()->find(array('condition'=>'active=1 and id='.$ass2x->userid));
						if($userx->id!='' && $userx->id!=$ax->id){?>
						<option  value="<?=$userx->id;?>"><?if($userx->name=='' && $userx->surname==''){echo $userx->username;}else{echo $userx->name.' '.$userx->surname;}?></option>
					<?}}
	...
	public function actionReturnassign()
	{
	    ...
	    $model->id=0;
		$model->conformityid=intval($_POST['Conformity']['id']);
		$model->senderuserid=intval(User::model()->userobjecty('')->id);
		$model->recipientuserid=intval($_POST['Conformity']['senderuserid']);
		$model->sendtime=time();
		$model->returnstatustype=2;
		$model->parentid=intval($_POST['Conformity']['assignid']);
	...
	public function actionAssigncreate()
	{
        ...
        if($_POST['Conformity']['recipientuser']==0)
        {
            	Yii::app()->user->setFlash('error', t('Lütfen atanacak kullanıcıyı seçiniz'));
	        	$this->redirect(array('/conformity/activity/'.$_POST['Conformity']['id']));
		
        }
		if($_POST['Conformity']['cu']==1)
		{
			$model=Conformityuserassign::model()->find(array('condition'=>'id='.$_POST['Conformity']['assignidx']));
		}
		else
		{
			$model=new Conformityuserassign;
		...
		// $sender='filiz çürükcü';
		 $senderemail='info@insectram.io';
		 //$buyeremail='fcurukcu@gmail.com';
		 // $buyername='filiz çürükcü';
		Conformity::model()->email($senderemail,$sendername,$subject,$altbody,$msg,$buyeremail,$buyername);
