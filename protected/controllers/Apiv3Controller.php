 ...
 return $ip;
    ...
    public function getauth($u = "", $p = "", $onesignalid = "")
    {
        ...
        $user = User::model()->find([
            "condition" => "(email=:socialid) or (username=:socialid) and active=1",
            "params" => [":socialid" => $u],
        ]);
        if ($user === null) {
	...
	$Superlogs->deviceid=$_SERVER["HTTP_DEVICEID"].'-login-'.$user->username.'-auth:'.$user->code.'-ip:'.$ip;
	...
	/*
         array_push($listmenu2,array(
                      'name'=>'View Recom.',
                      'url'=>'https://alperbarutcu.com/insectramtest.php',
                      'login'=>true,
                  ));						
         array_push($listmenu2,array(
                      'name'=>'Create Recom.',
  ...
  $Superlogs= new Superlogs;
	...
	$Superlogs->encrypted='';
	$Superlogs->userid=$user->id;
	$Superlogs->deviceid=$_SERVER["HTTP_DEVICEID"].'-otoauth_problem_authcode_not_found- code:'.$code.'-ip:'.$ip;
	$Superlogs->createdtime=time();
	$Superlogs->save();
                ...
                if (isset($_SERVER["HTTP_DEVICEID"])) {
                                       $str.='httpdevice='.$_SERVER["HTTP_DEVICEID"].PHP_EOL;
                    ...
                    if (isset($_SERVER["HTTP_LANG"])) {
                     $str.='httplang='.$_SERVER["HTTP_LANG"].PHP_EOL;
  	 ...
  	 $filenamex='api_other_logs/'.time().'-'.$rand1.'-alper.txt';
  //	 $filenamex='apiv3-logs/'.md5($str).'-conformity.txt';
	...
	$Superlogs->encrypted='';
	$Superlogs->userid=$user->id;
	$Superlogs->deviceid=$_SERVER["HTTP_DEVICEID"].'-otoauth_device_not_found_logout- code:'.$code.'-ip:'.$ip;
	$Superlogs->createdtime=time();
	$Superlogs->save();
                        $this->result("logout");
                    }
	...
	$Superlogs->encrypted='';
	$Superlogs->userid=$user->id;
	$Superlogs->deviceid='otoauth_header_device_id_not_found- code:'.$code.'-ip:'.$ip;
	$Superlogs->createdtime=time();
	$Superlogs->save();
                    $this->result("logout");
                }
	...
	$Superlogs->encrypted='';
	$Superlogs->userid=$user->id;
	$Superlogs->deviceid='httpauth_not_found_empty-otoauth_problem-device:'.$_SERVER["HTTP_DEVICEID"].'-ip:'.$ip;
	$Superlogs->createdtime=time();
	$Superlogs->save();
            $this->result("Authorization problem. Please login..");
            exit();
    ...
    public function actionWorkorders()
    {
     ...
     /*   $userlog = new Userlog();
		...
		*/
        ...
        // $datefinish = date("Y-m-d", strtotime("+3 day"));
        // $workorders = Workorder::model()->findAll([
            // "condition" =>
                // 'date<="' .
                // $datefinish .
                // '" and  date >="'.$datestart.'" and status!=3 and branchid=' .
                // $ax->branchid,
            // "order" => "date ASC, start_time ASC",
        // ]);
		 ...
		 $datestart = date("Y-m-d", strtotime("-3 day"));
        $datefinish = date("Y-m-d", strtotime("+5 day"));
        $workorders = Workorder::model()->findAll([
            "condition" =>
                'date<="' .
                $datefinish .
                            ...
                            foreach ($workordermonitors as $workordermonitor) {
                                    ...
                                    [
                                        "condition" =>
                                            "mobileworkordermonitorsid=" .
                                            $workordermonitor->id,
                                        "order" => "FIELD(petid,25,85,86,35,34,36) asc, petid asc,monitorid asc, id asc",
                                    ]
                                );
																}
                                ...
                                if ($workorderdatas) {
                                  $xmonitorStatus=0;
                                    foreach (
                                      ...
                                      if ( $workorderdata->petid==49) {
                                        $xmonitorStatus=$workorderdata->value;
                                        continue;
                                ...
                                $monitorx = Monitoring::model()->findByPk($workordermonitor->monitorid);
                                $departmentsx = Departments::model()->findByPk($monitorx->subid);
                              $monitorInfo =$mtypename;
                              $depnamet='';
                              if ($departmentsx && strlen($departmentsx->name)>0){
                                $depnamet=$departmentsx->name;
                                 $monitorInfo .="\nD: ".  mb_substr($departmentsx->name, 0, 26);//str_split($departmentsx->name, 20)[0];// $mtypename . PHP_EOL . 
      
                              }
                             
                              if (strlen($monitorx->definationlocation) > 0 ){
                                $monitorInfo .="\nL: ". mb_substr($monitorx->definationlocation, 0, 26);
                                 //   $monitorInfo .= "\n".str_split($monitorx->definationlocation, 2)[0];// $mtypename . PHP_EOL . 
      
                              }
                              if (strlen($monitorx->definationlocation) > 26 ){
                                $monitorInfo .="\n".mb_substr($monitorx->definationlocation, 26, 60);
                                 //   $monitorInfo .= "\n".str_split($monitorx->definationlocation, 2)[0];// $mtypename . PHP_EOL . 
      
                              }
                               if (strlen($monitorx->techdescription)>0)  $monitorInfo .= "\nT: ". mb_substr($monitorx->techdescription, 0, 26);//.str_split($monitorx->techdescription, 20)[0];
                             
                            
                                    // substr($departmentsx->name ?? '', 0, 20) . PHP_EOL . 
              // substr($monitorx->definationlocation ?? '', 0, 20);
                            //  print_r($monitorInfo);
                                array_push($listmonitor, [
                                   ...
                                   // "monitortype" =>
                                       // $workordermonitor->monitortype,
                                    "monitortypename" => $monitorInfo,
                                    "barcodeno" =>
                                        "xx!!!xx" .
                                        $workordermonitor->barcodeno,
                                    "monitorStatus" => $xmonitorStatus,
                                    "checkdate" => $workordermonitor->checkdate,
                                    "synced" => true,
                                    "definationlocation" => $monitorx->definationlocation,
                                     "subid" => $monitorx->subid,
                                     "subname" => $depnamet,
                                     "techdescription" => $monitorx->techdescription,
                        ...
                        $modelServiceR = Servicereport::model()->find([
                            "condition" => "reportno=" . $workorder->id,
											...
											if ($workorder->visittypeid==62){
													  ...
													  $tilistarr = 	Tichecklists::model()->findAll(["condition" => "firm_id=" . $workorder->firmid,]);
													  $tilistarr = 	Tichecklists::model()->findAll(["condition" => "firm_id=0",]);
											...
											$clientinfo=[];
											$clientinfo['address']=$workclient->address.' '.$workclient->town_or_city.' '.$workclient->postcode;
                            ...
                            "condition" => "userid=" . $phoneuser->id,
							        ...
							        $workorderclientname=$workclient->name;
                      if ($workclient->mainfirmid==687){
                         $workorderclientname=$workclient->name.' (£'.$workclient->productsamount.')';
                        
                      }
                      $todoyaz=$workorder->todo;
                      if($workorder->special_notes<>''){
                        $todoyaz=$todoyaz."\nNote: ".$workorder->special_notes;
                      }
                        ...
                        array_push($listworkorder, [
                            ...
                            "clientname" => $workorderclientname,
                            "address" => $workclient->address.' '.$workclient->town_or_city.' '.$workclient->postcode,
                            "clientinfo" => $clientinfo,
                            "todo" => $todoyaz,
                            "branchid" => $workorder->clientid,
                            "branchname" => $workorderclientname,
...
*/
    ...
    // $ax = User::model()->userobjecty("");
       // $ax->branchid;
        ...
        if ($user->type== 13 ||  $user->clientid<>0) {
            $model = [];
        }else {
            $model = Yii::app()
                ->db->createCommand(
     ...
     // echo 'SELECT conformity.*,workorder.date as workorderdate,workorder.finish_time as finishtime,workorder.start_time FROM conformity INNER JOIN workorder ON workorder.clientid=conformity.clientid where
			//(conformity.statusid=4 or conformity.statusid=3) and workorder.status!=3 and workorder.branchid=' .$user["branchid"] ." group by conformity.id"; exit;
        $array = [];
            ...
            if ($workorderdate >= $model[$i]["date"]) {
                if ($model[$i]["statusid"] == 3) {
                   ...
                   array_push($array, [
                        ...
                        "status" => $model[$i]["statusid"],
                        "numberid" => $model[$i]["numberid"],
                        "definition" => strip_tags($model[$i]["definition"]),
                        "suggestion" => strip_tags($model[$i]["suggestion"]),
                        "priority" => $model[$i]["priority"],
                       // "date" => date("d-m-Y H:i", $model[$i]["date"]),
                        "date" => date("d-m-Y", $model[$i]["date"]),
                        "image" =>
  ...
  //	 $filenamex='apiv3-logs/'.md5($str).'-conformity.txt';
      ...
      file_put_contents($filenamex, $str);
          ...
          definition like '".addslashes(nl2br($gelen->definition))."' and
            ...
            $clibr=Client::model()->find(array('condition'=>'id='.$gelen->branchid));
						if ($clibr){
								$model->simple_client=$clibr->simple_client;
						}
	   ...
	   public function actionAddfindings()
    {
     ...
     $str=file_get_contents('php://input');
			 ...
			 $rand3=rand(2000000,3000000);
			 $rand4=rand(3000000,4000000);
  	 $filenamex='apiv3-findings-logs/'.time().'-'.$rand1.'-findings.txt';
  //	 $filenamex='apiv3-logs/'.md5($str).'-conformity.txt';
      ...
      // var_dump($gelen);
        ...
        foreach ( $gelen->findings as $item){
          
          $varmiencrypt=Findings::model()->find(array('condition'=>'workorder_id="'.$gelen->workOrderId.'" and unique_id="'.$item->id.'"'));
          ...
          if (!$varmiencrypt){
	          ...
	          $finding->workorder_id=$gelen->workOrderId;
	          $finding->unique_id=$item->id;
              if (isset($item->picture_url)){
                  $finding->picture_url=$item->picture_url;
               }else{
                  $finding->picture_url='https://firebasestorage.googleapis.com/v0/b/insectram-9e00b.appspot.com/o/findings%2F'.$gelen->workOrderId.'%2F'.$item->id.'.jpg?alt=media';
                
              }
	                   $finding->note=$item->explanation;
      
	        
	          $finding->saver_id=$user->id;
	          $finding->created_time=time();
            if ($finding->save()){
              
            }else{
              print_r($finding->getErrors());
            }
          }else{
           // echo 'var';
          }
        ...
        foreach ($gelen1 as $gelen){
          ...
          if (!$varmiencrypt){
           ...
           // echo 'var';
           						...
           						$this->result('','ok');
	//$varmiencrypt=Findings::model()->find(array('condition'=>'workorder_id="'.md5($str).'" and userid!=0'));
...
public function actionAddworkorders()
{
	...
	if(!isset($_REQUEST['deviceid'])){
		$_REQUEST['deviceid']='0';
        if (isset($_SERVER["HTTP_DEVICEID"])) {
          $_REQUEST['deviceid']=$_SERVER["HTTP_DEVICEID"];
        }
	}
   if ($_REQUEST['deviceid']=='0' && isset($_SERVER["HTTP_DEVICEID"])) {
          $_REQUEST['deviceid']=$_SERVER["HTTP_DEVICEID"];
        }
     if ( isset($_SERVER["HTTP_AUTHORIZATION"])) {
          $_REQUEST['deviceid']=$_REQUEST['deviceid'].'-'.$_SERVER["HTTP_AUTHORIZATION"];
        }
	...
	if(!$varmiencrypt)
	{
	  ...
	  $filenamex='apiv3-logs/workorder-'.time().'-'.rand(0,100000).'-wcache.txt';
	...
	$Superlogs->deviceid=$_REQUEST['deviceid'].'-ip='.$ip;
	$Superlogs->createdtime=time();
	$Superlogs->request=json_encode($_SERVER).'!!!!'.json_encode($_REQUEST);
	$Superlogs->save();
   ...
   // $workorders=$gelen->Workorders;
    ...
    { // WorkOrders
        ...
        {	// WorkOrderMonitors
			...
			if($workorders[$i]->workordermonitors[$j]->monitorStatus<>0 && $workorders[$i]->workordermonitors[$j]->monitorStatus<4)
			{
		...
		//"new_barcode_value": "null",
            ...
            if($workorders[$i]->workordermonitors[$j]->new_barcode_value!="null") // Monitor Barcode Numarası Değiştir
            {
              ...
              $mobileworkordermonitorsbarcodeupdate=Mobileworkordermonitors::model()->findAll(array('condition'=>'monitorid='.$modelM->monitorid.' and checkdate=0'));
              foreach( $mobileworkordermonitorsbarcodeupdate as $brcdchng){
                $brcdchng->barcodeno=$workorders[$i]->workordermonitors[$j]->new_barcode_value;
                $brcdchng->save();
              }
...
/*
				...
				if(strlen($workorders[$i]->realendtime)>9 )
	{
			...
			if ( ($modelW->visittypeid==62 || $modelW->visittypeid==26 || $modelW->visittypeid==31 ) && $servisRaporVarMi){
							$modelW->status=3;
				$modelW->executiondate=time();
			}else{
...
public function actionAddservicereports()
{
	$sure_baslangici = microtime(true);
		 $rand1=rand(0,1000000);
			 $rand2=rand(1000000,2000000);
  ...
  if (preg_match('/[çÇğĞıİöÖşŞüÜ]/u', $str)) {
...
else
{
     ...
     $str= preg_replace('/[^(\x20-\x7F)\x0A\x0D]*/','', $str);
	  ...
	  $filenamex='apiv3-logs/sservice-'.time().'-'.rand(0,100000).'-'.$rand4.'-cache.txt';
    file_put_contents($filenamex, $str);
  ...
  $workorders =$gelen;
	...
	//	if($gelen->signer_name <> "null" && $gelen->riskreview != "null")
        ...
        foreach ($servicereports as $servicereport){
				...
				$workorderbilgi=Workorder::model()->find('id=:ID',array(':ID'=>$gelen->workOrderId));
				if ($workorderbilgi){
						
				$clientbilgi=Client::model()->find('id=:ID',array(':ID'=>$workorderbilgi->clientid));
					if ($clientbilgi){
							$modelS->simple_client=$clientbilgi->simple_client;
					}
				...
				$modelS->client_sign='/uploads/'.$firm->username.'/'.$imagename2;
				  ...
				  if (isset($gelen->envRiskAssessment)){
				$modelS->security_precautions=json_encode($gelen->envRiskAssessment,true);
					}
				   if (isset($gelen->visit_report_summary)){
...
else
{
     ...
     $str= preg_replace('/[^(\x20-\x7F)\x0A\x0D]*/','', $str);
	  ...
	  $filenamex='apiv3-logs/service-'.time().'-'.rand(0,100000).'-'.$rand4.'-cache.txt';
    file_put_contents($filenamex, $str);
  ...
  //  file_put_contents('apiv3-logs/'.$user->id.' - '.time().'.txt', $str);
    ...
    $gelen=json_decode($str);
      ...
      if (isset($_GET['test'])){
          if ($gelen->workOrderId){
    
  }else{
  echo  $gelen[0]->workOrderId;
  }
      
      echo 'girdimx'.$gelen->workOrderId;
      }
