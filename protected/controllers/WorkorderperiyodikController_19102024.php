  ...
  public function actionCreate()
	...
	*/
	...
	public function actionAddsave()
	{
		...
		{
			$where.=' and departmentid='.$_POST['Workorder']['departmentid'];
		}
		if($_POST['Workorder']['subdepartmentid']!==null && $_POST['Workorder']['subdepartmentid']!=='' && $_POST['Workorder']['subdepartmentid']!==0)
		{
			$where.=' and subdepartmentid='.$_POST['Workorder']['subdepartmentid'];
		}
...
Yii::app()->db->createCommand('DELETE FROM mobileworkordermonitors
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Workorder']))
		{
			if($model->isperiod==1)  //// periyotlu olan workorderda sadece bu workorderda işlem yapılacağı için periyot kodu siliniyor
			{
...
// */
  	...
  	public function actionDeleteperiyodik($id)
	{
		...
		{
			$wdeleteAll=Workorder::model()->findAll(array('condition'=>'code="'.$model->code.'" and date>="'.$model->date.'" and id not in (select reportno from servicereport )'));
		...
		{
			$wdeleteAll=Workorder::model()->findAll(array('condition'=>'id='.$id.' and id not in (select reportno from servicereport )'));
			foreach($wdeleteAll as $wdeletex)
			{
                  $wdelete=Workorder::model()->deleteAll(array('condition'=>'id='.$wdeletex->id));
				  Workorder::model()->monitorupdate($wdeletex->id);
