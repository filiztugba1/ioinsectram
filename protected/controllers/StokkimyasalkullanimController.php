...
class StokkimyasalkullanimController extends Controller
{
	 ...
	 */
	 public function actionMonitorpets()
	 {
		...
		foreach($pets as $pet)
		{
			?>	<option value="<?=$pet->petsid;?>"><?=t(Pets::model()->findByPk($pet->petsid)->name);?></option><?
		}
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Stokkimyasalkullanim']))
		{
			...
			$model->ruhsattarihbaslik=$_POST['Stokkimyasalkullanim']['ruhsattarihbaslik'];
			$model->ruhsatgecerliliktarihi=$_POST['Stokkimyasalkullanim']['ruhsatgecerliliktarihi'];
			$model->urunAmbajMiktari=$_POST['Stokkimyasalkullanim']['urunAmbajMiktari'];
			$model->urunAmbajBirimi=$_POST['Stokkimyasalkullanim']['urunAmbajBirimi'];
			$model->urunAntidotu=$_POST['Stokkimyasalkullanim']['urunAntidotu'];
			$model->isactive=1;
					...
					<div class="text-center">
					 ...
					 <?}else{?> <a class="btn btn-danger btn-sm" style='float:right;color:#fff'><?=t('Passive');?> </a><?}?>
						<div class="card-body table-responsive">
							...
							<table style="width: 100%;font-size: 13px;" id="example2" class="table table-hover table-striped table-bordered tablebg ">
									<thead>
									<tr>
											<th>Zararlı Adı</th>
											<th>Dozaj</th>
									...
									<tbody id='tableBodyx'>
										<?
										$hedefzararli=Stokkimyasalhedeflenenzararli::model()->findAll(array("condition"=>"stokkimyasalkullanimid=".$stokkimyasalx->id));
										foreach ($hedefzararli as $hedefzararlix) {?>
											<tr>
												...
												<td><a class="btn btn-danger btn-sm" onclick="openmodalsil(this)"
					data-id=""
					data-userid=""><i style="color:#fff;" class="fa fa-trash"></i></a></td>
											</tr>
										<?}?>
	...
	public function actionPagination()
	{
		$ax= User::model()->userobjecty('');
			...
			<?for($i=1;$i<=ceil($total/$_POST['kolon']);$i++){?>
	...
	public function actionUpdate()
	{
		...
		if(isset($_POST['Stokkimyasalkullanim']))
		{
			...
			$model->urunAmbajMiktari=floatval($_POST['Stokkimyasalkullanim']['urunAmbajMiktari']);
			$model->urunAmbajBirimi=intval($_POST['Stokkimyasalkullanim']['urunAmbajBirimi']);
			$model->urunAntidotu=$_POST['Stokkimyasalkullanim']['urunAntidotu'];
			$model->isactive=intval($_POST['Stokkimyasalkullanim']['isactive']);
			if($_POST['Stokkimyasalkullanim']['firmid'])
			{
				$model->firmid=$_POST['Stokkimyasalkullanim']['firmid'];
			}
