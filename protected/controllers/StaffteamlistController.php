	...
	public function actionUpdate($id)
	{
		...
		{	$userstaff=User::model()->findByPk($_POST['Staffteamlist']['userid']);
			...
			if($usertaffinfo)
			{
				...
				$usertaffinfo->birthplace=isset($_POST['Staffteamlist']['birthplace'])?$_POST['Staffteamlist']['birthplace']:$usertaffinfo->birthplace;
			$usertaffinfo->birthdate=isset($_POST['Staffteamlist']['birthdate'])?$_POST['Staffteamlist']['birthdate']:$usertaffinfo->birthdate;
			$usertaffinfo->gender=isset($_POST['Staffteamlist']['gender'])?$_POST['Staffteamlist']['gender']:$usertaffinfo->gender;
			$usertaffinfo->primaryphone=isset($_POST['Staffteamlist']['phone'])?$_POST['Staffteamlist']['phone']:$usertaffinfo->primaryphone;
			$usertaffinfo->save();
			}
			...
			if($_POST['type']=='branch')
					...
					else
					{
						...
						$auth2=AuthAssignment::model()->find(array('condition'=>'userid='.$_POST['Staffteamlist']['userid'].' and (itemname="'.$authuser[0].'.'.$authuser[1].'.'.$authuser[2].'.'.$authuser[3].'.'.'Staff"
						or itemname="'.$authuser[0].'.'.$authuser[1].'.'.$authuser[2].'.'.$authuser[3].'.'.'Admin")'));
						
						$authuser2=explode('.',$auth2->itemname);
						// echo 'userid='.$_POST['Staffteamlist']['userid'].' and (itemname="'.$authuser2[0].'.'.$authuser2[1].'.'.$authuser2[2].'.'.$authuser2[3].'.'.'Staff"
						// or itemname="'.$authuser2[0].'.'.$authuser2[1].'.'.$authuser2[2].'.'.$authuser2[3].'.'.'Admin")';
						// exit;
						
						$auth2->itemname=$authuser2[0].'.'.$authuser2[1].'.'.$authuser2[2].'.'.$authuser2[3].'.'.$_POST['authtype'];
     	...
     	$serviceemail=Generalsettings::model()->find(array(
								   'condition'=>'name=:name and userid=:userid','params'=>array('name'=>'serviceemail','userid'=>$userstaff->id)
							   ));
			if(isset($serviceemail))
			{
				$serviceemail->type=$_POST['Conformity']['isservicemail'];
				$serviceemail->isactive=$_POST['Conformity']['isservicemail'];
			}
			else
			{

				$serviceemail=new Generalsettings;
				$serviceemail->type=$_POST['Conformity']['isservicemail'];
				$serviceemail->isactive=$_POST['Conformity']['isservicemail'];
				$serviceemail->name="serviceemail";
				$serviceemail->userid=$userstaff->id;
			}
$serviceemail->save();
...
/*
			...
			$usertaffinfo->birthdate=isset($_POST['Staffteamlist']['birthdate'])?$_POST['Staffteamlist']['birthdate']:$usertaffinfo->birthdate;
			$usertaffinfo->gender=isset($_POST['Staffteamlist']['gender'])?$_POST['Staffteamlist']['gender']:$usertaffinfo->gender;
			$usertaffinfo->primaryphone=isset($_POST['Staffteamlist']['primaryphone'])?$_POST['Staffteamlist']['primaryphone']:$usertaffinfo->primaryphone;
			$usertaffinfo->save();
...
*/
	...
	public function actionDelete($id)
	{
		...
		if($id==0)
		{
			$staff=User::model()->find(array(
								   'condition'=>'id=:id','params'=>array('id'=>$_POST['Staffteamlist']['userid'])
							   ));
		...
		Departmentpermission::model()->deleteAll(array('condition'=>'userid='.$id));  // kullanýcý silinince yetkisini de silme

		User::model()->deleteAll(array('condition'=>'id='.$id));
		Userinfo::model()->deleteAll(array('condition'=>'userid='.$id));
