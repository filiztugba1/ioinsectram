...
class DocumentsController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','tabs','fileandtable','create2','create3','documentlist','documentmenu','subview','subfile','documentlist2','update3',"documandetay",'documentstatusupdate'),
				'users'=>array('*'),
			),
	...
	{
		// if(isset($_POST['Documents']['filetype']) && intval($_POST['Documents']['filetype'])==1)
		// {
				// $ax= User::model()->userobjecty('');
				// Documentviewfirm::model()->DeleteAll(array('condition'=>'documentid=:id and clientbranchid=:cbid','params'=>array('id'=>$_POST['Documents']['id'],'cbid'=>$_POST['Documents']['cbid'])));
				// Userdocumentview::model()->DeleteAll(array('condition'=>'documentid=:id and userid=:uid','params'=>array('id'=>$_POST['Documents']['id'],'uid'=>$ax->id)));
				// $x='successful';
					// Logs::model()->logsaction();
				// echo json_encode($x);
		// }
			...
			<div class="card-content collapse show">
                 <div class="card-body card-dashboard">
					  ...
					  <?
						if(isset($_GET['cid']))
						{
							Documents::model()->kategoritabloyaz($_GET['id'],0,$_GET['cid']);
	}
	else {
	Documents::model()->kategoritabloyaz($_GET['id'],0);
	...
	{
			Yii::app()->db->createCommand('UPDATE documents SET arsivorguncel=0 WHERE createdtime<'.strTotime(date('Y').'-01-01 00:00:01'))->execute();
			echo 'ok';
			exit;
	}
	...
	public function actionDocumentlist()
	{
		...
		$ax= User::model()->userobjecty('');
		$documentscategory=Documentcategory::model()->find(array('condition'=>'id=:id','params'=>array('id'=>$_GET['id'])));		
		?>
		<div class="card">
        <div class="card-header">
            <div class="row">
				<div class="col-xl-9 col-lg-9 col-md-9 mb-1">
					<h4 id='documentlisth4' class="card-title"><?=t($documentscategory->name);?></h4>
				</div>
				<?php if (Yii::app()->user->checkAccess('documents.create')){ ?>
            ...
            <div class="card-body">
				<table class="table table-striped table-bordered dataex-html5-export table-responsive">
                    ...
                    <tbody>
					 ...
					 $document= Yii::app()->db->createCommand('SELECT documents.arsivorguncel as arsivorguncel,documents.arsiv_date,
						 documents.id as id,
						 ...
						 where documents.categoryid='.$_GET['id'].'
						 and documentviewfirm.firmid='.$fid.'
						 and documentviewfirm.branchid='.$bid.'
						 and documentviewfirm.clientid='.$cid.'
						 and documentviewfirm.clientbranchid='.$cbid.$where.' GROUP BY documentviewfirm.documentid')->queryAll();
					 ...
					 <tr>
						...
						<td><?=t(Documents::model()->subdocument($document[$i]['id']));?></td>
						<td><?=date('Y-m-d',$document[$i]['createdtime']);?></td> 
						
						<? if(isset($_GET['status']) && intval($_GET['status'])==0){?>
							<td><?=intval($document[$i]['arsiv_date'])!=0 && intval($document[$i]['arsivorguncel'])==0?date('Y-m-d',$document[$i]['arsiv_date']):'';?></td> 
	...
	if (isset($_POST['Documents']))
	{
		...
		if (isset($photos) && count($photos) > 0)
		{
			foreach ($photos as $image => $pic)
			{
				...
				{
					if ($pic->saveAs(Yii::getPathOfAlias('webroot').'/uploads/'.$firm->username.'/'.$type[0].'-'.date('Y-m-d H:i:s').'.'.$type[count($type)-1]))
					{
						$img_add = new Documents();
						$img_add->fileurl='uploads/'.$firm->username.'/'.$type[0].'-'.date('Y-m-d H:i:s').'.'.$type[count($type)-1];
						$img_add->categoryid=$_POST['Documents']['categoryid'];
            ...
            if (is_countable($_POST['Documents']['viewer'])){
              for($i=0;$i<count($_POST['Documents']['viewer']);$i++)
						...
						{
							// echo 'sdf';
							if(intval($_POST['Documents']['filetype'])===1)
							{
								$ax->firmid=$_POST['Documents']['fid'];
								$ax->branchid=$_POST['Documents']['fbid'];
								$ax->clientid=$_POST['Documents']['cid'];
								$ax->clientbranchid=$_POST['Documents']['cbid'];
							}
...
public function actionUpdate3()
{
	...
	if(isset($_POST['Documents']['filetype']) && intval($_POST['Documents']['filetype'])===1)
	{
			...
			if(intval($_POST['Documents']['arsivorguncel'])===0)
			{
				$img_add->arsiv_date=time();
			}
			else
			{
				$img_add->arsiv_date=null;
			}
			
			$img_add->save();
			$array[]='0,1,'.$img_add->name;
			echo json_encode($array);
			exit;
	}
	$ax= User::model()->userobjecty('');
