...
foreach($document as $documentx)
{
  ...
  if(date("Y-m-d",strtotime('-1 month',$documentx->finishdate))==date("Y-m-d"))
  {
    ...
    foreach($usermail as $usermailx)
      ...
      $subject=$documentx->name.' '.User::model()->dilbul($usermailx->languageid,'adlı dosyanızın son geçerlilik süresine 1 ay kaldı');
			$altbody=$documentx->name.' '.User::model()->dilbul($usermailx->languageid,'adlı dosyanızın son geçerlilik süresine 1 ay kaldı');
			$msg=$subject;
    ...
    if(date("Y-m-d",strtotime('-1 week',$documentx->finishdate))==date("Y-m-d"))
  {
    $where='';
    if($documentx->firmid>0)
    {
    ...
    foreach($usermail as $usermailx)
		{
			...
			$sendername=$firm->name;
			// $subject=count($countmail).' '.User::model()->dilbul($userx->languageid,'pieces of non-conformity opened');
      $subject=$documentx->name.' '.User::model()->dilbul($usermailx->languageid,'adlı dosyanızın son geçerlilik süresine 1 hafta kaldı');
			$altbody=$documentx->name.' '.User::model()->dilbul($usermailx->languageid,'adlı dosyanızın son geçerlilik süresine 1 hafta kaldı');
			$msg=$subject;
      ...
      if(date("Y-m-d",strtotime('-1 day',$documentx->finishdate))==date("Y-m-d"))
  {
    $where='';
    if($documentx->firmid>0)
    {
    ...
    foreach($usermail as $usermailx)
		{
			...
			$sendername=$firm->name;
			// $subject=count($countmail).' '.User::model()->dilbul($userx->languageid,'pieces of non-conformity opened');
      $subject=$documentx->name.' '.User::model()->dilbul($usermailx->languageid,'adlı dosyanızın son geçerlilik süresine 1 gün kaldı');
			$altbody=$documentx->name.' '.User::model()->dilbul($usermailx->languageid,'adlı dosyanızın son geçerlilik süresine 1 gün kaldı');
			$msg=$subject;
...
Yii::app()->db->createCommand("
    ...
    SET arsivorguncel = 0,
	arsiv_date=".time()."
    WHERE finishdate is not null and finishdate!='' and FROM_UNIXTIME(finishdate, '%Y-%m-%d')<= CURDATE() and finishdate!=0;
")->execute();
			exit;
?>
