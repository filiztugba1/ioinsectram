						...
						<div class="card" style="border: solid 1px #e3ebf3;border-radius: 5px;">
						 ...
						 <?}else{?> <a class="btn btn-danger btn-sm" style='float:right;color:#fff'><?=t('Passive');?> </a><?}?>
							...
							<div class="card-body">
							  ...
							  <img src="<?if($user[$i]['image']!=''){echo Yii::app()->baseUrl.'/'.$user[$i]['image'];}else {if($user[$i]['gender']==0){ echo Yii::app()->theme->baseUrl.'/app-assets/images/staff-logo-mr.png';}else{echo Yii::app()->theme->baseUrl.'/app-assets/images/staff-logo-mrs.png';?><?}}?>" class="rounded-circle  height-150" alt="<?=$user[$i]['name'].' '.$user[$i]['surname'];?>">
							</div>
							<div class="card-body">
             			...
             			<?php foreach($user as $users):?>
                                <tr>
									...
									<div class="form-group pb-1">
										<input type="checkbox" data-size="sm" id="switchery"  class="switchery" data-id="<?=$users->id;?>"  <?if($users->active==1){echo "checked";}?> <?php if (Yii::app()->user->checkAccess('user.list.active')==0){?>disabled<?}?> />
									</div>								
								</td>
