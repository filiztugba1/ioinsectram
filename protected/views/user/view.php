...
// İmza yükleme işlemi
if(isset($_POST['signature_data']) && isset($_POST['user_id'])) {
    $userId = (int)$_POST['user_id'];
    $signatureData = $_POST['signature_data'];
    
    // Kullanıcının signature_photo alanını base64 veri ile güncelle
    $user = User::model()->findByPk($userId);
    if($user) {
        $user->signature_photo = $signatureData;
        if($user->save()) {
            echo 'success';
        } else {
            echo 'error';
...
$ax= User::model()->userobjecty('');
	...
	<?php if (Yii::app()->user->checkAccess('staff.detail.view') || $ax->id==$_GET['id']){
...
<div class="mt-2">
  <h5>İmza</h5>
  <?php
  // Mevcut imzayı göster (eğer varsa)
  if(!empty($users->signature_photo)) {
      echo '<img src="data:image/jpeg;base64,'.$users->signature_photo.'" class="img-fluid mb-2" style="max-width:256px;" alt="Kullanıcı İmzası">';
  }
  ?>
  <form id="signature-upload-form" enctype="multipart/form-data" class="mt-1">
    <input type="file" id="signature-file" name="signature" accept="image/jpeg" class="form-control mb-1">
    <button type="button" id="upload-signature-btn" class="btn btn-primary btn-sm">İmza Yükle</button>
  </form>
</div>

<script>
$(document).ready(function() {
  $('#upload-signature-btn').click(function() {
    var fileInput = document.getElementById('signature-file');
    if(fileInput.files.length === 0) {
      alert('Lütfen bir imza dosyası seçin');
      return;
    }
    
    var file = fileInput.files[0];
    if(file.type !== 'image/jpeg') {
      alert('Lütfen sadece JPEG formatında imza yükleyin');
      return;
    }
    ...
    reader.onload = function(e) {
      var img = new Image();
      img.onload = function() {
        // Gerekirse resmi yeniden boyutlandır
        var canvas = document.createElement('canvas');
        var ctx = canvas.getContext('2d');
        var width = img.width;
        var height = img.height;
        
        // Yeni boyutları hesapla, maksimum genişlik 256px
        if(width > 256) {
          var ratio = width / 256;
          width = 256;
          height = height / ratio;
        ...
        $.ajax({
          url: '<?php echo Yii::app()->createUrl("/user/view", array("id" => $users->id)); ?>',
          type: 'POST',
          data: {
            'signature_data': base64Data,
            'user_id': <?php echo $users->id; ?>
          },
          success: function(response) {
            if(response === 'success') {
              alert('İmza başarıyla yüklendi');
              location.reload();
            } else {
              alert('İmza yüklenirken bir hata oluştu');
            }
          },
          error: function() {
            alert('İmza yüklenirken bir hata oluştu');
                  ...
                  <div class="card-body">
                    ...
                    <div class="tab-content px-1 pt-1">
					  ...
					  <div class="tab-pane" id="tabIcon23" aria-labelledby="baseIcon-tab23">
						...
						<?php if (Yii::app()->user->checkAccess('staff.update') || $ax->id==$_GET['id']){ ?>
						...
						<?php if (Yii::app()->user->checkAccess('staff.detail.update') || $ax->id==$_GET['id']){ ?>
