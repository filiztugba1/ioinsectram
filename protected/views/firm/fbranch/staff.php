					...
					<div class="card-header" style="">
						<ul class="nav nav-tabs">
							...
							<?php if (Yii::app()->user->checkAccess('firm.staff.view')) { ?>
								<li class="nav-item">
									<a class="nav-link active" href="<?= Yii::app()->baseUrl; ?>/firm/staff?type=<?= $type; ?>&&id=<?= $_GET['id']; ?>"><span
							...
							<?php if ($type == "branch" && $ax->branchid == 0) { ?>
								<li class="nav-item">
									<a class="nav-link" href="<?= Yii::app()->baseUrl; ?>/firm/staffteam?type=<?= $type; ?>&&id=<?= $_GET['id']; ?>"><span
											class="btn-effect2"
		...
		<div class="row" id="createpage">
			<div class="col-xl-12 col-lg-12 col-md-12">
				<div class="card">
					...
					<form id="staffcreate-form" action="<?= Yii::app()->baseUrl; ?>/firm/staffcreate" method="post" autocomplete="new-password">
						<div class="card-content">
							<div class="card-body">
								<input type="hidden" class="form-control" id="basicInput1" name="Firm[firmid]"
									value="<?= $_GET['id']; ?>">
								<input type="hidden" class="form-control" id="basicInput2" name="Firm[ftype]"
									value="<?= $type; ?>">
									...
									<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
										...
										<fieldset class="form-group">
											<input type="text" class="form-control" id="username" onkeyup="javascript:kontrol()"
												placeholder="<?= t('User Name'); ?>" name="Firm[username]" required>
										</fieldset>
									</div>
									...
									<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
										...
										<fieldset class="form-group">
											<input type="password" class="form-control" id="password"
												placeholder="<?= t('Password'); ?>" name="Firm[password]"
									...
									<div class="col-xl-6 col-lg-6 col-md-6 mb-1">
										...
										<fieldset class="form-group">
											<input type="text" class="form-control" id="basicInput3"
												placeholder="<?= t('Name'); ?>" name="Firm[name]">
										</fieldset>
									</div>
									...
									<div class="col-xl-6 col-lg-6 col-md-6 mb-1">
										...
										<fieldset class="form-group">
											<input type="text" class="form-control" id="basicInput4"
												placeholder="<?= t('Surname'); ?>" name="Firm[surname]">
										</fieldset>
									</div>
					...
					-->
									...
									<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
										...
										<fieldset class="form-group">
											<input type="text" class="form-control" id="basicInput5"
												placeholder="<?= t('Birth Place'); ?>" name="Firm[birthplace]">
										</fieldset>
									</div>
									...
									<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
										...
										<fieldset class="form-group">
											<input type="date" class="form-control" id="basicInput6"
												placeholder="<?= t('Birth Date'); ?>" name="Firm[birthdate]">
										</fieldset>
									</div>
											...
											<select class="custom-select block" id="userlgid" name="Firm[userlgid]" required>
												...
												<?php foreach ($languages as $language): ?>
													<option value="<?= $language->id; ?>"
														<?= (in_array(intval($firmx->country_id), [0, 1]) ? 1 : 2) == $language->id ? 'selected' : '' ?>><?= t($language->name); ?></option>
												<?php endforeach; ?>
											</select>
										</fieldset>
									</div>
									...
									<div class="col-xl-<?= $col; ?> col-lg-<?= $col; ?> col-md-<?= $col; ?> mb-1">
										...
										<fieldset class="form-group">
											<input type="text" class="form-control" id="basicInput7"
												placeholder="<?= t('Phone'); ?>" name="Firm[phone]">
										</fieldset>
									</div>
										...
										<fieldset class="form-group">
											<div class="input-group-append" id="button-addon2" style="float:right">
												<button class="btn btn-primary block-page-create"
													onclick="clicked(); return false;" type="submit"><?= t('Create'); ?></button>
											</div>
										</fieldset>
									</div>
			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						...
						<?php $isActive = isset($_GET['status']) ? intval($_GET['status']) : 1; ?>
					...
					<div class="card-content collapse show">
						<div class="card-body card-dashboard">
							...
							<input type="hidden" class="form-control form-control-xl input-xl" id="firmid"
								value="<?= $_GET['id']; ?>">
											...
											<div class="text-center">
												...
												<div class="card-body">
													<div class="card-title"
														style="background:#<?= $user[$i]['color']; ?>;height:5px"></div>
												...
												<div class="text-center" style="margin-bottom:10px">
													<?php if (Yii::app()->user->checkAccess('firm.staff.update') && ($upisActive || $user[$i]['id'] == $ax->id)) { ?>

														<a class="btn btn-warning btn-sm" onclick="openmodal(this)"
															data-id="<?= $user[$i]['userid']; ?>"
															data-username="<?= $user[$i]['username']; ?>"
															data-type="<?= $user[$i]['type']; ?>" data-name="<?= $user[$i]['name']; ?>"
														 ...
														 data-conformityemail="<?=$user[$i]['conformityemail'] ?>"><i
																style="color:#fff;" class="fa fa-edit"></i></a>

													<?php } ?>

													<?php if (Yii::app()->user->checkAccess('firm.staff.update') && ($upisActive || $user[$i]['id'] == $ax->id)) {
														if ($type == 'branch' && $firmx->id != 0) { ?>
															<a href="<?= Yii::app()->baseUrl ?>/firm/auth/<?= $user[$i]['id']; ?>"
													...
													<?php if (Yii::app()->user->checkAccess('firm.staff.delete') && $delisActive) { ?>
					...
					<div class="modal-dialog modal-lg" role="document">
						<div class="modal-content">
							<div class="modal-header bg-warning white">
								<h4 class="modal-title" id="myModalLabel8"><?= $availablefirm->name . t('Staff Update'); ?><span
										id="staff"></span></h4>
							...
							<form id="staffteamlist-form" action="<?= Yii::app()->baseUrl; ?>/firm/staffupdate/0" method="post"
								autocomplete="new-password">
								<div class="card-content">
									<div class="card-body">

										<input type="hidden" class="form-control" id="basicInput8" name="Firm[firmid]"
											value="<?= $_GET['id']; ?>">
										<input type="hidden" class="form-control" id="modalstaffuserid" name="Firm[userid]">
										<input type="hidden" class="form-control" id="basicInput9" name="Firm[ftype]"
											value="<?= $type; ?>">
										<input type="hidden" class="form-control" id="auth" name="authtype" value="<?= $type; ?>">

										<div class="row">

											<?php if (($type == 'branch' && $ax->type == 13) || ($type == 'firm' && $ax->type == 1) || ($type == 'branch' && $ax->type == 1)) { ?>
												<div class="col-xl-12 col-lg-12 col-md-12 mb-1">
													<label for="basicSelect"><?= t('Auth Type'); ?></label>
													<fieldset class="form-group">
											...
											<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
												...
												<fieldset class="form-group">
													<input type="text" class="form-control" id="modalstaffemail"
														placeholder="<?= t('Email/User Name'); ?>" name="Firm[email]" required>
												</fieldset>
											</div>
											...
											<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
												<label for="basicSelect"><?= t('LANGUAGE'); ?></label>
												<fieldset class="form-group">
													<select class="custom-select block" id="modaluserlgid" name="Firm[userlgid]"
														required>
														<?php foreach ($languages as $language): ?>
											...
											<?php if ($type == 'branch' || $availablefirm->package == 'Packagelite') { ?>
												<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
													<label for="basicSelect"><?= t('Color'); ?></label>
													<fieldset class="form-group">
														<input id='color2' onchange='colorchange()'
															class="form-control jscolor {position:'right', borderColor:'#FFF', insetColor:'#FFF', backgroundColor:'#666'}"
															name="Firm[color]" required>
													</fieldset>
												</div>
											...
											<? if ($type== 'branch') { ?>
												<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
													...
													<fieldset class="form-group">
														<select class="custom-select block" id="modalconformityemail"
															name="Firm[usertrasfer]">
															<?php 
															foreach ($firmtrasfer as $firmtrasferx) {
																?>
																<option value="<?= $firmtrasferx->id; ?>"
																	<? if ($firmtrasferx->id == $availablefirm->id) {
																		echo 'selected';
																	} ?>>
																	<?= $firmtrasferx->name; ?></option>
															<?php } ?>
														</select>
													</fieldset>
												</div>
				...
				<div class="modal fade text-left" id="sil" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
					...
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							...
							<!--form baslangıç-->
							<form id="staffteamlist-form" action="<?= Yii::app()->baseUrl; ?>/firm/staffdelete/0" method="post"
								autocomplete="new-password">

								<input type="hidden" class="form-control" id="basicInput10" name="Firm[firmid]"
									value="<?= $_GET['id']; ?>">
								<input type="hidden" class="form-control" id="modalstaffuserid2" name="Firm[userid]">
								<input type="hidden" class="form-control" id="basicInput11" name="Firm[ftype]"
									value="<?= $type; ?>">
...
<script>
	...
	function clicked() {
		...
		$.post("<?= Yii::app()->baseUrl ?>/staffteam/color?ara=" + color + '&&branch=' + <?= $_GET['id']; ?>).done(function (data) {
			//$('#staffteam').html(data);
			if (data == 1) {
...
<script>
	...
	$(function () {
		$('#staffsearch').keyup(function () {
			...
			$.post("<?= Yii::app()->baseUrl ?>/firm/staffsearch?id=" + firmid + '&&ara=' + staff + '&&type=<?= $_GET['type']; ?>').done(function (data) {
				$('#staffteam').html(data);
