...
<?= User::model()->geturl('Firm', 'Branch Raporu', $_GET['firmid'], 'reports'); ?>
<div class="card">
    <div class="card-header" style="">
        <ul class="nav nav-tabs">
            ...
            <?php if (Yii::app()->user->checkAccess('firm.staff.view')) { ?>
                <li class="nav-item">
                    <a class="nav-link" href="<?=Yii::app()->baseUrl;?>/firm/staff?type=<?= $type; ?>&&id=<?= $_GET['firmid']; ?>"><span
            ...
            <?php if ($type == "branch" && $ax->branchid == 0) { ?>
                <li class="nav-item">
                    <a class="nav-link" href="<?=Yii::app()->baseUrl;?>/firm/staffteam?type=<?= $type; ?>&&id=<?= $_GET['firmid']; ?>"><span
                            class="btn-effect2"
            ...
            <?php if ($type == "branch" && $ax->branchid == 0) { ?>
                <li class="nav-item">
                    <a class="nav-link " href="<?=Yii::app()->baseUrl;?>/firm/client?type=<?= $type; ?>&&id=<?= $_GET['firmid']; ?>"><span
                            class="btn-effect2"
            ...
            <li class="nav-item">
                <a class="nav-link active"
                    href="<?= Yii::app()->baseUrl ?>/firm/reports?id=<?= $_GET['firmid']; ?>&firmid=<?= $ax->branchid; ?>"><span
    ...
    <form id="reports-form" action="<?=Yii::app()->baseUrl;?>/client/reportcreate" method="post" target="_blank">
        <div class="card-content">
            <div class="card-body">
                ...
                <input type="hidden" class="form-control" id="basicInput" name="Report[firm]"
                    value="<?= $_GET['firmid']; ?>">
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-4 mb-1">
                        ...
                        <fieldset class="form-group">
                            ...
                            <select class="select2" style="width:100%" name="Report[type]" id="reptype" required>
                                <option value="9"><?= t('Fatura kontrol raporu - PDF') ?></option>
                                <option value="21"><?= t('Fatura kontrol raporu - EXCEL') ?></option>
                                <!-- <option value="20"><?= t('Müşteri fatura kontrol raporu') ?></option> -->
                            </select>
                        </fieldset>
                    </div>
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-4 mb-1">
                        ...
                        <fieldset class="form-group">
                            ...
                            <select class="select2" style="width:100%" name="Report[siralama]" required>
                                <option value="1"><?= t('Bitiş tarihine göre sırala') ?></option>
                                        ...
                                        foreach ($clientbranchs as $clientbranch) {
                                            if ($ax->clientbranchid != 0) {
                                                ...
                                                if ($return) {
                                                    ?>
                                                    <option 
                                                        value="<?= $clientbranch->id; ?>"><?= $client->name; ?> -> <?= $clientbranch->name; ?>
                                                    </option>
...
<script>
    ...
    function myFunction() {
        ...
        if ($("#typeselect").val() == 0) {
            $.post("<?=Yii::app()->baseUrl;?>/client/tumunugetir?id=" + <?= isset($_GET["id"]) ? $_GET["id"] : 0 ?>).done(function (data) {
                $('#pointno').html(data);
    ...
    function myType() {
        ...
        var client = <?= isset($_GET["id"]) ? $_GET["id"] : 0 ?>;
