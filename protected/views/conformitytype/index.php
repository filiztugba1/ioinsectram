...
$col=6;
    ...
    $status=Conformitytype::model()->findAll(array('condition'=>$where));


// type list sql finish


$type=  Conformitytype::model()->findAll();	?>

	<?php if (Yii::app()->user->checkAccess('nonconformitytype.view')){?>

	<?=User::model()->geturl('Conformity Type','',0,'conformitytype');?>


	<?php if (Yii::app()->user->checkAccess('nonconformitytype.create')){?>
		<!--
		<form id="conformitytype-form" action="/conformitytype/create" method="post">
			<div class="card col-xl-6 col-lg-6 col-md-12">
				<div class="card-header">
					<h4 class="card-title">Non-Conformity Type Add</h4>
				</div>
				<div class="card-content">
					<div class="card-body">
						...
						<fieldset>
							...
							<input maxlength="64" placeholder="Conformity status name" class="form-control" name="Conformitytype[name]"  type="text" />
							<input  type="hidden" name="Conformitytype[isactive]" value="1" />
							<div class="input-group-append" id="button-addon2">
									<button class="btn btn-primary block-page" type="submit">Create</button>
								</div>
							</div>
						</fieldset>
					</div>
...
<div id="createpage" >
	<div class="card col-xl-12 col-lg-12 col-md-12">
				<div class="card-header">
					<div class="row">
						<div class="col-md-6">
									  <h4  class="card-title"><?=t('Non-Conformity Type Add');?></h4>
						</div>
						<div class="col-md-6">
					...
					<div class="card-body">
						...
						<div class="col-xl-<?=$col;?> col-lg-<?=$col;?> col-md-<?=$col;?> mb-1">
							...
							<fieldset class="form-group">
								<select class="select2" style="width:100%" id="firm" name="Conformitytype[firmid]" onchange="myfirm()" requred>
						...
						<?php }else{?>
							<input type="hidden" class="form-control" id="firm" name="Conformitytype[firmid]" value="<?=$ax->firmid;?>" requred>
						<?php }?>
						...
						<label for="basicSelect"><?=t('Branch');?></label>
							<fieldset class="form-group">
								<select class="select2" style="width:100%" id="branch" name="Conformitytype[branchid]" onchange="mybranch()" disabled requred>
									<option value="0"><?=t('Please Chose');?></option>
						...
						<?php }else{?>
							<input type="hidden" class="form-control" id="branch" name="Conformitytype[branchid]" value="<?=$ax->branchid;?>" requred>
						<?php }?>
<? // client için açmak için aşağıdaki -1 i 0 yapacağız ?>
					<?php if($ax->clientid==-1){?>
					<div class="col-xl-<?=$col;?> col-lg-<?=$col;?> col-md-<?=$col;?> mb-1">
					<label for="basicSelect"><?=t('Client');?></label>
                        <fieldset class="form-group">

                          <select class="select2" style="width:100%" id="client" name="Conformitytype[clientid]" disabled requred >
					...
					<?php }else{?>
							<input type="hidden" class="form-control" id="client" name="Conformitytype[clientid]" value="<?=$ax->branchid;?>" requred>
					<?php }?>
					...
					<div class="col-xl-<?=$col;?> col-lg-<?=$col;?> col-md-<?=$col;?> mb-1">
						<label for="basicSelect"><?=t('Conformity status name');?></label>
							<fieldset class="form-group">
							<input placeholder="Conformity status name" class="form-control" name="Conformitytype[name]"  type="text" />
							</fieldset>
					</div>
              ...
              <div class="card">
                <div class="card-header">
					<div class="row" style="border-bottom: 1px solid #e3ebf3;">
					   ...
					   <h4 style class="card-title"><?=t('Non-Conformity Type List');?></h4>
						</div>
						<?php if (Yii::app()->user->checkAccess('nonconformitytype.create')){ ?>
                      ...
                      <table class="table table-striped table-bordered dataex-html5-export">
                        <thead>
						  ...
						  <th style='width:1px;'><input type="checkbox" name="select_all" value="1" id="select_all"></th>
							 ...
							 <?php }?>
							 <?php if (Yii::app()->user->checkAccess('nonconformitytype.update')){?>
							 <th><?=t('Active');?></th>
							 <?php }?>
							 <?php if (Yii::app()->user->checkAccess('nonconformitytype.update') || Yii::app()->user->checkAccess('nonconformitytype.update') ){?>
                            <th><?=t('PROCESS');?></th>
							 <?php }?>
                        ...
                        <tbody>
             			<?php foreach($type as $typex):?>
										...
										if($typex->firmid!=0 && ($ax->firmid==$typex->firmid || $ax->firmid==0))
										{
                                ...
                                <tr>
								<td>
										<?php if (Yii::app()->user->checkAccess('nonconformitytype.delete') &&$deleteUpdateActive){?>
									<input type="checkbox" name="Conformitytype[id][]" class='sec' value="<?=$typex->id;?>">
									<?php }?>
								</td>
                                    <td><?=t($typex->name);?></td>
									 ...
									 <?php if($ax->firmid==0){?>
										 <td>
										 <?php if($typex->firmid!=0){?>
											<?=Firm::model()->find(array('condition'=>'id='.$typex->firmid))->name;?>
										 <?php }?>
										 </td>
									 <?php }?>
									 <?php if($ax->branchid==0){?>
										 <td>
											<?php if($typex->branchid!=0){?>
											<?=Firm::model()->find(array('condition'=>'id='.$typex->branchid))->name;?>
											<?php }?>
										 </td>
									 <?php }?>

									<?php if($typex->clientid!=0){?>
										<?$client=Client::model()->find(array('condition'=>'id='.$typex->clientid));?>
									<?php }?>
							...
							<td>
								<?php if (Yii::app()->user->checkAccess('nonconformitytype.update') && $deleteUpdateActive){?>

									<div class="form-group pb-1">
										<input type="checkbox" id="switchery" data-size="sm"  class="switchery" data-id="<?=$typex->id;?>"  <?php if($typex->isactive==1){echo "checked";}?>/>
									</div>
							...
							<td>
									 <?php if (Yii::app()->user->checkAccess('nonconformitytype.update') && $deleteUpdateActive){?>
										...
										<?php if (Yii::app()->user->checkAccess('nonconformitytype.update') && $deleteUpdateActive){?>
											<a  class="btn btn-warning btn-sm" onclick="openmodal(this)" data-id="<?=$typex->id;?>" data-name="<?=$typex->name;?>" data-active="<?=$typex->isactive;?>"
                        ...
                        <tfoot>
                          <tr>
						  <th style='width:1px;'>
								<?php if (Yii::app()->user->checkAccess('nonconformitytype.delete')){?>
								<div class="input-group-append" id="button-addon2" style="float: right; text-align: right;">
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="duzenle" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
                ...
                <div class="modal-dialog" role="document">
					...
					<!--form baslang��-->
						<form id="conformitytype-form" action="/conformitytype/update/0" method="post">
                            <div class="modal-body">
								<input type="hidden" class="form-control" id="modaltypeid" name="Conformitytype[id]" value="0">
								<div class="col-xl-12 col-lg-12 col-md-12 mb-1">
								<label for="basicSelect"><?=t('Name');?></label>
									<fieldset class="form-group">
										<input type="text" class="form-control" id="modaltypename" placeholder="<?=t('Name');?>" name="Conformitytype[name]" value="">
									</fieldset>
								</div>
								...
								<label for="basicSelect"><?=t('Active');?></label>
									<fieldset class="form-group">
										 <select class="custom-select block" id="modaltypeactive" name="Conformitytype[isactive]">
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="sil" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
                ...
                <div class="modal-dialog" role="document">
					...
					<!--form baslang��-->
						<form id="conformitytype-form" action="/conformitytype/delete/0" method="post">

						<input type="hidden" class="form-control" id="modaltypeid2" name="Conformitytype[id]" value="0">
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="deleteall" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-danger white">
                            <h4 class="modal-title" id="myModalLabel8"><?=t('Conformity Type Delete');?></h4>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
                            </button>
                        </div>
				...
				<!--form baslangıç-->
						<form action="/conformitytype/deleteall" method="post">

						<input type="hidden" class="form-control" id="modalid3" name="Conformitytype[id]" value="0">
