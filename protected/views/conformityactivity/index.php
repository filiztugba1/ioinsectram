			...
			<div class="card">
							...
							/*	if( gmdate('m',$conformity['date'])==2)
								...
								*/
								...
								<?php if (Yii::app()->user->checkAccess('nonconformitymanagement.update')){ ?>
										 ...
										 data-filesf2="<?=$conformity->filesf2;?>"
										 ><?=t('Update');?></button>
								<?php }}?>
				...
				<div class="card-content">
					<div class="card-body">
							...
							<div class='col-md-3'>
								<div class='row'>
								<div class="col-md-6 img-cart" style="width:100%">
								<h5><?=t('Upload File');?></h5>

										<?php if(file_exists(Yii::getPathOfAlias('webroot').'/'.$conformity->filesf) && $conformity->filesf!=''){?>
										 <a href="<?=$conformity->filesf;?>" download> <img class="brand-logo" alt="stack admin logo" style="width: 100%;max-height: 130px;" src="<?=Yii::app()->baseUrl.$conformity->filesf;?>"> </a>
										<?php }
										else
										{?>
											<div style='text-align: center; background: #65799c; color: #fff; padding: 50px 0 50px 0;font-size: 21px;border-radius: 5px;'><?=t('No picture')?></div>
										<?php }?>
										<?php if($ax->clientid==0){?>
											<form action="/conformity/fileupdate/<?=$_GET['id'];?>" enctype="multipart/form-data"  method="post">
											<fieldset class="form-group mt-1">
											  <input type="file"  class="form-control"  name="Conformity[filesf]">
											  <input type="hidden"  class="form-control"  name="Conformity[filetype]" value='filesf'>
											  <input type="hidden"  class="form-control"  name="Conformity[id]" value='<?=$_GET['id'];?>'>
											    <input type="hidden" class="form-control" name="Conformity[firmid]" value="<?=$conformity->firmid;?>">
												<input type="hidden"  class="form-control"  name="Conformity[filesfx]" value='<?=$conformity->filesf;?>'>
											</fieldset>
											<fieldset class="form-group">
											  <button class="btn btn-warning block-page btn-sm" type="submit"><?=t('Update');?></button>
											  <?php if(file_exists(Yii::getPathOfAlias('webroot').'/'.$conformity->filesf) && $conformity->filesf!=''){?>
											  <a class="btn btn-danger btn-sm" style='color:#fff' data-id='<?=$_GET['id'];?>' data-file='filesf' onclick="conformityfilesil(this)"><?=t('Delete');?></a>
											  <?php }?>
											</fieldset>
											</form>
										<?php }?>
							...
							<div class="col-md-6 img-cart" style="width:100%"><h5><?=t('Completion File');?></h5>

										<?php if(file_exists(Yii::getPathOfAlias('webroot').'/'.$conformity->filesf2) && $conformity->filesf2!=''){?>
										 <a href="<?=$conformity->filesf2;?>" download> <img class="brand-logo" alt="stack admin logo" style="width: 100%;max-height: 130px;" src="<?=Yii::app()->baseUrl.$conformity->filesf2;?>"> </a>
										<?php }
										else
										{?>
											<div style='text-align: center; background: #65799c; color: #fff; padding: 50px 0 50px 0;font-size: 21px;border-radius: 5px;'><?=t('No picture')?></div>
										...
										<fieldset class="form-group mt-1">
											  <input type="file"  class="form-control"  name="Conformity[filesf2]">
											   <input type="hidden"  class="form-control"  name="Conformity[filetype]" value='filesf2'>
											  <input type="hidden"  class="form-control"  name="Conformity[id]" value='<?=$_GET['id'];?>'>
											  <input type="hidden" class="form-control" name="Conformity[firmid]" value="<?=$conformity->firmid;?>">
											  <input type="hidden"  class="form-control"  name="Conformity[filesfx2]" value='<?=$conformity->filesf2;?>'>
											</fieldset>
											<fieldset class="form-group">
											  <button class="btn btn-warning block-page btn-sm" type="submit"><?=t('Update');?></button>
											  <?php if(file_exists(Yii::getPathOfAlias('webroot').'/'.$conformity->filesf2) && $conformity->filesf2!=''){?>
											  <a class="btn btn-danger btn-sm" style='color:#fff' data-id='<?=$_GET['id'];?>' data-file='filesf2' onclick="conformityfilesil(this)"><?=t('Delete');?></a>
											   <?php }?>
											</fieldset>
									</form>
							</div>
								...
								<div class="row" >
									<div class="col-md-2" style="">
										<p style="color: #636161;">
												...
												<?$senderuser=User::model()->find(array('condition'=>'id='.$assisgnx->senderuserid));
									...
									<div class="col-md-2" style="">
										<p style="color: #636161;">
												<?$recipient=User::model()->find(array('condition'=>'id='.$assisgnx->recipientuserid));
					...
					<a style='margin: 2px;float:right;color:#fff' class="btn btn-success" onclick="assign(this,1,<?=$atananId?>)" data-id="<?=$_GET['id'];?>"
									data-datex="<?=date('Y-m-d',$conformity->closedtime);?>"
									><?=t('Atamayı Güncelle');?></a>
					<?php }}
					?>
					...
					<span data-id="<?=$_GET['id'];?>"
					data-datex="<?=date('Y-m-d',time());?>" onclick="modalok(this)" style='margin: 2px;color:#fff;float:right' class="btn btn-success" ><?=t('OK - Completed');?></span>
                  ...
                  <div class="card-body card-dashboard">
                      ...
                      <table  class="table table-striped table-bordered dataex-html5-export table-responsive">
                        ...
                        <tbody id='waypointsTable'>
							...
							<?$loglar=Loglar::model()->findAll(array('condition'=>"tablename='conformity' and data LIKE '%{\"id\":".'"'.$_GET['id']."\"%'"));?>
					 		<?php
							...
							<!-- <a  class="btn btn-success btn-sm" onclick="openmodal(this)"
							 ...
							 data-id="<?=$loglarx->id;?>"  data-conformityid="<?=$x->id;?>" ><i style="color:#fff;" class="fa fa-trash"></i></a>
									...
									<td><?=$x->date!==''?date('d:m:Y',$x->date):'';?></td>
	...
	<div class="col-lg-4 col-md-6 col-sm-12">
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="conformityduzenle" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
				...
				<input type="hidden"  class="form-control"  name="Conformity[filesfx2]" id="modalfilesf2">
...
<div class="col-lg-4 col-md-6 col-sm-12">
        <div class="form-group">
                           <!-- Modal -->
            <div class="modal fade text-left" id="filesil" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
                          aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
					...
					<!--form baslangıç-->
						<form id="user-form" action="/conformity/filedelete/0" method="post">

					 <input type="hidden" class="form-control" id="conid" name="Conformity[id]" value="0">
					 <input type="hidden"  class="form-control"  name="Conformity[filetype]" id="fileType">
                            <div class="modal-body">
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="conformitysil" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
                ...
                <div class="modal-dialog" role="document">
					...
					<input type="hidden"  class="form-control"  name="Conformity[filesfx2]" id="modalfile2">
                            <div class="modal-body">
				     ...
				     <div class="card-header">
						 <div class="row" style="padding-bottom: 10px;border-bottom: 1px solid #f8f8f9;">
							...
							<?php if($gerigonderim==0){?>
								  <a style='margin: 2px;float:right;color:#fff' class="btn btn-success" onclick="assign(this,0,0)" data-id="<?=$_GET['id'];?>"
									data-datex="<?=date('Y-m-d',$conformity->closedtime);?>"
									><?=t('Assign');?></a>
							<?php }?>
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="assingnuser" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
				...
				<form id="conformity-form2" action="/conformity/assigncreate" method="post" enctype="multipart/form-data">
					 ...
					 <input type="hidden"  class="form-control"  name="Conformity[id]" value="<?=$_GET['id'];?>">
					<input type="hidden" id='cu'  class="form-control"  name="Conformity[cu]">
					<input type="hidden" id='assignidx'  class="form-control"  name="Conformity[assignidx]">
 ...
 function modalclosed(obj)
{
 ...
 function assign(obj,createorupdate,id)
{
...
<?php }?>
	...
	if($(obj).data('filesf')!=='')
	{
		$('#img').html('<img src="'+$(obj).data('filesf')+'" style="    max-width: 100px;max-height: 100px;" />');	
	}
	
	$('#modalfilesf2').val($(obj).data('filesf2'));
	if($(obj).data('filesf2')!=='')
	{
		$('#img2').html('<img src="'+$(obj).data('filesf2')+'" style="    max-width: 100px;max-height: 100px;" />');	
	}
 ...
 function conformitymodalsil(obj)
{
	...
	$('#modalfile2').val($(obj).data('file2'));
	$('#conformitysil').modal('show');

}

 function conformityfilesil(obj)
{
	if($(obj).data('file')=='filesf2')
	{
		$('#filemodaltitle').html('<?=t('Tamamlama Fotoğrafı Silme')?>');
	}
	else{
		$('#filemodaltitle').html('<?=t('Başlangıç Fotoğrafı Silme')?>');
	}
	
	$('#conid').val($(obj).data('id'));
	$('#fileType').val($(obj).data('file'));
	$('#filesil').modal('show');
