<?php
/* @var $this TranslateslanguageController */
/* @var $model Translateslanguage */

$this->breadcrumbs=array(
	'Translateslanguages'=>array('index'),
	$model->name,
);

$this->menu=array(
	array('label'=>'List Translateslanguage', 'url'=>array('index')),
	array('label'=>'Create Translateslanguage', 'url'=>array('create')),
	array('label'=>'Update Translateslanguage', 'url'=>array('update', 'id'=>$model->id)),
	array('label'=>'Delete Translateslanguage', 'url'=>'#', 'linkOptions'=>array('submit'=>array('delete','id'=>$model->id),'confirm'=>'Are you sure you want to delete this item?')),
	array('label'=>'Manage Translateslanguage', 'url'=>array('admin')),
);
?>

<h1>View Translateslanguage #<?php echo $model->id; ?></h1>

<?php $this->widget('zii.widgets.CDetailView', array(
	'data'=>$model,
	'attributes'=>array(
		'id',
		'name',
	),
)); ?>
