...
{
	$workorder=Workorderti::model()->find(array('condition'=>'id='.$_GET['id']));
	$date=$workorder->date;
}
								...
								<fieldset class="form-group">
								  <select class="select2"  id="staff" onchange="staffinput()"  style="width:100%;" name="Workorder[staffid][]" <?php							if( $workorder->teamstaffid!=0 && ($workorder->staffid=='' || $workorder->staffid==0) ){echo 'disabled';}?>>
										...
										foreach($staff as $staffx){?>
										  <option value="<?=$staffx->id;?>" <?php if(isset($_GET['id'])){if(Workorderti::model()->isnumber($staffx->id,$workorder->staffid)){echo "selected";}}?>
					...
					<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
						<label for="basicSelect"><?=t('Visit Type');?></label>
							<fieldset class="form-group">
							  ...
							  <select class="select2" style="width:100%" id="visittype"  disabled="true" name="Workorder[visittypeid]" >
										<option disabled  value="" hidden><?=t('Please Chose');?></option>
										<?php										if($workorder->branchid!=0){
										...
										<option disabled <?php if($typex->id==$workorder->visittypeid){echo "selected";}?> value="<?=$typex->id;?>"><?=t($typex->name);?></option>
										<?php }}?>
...
if(isset($_GET['id'])){
		...
		$dvisit=Departmentvisitedti::model()->find(array('condition'=>'treatmenttypeid!="" and workorderid='.$_GET['id']));
...
<input type='hidden'  name="Workorder[mavailable]" value="1" /> 
								...
								<fieldset class="form-group">
								  <select class="select2-placeholder-multiple form-control" multiple="multiple" id="treatmenttype" style="width:100%;" name="Workorder[treatmenttypeid][]" <?php if(!$dvisit && !isset($_GET['id'])){echo 'disabled';}?>>
									...
									foreach($treatmenttypes as $treatmenttype){?>
										 ...
										 <?php if($dvisit->treatmenttypeid!=''){if(Workorderti::model()->isnumber($treatmenttype->id,$dvisit->treatmenttypeid)){echo "selected";}}?>
              ...
              */?>
                        ...
                        <div class="input-group-append" id="workorderupdate" style="float:right">
								...
								<button class="btn btn-primary block-page" id="kydt" type="submit"><?=t('Save');?></button>
								</div>
                        </fieldset>
                    </div>
...
if(isset($_GET['id'])){
	...
	if(isset($_GET['visitid']))
	{
		...
		$dvisit=Departmentvisitedti::model()->find(array(
...
<div class="row">
	<div class="col-xl-12 col-lg-12 col-md-12">
		...
		<form id="departmentvisited-form" action="<?php if(isset($_GET['visitid'])){echo '/workorder/visitedupdate?id='.$_GET['visitid'];}else{echo '/workorder/addsaveti';}?>" method="post">
				<div class="card-content">
					<div class="card-body">
						<div class="row">
								...
								<fieldset class="form-group">
								  <select class="select2-placeholder-multiple form-control" multiple="multiple" id="department" onchange="mydepartment()" style="width:100%;" name="Workorder[departmentid][]">
										...
										foreach($department as $departmentx){?>
										  <option value="<?=$departmentx->id;?>" <?php if(isset($_GET['visitid'])){if(Workorderti::model()->isnumber($departmentx->id,$dvisit->departmentid)){echo "selected";}}?>
							...
							<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
								...
								<fieldset class="form-group">
								  <select class="select2-placeholder-multiple form-control" multiple="multiple" id="subdepartment" onchange="mysubdepartment()" style="width:100%;" name="Workorder[subdepartmentid][]" disabled>
										...
										foreach($department as $departmentx){?>
										  <option value="<?=$departmentx->id;?>" <?php if(isset($_GET['visitid']) &&$dvisit->subdepartmentid!=''){if(Workorderti::model()->isnumber($departmentx->id,$dvisit->subdepartmentid)){echo "selected";}}?>
										  ><?=$departmentx->name;?></option>
										<?php }}?>
								 </select>
								</fieldset>
	 ...
	 if ($country<>0) {
						 $monitoringtype=Monitoringtype::model()->findAll(array(
								...
								<fieldset class="form-group">
								  <select class="select2-placeholder-multiple form-control" multiple="multiple" id="monitoringtype" onchange="mymonitoringtype()" style="width:100%;" name="Workorder[monitoringtype][]">
										<?php foreach($monitoringtype as $monitoringtypex){?>
											<option value="<?=$monitoringtypex->id;?>" <?php if(isset($_GET['visitid']) && $dvisit->monitoringtype!=''){if(Workorderti::model()->isnumber($monitoringtypex->id,$dvisit->monitoringtype)){echo "selected";}}?>
								...
								<fieldset class="form-group">
								  <select class="select2-placeholder-multiple form-control" multiple="multiple" id="multi_placehodler" style="width:100%;" name="Workorder[treatmenttypeid][]">
									...
									foreach($treatmenttypes as $treatmenttype){?>
										 ...
										 <?php if(isset($_GET['visitid']) && $dvisit->treatmenttypeid!=''){if(Workorderti::model()->isnumber($treatmenttype->id,$dvisit->treatmenttypeid)){echo "selected";}}?>
										  ><?=$treatmenttype->name;?></option>
										 <?php }?>
              ...
              <div class="card">
                ...
                <div class="card-content collapse show">
                  <div class="card-body card-dashboard" id="">
						...
						<?php						$visits=Departmentvisitedti::model()->findall(array(
							...
							{
							$departments=Departments::model()->findall(array('condition'=>Workorderti::model()->serarchsplit('id',$visit->departmentid)));
							}
							if($visit->subdepartmentid!='')
							{
							$subdepartments=Departments::model()->findall(array('condition'=>Workorderti::model()->serarchsplit('id',$visit->subdepartmentid)));
							}
							if($visit->monitoringtype!='')
							{
							$monitoringtypes=Monitoringtype::model()->findall(array('condition'=>Workorderti::model()->serarchsplit('id',$visit->monitoringtype)));
							}
							...
							if($workorder->teamstaffid!='' && $workorder->teamstaffid!=0)
							{
								...
								$user=User::model()->findall(array('condition'=>Workorderti::model()->serarchsplit('id',$team->staff)));
							}

							if($workorder->staffid!='' && $workorder->staffid>1)
							{

					$user=User::model()->findall(array('condition'=>Workorderti::model()->serarchsplit('id',$workorder->staffid)));
							}
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="sil" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
                ...
                <div class="modal-dialog" role="document">
					...
					<!--form baslangıç-->
						<form id="user-form" action="/workorder/visiteddeleteti/0" method="post">
            ...
            <div class="modal fade text-left" id="workordersil" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
                ...
                <div class="modal-dialog" role="document">
					...
					<!--form baslangıç-->
						...
						<form id="user-form" action="/workorder/deleteti/0" method="post">
