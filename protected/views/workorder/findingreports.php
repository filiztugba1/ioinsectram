...
<?php if (Yii::app()->user->checkAccess('nonconformitymanagement.view')){ ?>
	<!-- Sayfada neredeyiz -->
<?=User::model()->geturl('Finding Reports','',0,'workorder/findingreports');?>
	...
	<div class="card">
		<div class="card-header">
			<div class="row" style="padding-bottom: 10px;border-bottom: 1px solid #f8f8f9;">
				<div class="col-md-6">
					<h4  class="card-title"><?=t('Finding Reports');?></h4>
				</div>
				<div class="col-md-6">
									...
									foreach($client as $clientx)
										...
										if(is_countable($clientbranchs) && count($clientbranchs)>0){?>
											<optgroup label="<?=$clientx->name;?>">
												<?$clientbranchs=Client::model()->findAll(array('condition'=>'isdelete=0 and firmid='.$ax->branchid.' and parentid='.$clientx->id));
...
<div class="col-xl-6 col-lg-6 col-md-6 mb-1">
								...
								<fieldset class="form-group">
								  <select class="select2"  id="staff"  style="width:100%;" name="Workorder[staffid]" >
										<option  value="" hidden><?=t('Please Chose');?></option>
										<?php										
											$firm=Firm::model()->find(array('condition'=>'id='.$ax->firmid));
											if($ax->firmid==0)
											{
												$liteuser=User::model()->find(array('condition'=>'firmid!=0'));
											...
											{
												$liteuser=User::model()->find(array('condition'=>'branchid='.$ax->branchid.' and firmid='.$ax->firmid));
											}
...
tableList("confotmityListTable", "confotmityList", freeListColumnArray, null, "/workorder/findingreportlist", "POST", formData,"<?=t('Non-Conformity-Report')?> (<?=date('d-m-Y H:i:s');?>)","");
