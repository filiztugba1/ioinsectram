...
$ax= User::model()->userobjecty('');
	...
	if($where <> "")
	{
		$where= $where.' and medfirmid='.$model->id;
	}
	else{
		$where = 'medfirmid='.$model->id;
	}
$meds=Meds::model()->findAll(array('order'=>'name ASC','condition'=>$where));
//$visittypes=Visittype::model()->findAll(array('order'=>'name ASC'));?>


<?php if (Yii::app()->user->checkAccess('meds.view')){ ?>	
<!-- Sayfada neredeyiz -->
<?=User::model()->geturl('Meds','',0,'medfirms');?>
<?php if (Yii::app()->user->checkAccess('meds.create')){ ?>	
			...
			<div class="card">
				    <div class="card-header">
						 <div class="row" style="padding-bottom: 10px;border-bottom: 1px solid #f8f8f9;">
							 <div class="col-md-6">
								  <h4  class="card-title"><?=t('Medicines');?></h4>
									</div>
									 <div class="col-md-6">
				...
				<form id="meds-form" action="/meds/create" method="post">
				<input type="hidden" name="Meds[medfirmid]" value="<?=$model->id?>" > 
				<div class="card-content">
					...
					<div class="row">
						...
						<div class="col-xl-12 col-lg-12 col-md-12 mb-1">
							<label for="basicSelect"><?=t('Brand');?></label>
							<fieldset class="form-group">
							  <input type="text" class="form-control" id="basicInput" placeholder="<?=t('Brand');?>" name="Meds[brand]" requred>
							</fieldset>
						</div>
						...
						<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
							...
							<fieldset class="form-group">
								<select class="select2" style="width:100%" id="firm" name="Meds[firmid]" onchange="myfirm()" requred>
						...
						<?php }else{?>
							<input type="hidden" class="form-control" id="firm" name="Meds[firmid]" value="<?=$ax->firmid;?>" requred>
						<?php }?>
						...
						<label for="basicSelect"><?=t('Branch');?></label>
							<fieldset class="form-group">
								<select class="select2" style="width:100%" id="branch" name="Meds[branchid]" disabled requred>
									<option value="0"><?=t('Please Chose');?></option>
						...
						<?php }else{?>
							<input type="hidden" class="form-control" id="branch" name="Meds[branchid]" value="<?=$ax->branchid;?>" requred>
						<?php }?>
					...
					<label for="basicSelect"><?=t('Name');?></label>
                        <fieldset class="form-group">
                          <input type="text" class="form-control" id="basicInput" placeholder="<?=t('Name');?>" name="Meds[name]" requred>
                        </fieldset>
                    </div>

					<div class="col-xl-12 col-lg-12 col-md-12 mb-1">
                    <label for="basicSelect"><?=t('Unit');?></label>
						<fieldset class="form-group">
							<select class="form-control" id="modalMedunit" placeholder="<?=t('Unit');?>" name="Meds[unit]">
							<?php
							$birimler=Units::model()->findAll(array('condition'=>'active=1'));
					...
					<label for="basicSelect"><?=t('Is Active');?></label>
                       <fieldset class="form-group">
                          <select class="custom-select block" id="customSelect" name="Meds[isactive]">
					   ...
					   <div class="col-xl-9 col-lg-9 col-md-9 mb-1">
						...
						<div class="col-xl-3 col-lg-3 col-md-3 mb-1">
								<div class="input-group-append" id="button-addon2" style="float: right; text-align: right;">
									<button class="btn btn-info" id="createbutton" type="submit"><?=t('Add Medicine');?> <i class="fa fa-plus"></i></button>
								</div>
						</div>
						<?php }?>
					</div>
                        ...
                        <tbody>
             			<?php foreach($meds as $med):
							  $unit=Units::model()->findByPk($med->unit);
						  	  if($unit){ $ilac=$unit->name;}else{$ilac="-";}
						  ...
						  ?>
                                <tr>
									...
									<td><?=$ilac?></td>
								<?php if($ax->firmid==0){?>
								<td><?=Firm::model()->find(array('condition'=>'id='.$med->firmid))->name;?></td>
								<?php }?>
								
								<?php if($ax->branchid==0){?>
								<td><?=Firm::model()->find(array('condition'=>'id='.$med->branchid))->name;?></td>
								<?php }?>
								
								
								<td> 
									<div class="form-group pb-1">
										<input type="checkbox" id="switchery" data-size="sm"  class="switchery" data-id="<?=$med->id;?>"  <?php if($med->isactive==1){echo "checked";}?>  <?php if (Yii::app()->user->checkAccess('meds.update')==0){?>disabled<?php }?> />
									</div>								
								</td>
									
									<td>
									<?php if (Yii::app()->user->checkAccess('meds.update')){ ?>		
										<a  class="btn btn-warning btn-sm" onclick="openmodal(this)" data-id="<?=$med->id;?>" data-brand="<?=$med->brand?>" data-name="<?=$med->name;?>" data-unit="<?=$med->unit;?>" data-active="<?=$med->isactive;?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?=t('Update');?>"><i style="color:#fff;" class="fa fa-edit"></i></a>
									<?php }?>
									<?php if (Yii::app()->user->checkAccess('meds.delete')){ ?>		
									<a class="btn btn-danger btn-sm" onclick="openmodalsil(this)" data-id="<?=$med->id;?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?=t('Delete');?>"><i style="color:#fff;" class="fa fa-trash"></i></a>
									<?php }?>
									</td>
                                </tr>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-warning white">
                            <h4 class="modal-title" id="myModalLabel8"><?=t('Medfirm Update');?></h4>
					...
					<!--form baslangıç-->						
						<form id="meds-form" action="/meds/update/0" method="post">	
                            <div class="modal-body">
								<input type="hidden" class="form-control" id="modaltypeid" name="Meds[id]" value="0">
								<input type="hidden" class="form-control" id="basicInput" placeholder="<?=t('Name');?>" name="Meds[firmid]" value="<?=$ax->firmid;?>">
								<div class="col-xl-12 col-lg-12 col-md-12 mb-1">
								<label for="basicSelect"><?=t('Name');?></label>
									<fieldset class="form-group">
										<input type="text" class="form-control" id="modaltypename" placeholder="<?=t('Name');?>" name="Meds[name]" value="">
									</fieldset>
								</div>
								...
								<label for="basicSelect"><?=t('Unit');?></label>
									<fieldset class="form-group">
										<select name='Meds[unit]' class='form-control' id="modalMedunit2" placeholder="<?=t('Unit');?>">
											<?php
											$birimler=Units::model()->findAll(array('condition'=>'active=1'));
											foreach($birimler as $birim)
											{  ?>
								...
								<label for="basicSelect"><?=t('Is Active');?></label>
									<fieldset class="form-group">
										 <select class="custom-select block" id="modaltypeisactive" name="Meds[isactive]">
								...
								<label for="basicSelect"><?=t('Brand');?></label>
									<fieldset class="form-group">
										<input type="text" class="form-control" id="modalBrand" placeholder="<?=t('Brand');?>" name="Meds[brand]" value="">
									</fieldset>
								</div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-danger white">
                            <h4 class="modal-title" id="myModalLabel8"><?=t('Medfirm Delete');?></h4>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
                            </button>
                        </div>
					...
					<!--form baslangıç-->						
						<form id="med-form" action="/meds/delete1/0" method="post">	
						
						<input type="hidden" class="form-control" id="modaltypeid2" name="Meds[id]" value="0">
	...
	<!--delelete all start-->
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="deleteall" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
				...
				<!--form baslangıç-->						
						<form action="/meds/deleteall" method="post">	
						
						<input type="hidden" class="form-control" id="modalid3" name="Meds[id]" value="0">
...
function openmodal(obj)
{
	...
	$('#modaltypename').val($(obj).data('name'));
	$('#modalMedunit2').val($(obj).data('unit'));
	
	$('#modalBrand').val($(obj).data('brand'));
...
{
	<?=Yii::app()->baseUrl?>
$.post( "/Meds/?", { visittypeid: data, active: permission })
