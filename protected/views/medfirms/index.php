...
$medfirms=Medfirms::model()->findAll(array('order'=>'name ASC','condition'=>$where));
//$visittypes=Visittype::model()->findAll(array('order'=>'name ASC'));?>


<?php if (Yii::app()->user->checkAccess('medfirms.view')){ ?>
<!-- Sayfada neredeyiz -->
<?=User::model()->geturl('Medfirms','',0,'medfirms');?>
			...
			<div class="card">
				    <div class="card-header">
						 <div class="row" style="padding-bottom: 10px;border-bottom: 1px solid #f8f8f9;">
							 <div class="col-md-6">
								  <h4  class="card-title"><?=t('Medfirm Create');?></h4>
									</div>
									 <div class="col-md-6">
				...
				<form id="visittype-form" action="/medfirms/create" method="post">
				<div class="card-content">
					...
					<div class="row">
						...
						<div class="col-xl-4 col-lg-4 col-md-4 mb-1">
							...
							<fieldset class="form-group">
								<select class="select2" style="width:100%" id="firm" name="Medfirms[firmid]" onchange="myfirm()" requred>
						...
						<?php }else{?>
							<input type="hidden" class="form-control" id="firm" name="Medfirms[firmid]" value="<?=$ax->firmid;?>" requred>
						<?php }?>
						...
						<label for="basicSelect"><?=t('Branch');?></label>
							<fieldset class="form-group">
								<select class="select2" style="width:100%" id="branch" name="Medfirms[branchid]" disabled requred>
									<option value="0"><?=t('Please Chose');?></option>
						...
						<?php }else{?>
							<input type="hidden" class="form-control" id="branch" name="Medfirms[branchid]" value="<?=$ax->branchid;?>" requred>
						<?php }?>
					...
					<label for="basicSelect"><?=t('Name');?></label>
                        <fieldset class="form-group">
                          <input type="text" class="form-control" id="basicInput" placeholder="<?=t('Name');?>" name="Medfirms[name]" requred>
                        </fieldset>
                    </div>
					...
					<label for="basicSelect"><?=t('Is Active');?></label>
                       <fieldset class="form-group">
                          <select class="custom-select block" id="customSelect" name="Medfirms[isactive]">
                ...
                <div class="card-header">
					<div class="row" style="border-bottom: 1px solid #e3ebf3;">
					   <div class="col-xl-9 col-lg-9 col-md-9 mb-1">
						 <h4 class="card-title"><?=t('MEDFIRMS LIST');?></h4>
						</div>

						<?php if (Yii::app()->user->checkAccess('medfirms.create')){ ?>
						<div class="col-xl-3 col-lg-3 col-md-3 mb-1">
								<div class="input-group-append" id="button-addon2" style="float: right; text-align: right;">
									<button class="btn btn-info" id="createbutton" type="submit"><?=t('Add Medfirm');?> <i class="fa fa-plus"></i></button>
								</div>
						</div>
						<?php }?>
					</div>
                ...
                <div class="card-content collapse show">
                  <div class="card-body card-dashboard">
                      ...
                      <table class="table table-striped table-bordered dataex-html5-export">
                        ...
                        <tbody>
             			<?php foreach($medfirms as $medfirm):?>
                                <tr>

                                    <td><a href="<?=Yii::app()->baseUrl;?>/medfirms/view/<?=$medfirm->id;?>"><?=$medfirm->name;?></a></td>

								<?php if($ax->firmid==0){?>
								<td><?=Firm::model()->find(array('condition'=>'id='.$medfirm->firmid))->name;?></td>
								<?php }?>

								<?php if($ax->branchid==0){?>
								<td><?=Firm::model()->find(array('condition'=>'id='.$medfirm->branchid))->name;?></td>
								<?php }?>


								<td>
									<div class="form-group pb-1">
										<input type="checkbox" id="switchery" data-size="sm"  class="switchery" data-id="<?=$medfirm->id;?>"  <?php if($medfirm->isactive==1){echo "checked";}?>  <?php if (Yii::app()->user->checkAccess('medfirms.update')==0){?>disabled<?php }?> />
									</div>
								</td>

									<td>
									<?php if (Yii::app()->user->checkAccess('medfirms.update')){ ?>
										<a  class="btn btn-warning btn-sm" onclick="openmodal(this)" data-id="<?=$medfirm->id;?>" data-name="<?=$medfirm->name;?>" data-active="<?=$medfirm->isactive;?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?=t('Update');?>"><i style="color:#fff;" class="fa fa-edit"></i></a>
									<?php }?>
									<?php if (Yii::app()->user->checkAccess('medfirms.delete')){ ?>

									<?php if($ax->firmid==0 || ($medfirm->firmid==$ax->firmid)|| (($medfirm->firmid==$ax->firmid) && ($medfirm->branchid==$ax->branchid))){?>
									<a class="btn btn-danger btn-sm" onclick="openmodalsil(this)" data-id="<?=$medfirm->id;?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?=t('Delete');?>"><i style="color:#fff;" class="fa fa-trash"></i></a>
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="duzenle" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
                ...
                <div class="modal-dialog" role="document">
					...
					<!--form baslangıç-->
						<form id="medfirms-form" action="/medfirms/update/0" method="post">
                            <div class="modal-body">
								<input type="hidden" class="form-control" id="modaltypeid" name="Medfirms[id]" value="0">
								<input type="hidden" class="form-control" id="basicInput" placeholder="<?=t('Name');?>" name="Medfirms[firmid]" value="<?=$ax->firmid;?>">
								<div class="col-xl-12 col-lg-12 col-md-12 mb-1">
								<label for="basicSelect"><?=t('Name');?></label>
									<fieldset class="form-group">
										<input type="text" class="form-control" id="modaltypename" placeholder="<?=t('Name');?>" name="Medfirms[name]" value="">
									</fieldset>
								</div>
								...
								<label for="basicSelect"><?=t('Is Active');?></label>
									<fieldset class="form-group">
										 <select class="custom-select block" id="modaltypeisactive" name="Medfirms[isactive]">
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="sil" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
                ...
                <div class="modal-dialog" role="document">
					...
					<!--form baslangıç-->
						<form id="medfirms-form" action="/medfirms/delete1/0" method="post">

						<input type="hidden" class="form-control" id="modaltypeid2" name="Medfirms[id]" value="0">
	...
	<!--delelete all start-->
        ...
        <div class="form-group">
            ...
            <div class="modal fade text-left" id="deleteall" tabindex="-1" role="dialog" aria-labelledby="myModalLabel8"
				...
				<!--form baslangıç-->
						<form action="/medfirms/deleteall" method="post">
