...
$fbtitle=Firm::model()->find(array('condition'=>'id=:id','params'=>array('id'=>$clientbtitle->firmid)));
...
<div class="card-content">
      ...
      ?>
      <li class="nav-item ">
                  <a class="nav-link category <?php if($i==1){echo 'active';}?>" id="<?=$category->id;?>"  data-toggle="tab" onclick="tabs(this)"  data-id="<?=$category->id;?>" data-name='<?=t($category->name);?>' aria-controls="tab<?=$category->id;?>" href="#tab<?=$category->id;?>" aria-expanded="true"><?=t($category->name);?>

          <?php            $altnew=Documentcategory::model()->findAll(array('order'=>'id ASC','condition'=>'isactive=1 and parent='.$category->id));
            $say=0;
            $say=$say+User::model()->newdocument($category->id,$_GET['id']);
            foreach($altnew as $altnewx)
            {
              $say=$say+User::model()->newdocument($altnewx->id,$_GET['id']);
            }
          ?>
    ...
    <div class="col-lg-4 col-md-4 col-sm-12" id="documents">
       <div class="card">
          ...
          <div class="horz-scroll-content">
					<?php  $categorys=Documentcategory::model()->find(array('order'=>'id ASC','condition'=>'isactive=1 and parent=0'.$documentCatulkewhere,));?>
					<?Documents::model()->kategoritabloyaz($categorys->id,0,$_GET['id']);?>
					</div>
					</div>
				</div>
			</div>
        ...
        <div class="card-content collapse show">
			<div class="card-body">
				<form id="documents-form-create" >
					<div class="row">
						<input  type="hidden" name="Documents[cbid]" value="<?=$_GET['id']?>"/>
						<input  type="hidden" name="Documents[cid]" value="<?=$clientbtitle->parentid;?>"/>
						<input  type="hidden" name="Documents[fbid]" value="<?=$clientbtitle->firmid;?>"/>
						<input  type="hidden" name="Documents[fid]" value="<?=$fbtitle->parentid;?>"/>

						<input  type="hidden" name="Documents[filetype]" value="1"/>
						<input  type="hidden" name="Documents[viewer][]" value="<?=$_GET['id']?>"/>
...
<!--S�L BA�LANGI�-->
<?php if (Yii::app()->user->checkAccess('client.branch.filemanagement.delete')){ ?>
<div class="col-lg-4 col-md-6 col-sm-12">
 <div class="form-group">
										<!-- Modal -->
...
#main-class>li>.category {
    min-width: 200px !important;
}
.horz-scroll-content>ul {
    padding-left: 0px;
}
</style>
...
$.post( "/documents/documentlist?id="+document.getElementById("categoriid").value+'&&firmid=<?=$who->id;?>&&cbid=<?=$_GET['id'];?>&&firmtype=<?=$who->type;?>').done(function( list ) {
 ...
 $("#documents-form-create").on('submit',(function(e) {
   ...
   url:"/documents/create3",
	...
	{
       $.post( "/documents/documentlist?id="+document.getElementById("categoriid").value+'&&firmid=<?=$who->id;?>&&cbid=<?=$_GET['id'];?>&&firmtype=<?=$who->type;?>&status='+status).done(function( list )
		{
		   	$.unblockUI();
			$('#documentlist').html(list);
			$('[data-toggle="tooltip"]').tooltip({
 ...
 //documents delete post start
	$("#documents-form-delete").on('submit',(function(e) {
	 ...
	 $.ajax({
		 ...
		 success: function(data)
			 {
			 $.post( "/documents/documentlist?id="+document.getElementById("categoriid").value+'&&firmid=<?=$who->id;?>&&cbid=<?=$_GET['id'];?>&&firmtype=<?=$who->type;?>&status='+arsivStatus).done(function( list ) {
				 $('#documentlist').html(list);
...
function tabs(obj)
{
	...
	$('#categoriid').val($(obj).data('id'));
	  ...
	  $.post( "/documents/documentmenu?id="+$(obj).data('id')+"&&cid=<?=$_GET['id']?>").done(function( data ) {
	 ...
	 $.post( "/documents/documentlist?id="+document.getElementById("categoriid").value+'&&firmid=<?=$who->id;?>&&cbid=<?=$_GET['id'];?>&&firmtype=<?=$who->type;?>').done(function( data ) {
			$('#documentlist').html(data);
