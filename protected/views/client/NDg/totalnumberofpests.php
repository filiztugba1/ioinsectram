...
{
	if(!empty($_POST['Monitoring']['subid']) && intval($_POST['Monitoring']['subid'][0])!==0)
	{
...
if(intval($_POST['Report']['subid'])!= 0)
{
...
// echo $sql.' clientbranchid='.$_POST['Report']['clientid'].' and isproduct=0   and checkdate!=0 and monitortype '.$monitortypexx;
$reports=MobileworkorderdataView::model()->findAll(array('condition'=>$sql.' clientbranchid='.$_POST['Report']['clientid'].' and isproduct=0   and checkdate!=0 and monitortype '.$monitortypexx,'order'=>'checkdate asc, petid asc'));



	foreach ($reports as $report)
	{
		if(!in_array(date("m-Y", $report->checkdate),$tarihler))
		{
			array_push($tarihler,date("m-Y", $report->checkdate));

		}
		/////////////////// Gösterilecek zamanlar start ////////////////////////
		if(!in_array(date("m/Y", $report->checkdate),$gostertarih))
		{
			array_push($gostertarih,date("m/Y", $report->checkdate));
		}
	...
	/*	$tarihbasla=strtotime($tarih);
		...
		/*$reports=Mobileworkorderdata::model()->findAll(array('select'=>'SUM(value) AS total','condition'=>$sql.' clientbranchid='.$_POST['Report']['clientid'].' and isproduct=0 and (checkdate between '.$tarihbasla.' and '.$tarihbiti.')','order'=>'petid asc'));
		*/
		$pet_ids=[];
    
     if(intval($_POST['Monitoring']['mtypeid'])==-100)
{
	$monitortypexxx=' in (24,25,26,30)';
       $monitorpettypes=Monitoringtypepets::model()->findAll(array('condition'=>'monitoringtypeid '.$monitortypexxx));
}
else
{
...
{
	$pet_ids[]=$monitorpettype->petsid;
}
		$parent= Yii::app()->db->createCommand('SELECT *,sum(value) as total FROM mobileworkorderdata_view where petid in ('.implode(', ', $pet_ids).') and  '.$sql.' clientbranchid='.$_POST['Report']['clientid'].' and monitortype '.$monitortypexx.' and isproduct=0 and (checkdate between '.$x1.' and '.$x2.') group by petid')->queryAll();
			...
			if($heyvan)
			{
				...
				if(!in_array($heyvan->name,$petler))
				{
					if(!in_array($heyvan->name,$temproray))
					{
						...
						array_push($petscolor,$heyvan->color);
					}
					...
					array_push($listeveri,array("name"=>$heyvan->name,"data"=>$report['total']));
          						...
          						array_push($temproray,$heyvan->name);
								array_push($petscolor,$heyvan->color);
					array_push($listeveri,array("name"=>$heyvan->name,"data"=>0));
...
$thh='<th>';
 foreach($monitortype as $monitortypek)
						  {
							 $thh.= $monitortypek->name.(count($monitortype)>1?', ':' '); 
						  }
                ...
                <div class="card-header">
					<div class="row" style="border-bottom: 1px solid #e3ebf3;">
					   <div class="col-xl-9 col-lg-9 col-md-9 mb-1">
						  <h4 class="card-title"><?=t('Total Number of Pests Based on Date');?>   <?=$_POST["Monitoring"]["date"];?> / <?=$_POST["Monitoring"]["date1"];?> <br>   
						  <?php 
						  foreach($monitortype as $monitortypek)
						  {
						  ...
						  ?>	</h4> <br> <?=t('Rapor Kriterleri')?>:<?=$kriterler?>
...
// specify chart configuration item and data
        option = {
            ...
            toolbox: {
			 ...
			 color: [<?php $yaz="";foreach ($petscolor as $color){ $yaz= $yaz.'"#'.trim($color).'"'.",";} echo rtrim($yaz,",");?>],
           
            // color: ["#F2490C","#e003fb","#7b1fa2","#ff7588","#65BFBF","#5C5A61","#F2B90C","#0018c5","#7d0404","#99ff66","#00ffff","#9933ff"],
            calculable : true,
            xAxis : [
                {
                    type : 'category',
	 ...
	 buttons: [
        {
			...
			messageTop:'<?=t("Total Number of Pests Based on Date");?>   <?=$_POST["Monitoring"]["date"];?> / <?=$_POST["Monitoring"]["date1"];?>  <?=$monitortype->name.' - '.$monitortype->detailed;?>	<?=t("Rapor Kriterleri")?>:<?=$kriterler?>'
        },
        {
            extend: 'excelHtml5',
            exportOptions: {
			...
			className: 'd-none d-sm-none d-md-block',
			title:'Monitoring Points (<?=date('d-m-Y H:i:s');?>)',
			messageTop:'<?=t("Total Number of Pests Based on Date");?>   <?=$_POST["Monitoring"]["date"];?> / <?php 
						  foreach($monitortype as $monitortypek)
						  {
							 echo $monitortypek->name.' - '.t($monitortypek->detailed).', '; 
						  }
			...
			text:'<?=t('PDF');?>',
			  ...
			  customize: function(doc) {
				doc.content.splice(0, 1, {
				 ...
				 {
					text: '<?=t("Total Number of Pests Based on Date");?>   <?=$_POST["Monitoring"]["date"];?> / <?=$_POST["Monitoring"]["date1"];?>  <?php 
						  foreach($monitortype as $monitortypek)
						  {
							 echo $monitortypek->name.' - '.t($monitortypek->detailed).', '; 
						  }
