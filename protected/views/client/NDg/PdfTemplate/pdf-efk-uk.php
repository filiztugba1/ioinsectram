...
foreach ($monitors as $monitor) {
  ...
  if($monitor->firmid==412)
  {
      $bolum=$bolum.' - '.$monitor->definationlocation;
        ...
        ';
        $reports = MobileworkorderdataView::model()->findAll(array('select'=>'value,createdtime','condition' => 'isproduct=0  and mobileworkordermonitorsid=' . $monitor->id, 'order' => 'FIELD(petid,72,77,73,78,74,75,29,31,76,25)'));
        foreach ($reports as $report) {
            $veriler = $veriler . '<td align="center">' .($report->createdtime>0?$report->value:'-') . '</td>'; // Kapalı
...
<table border="0" width="100%" cellpadding="0" cellspacing="0">
	...
	<tr>
		<td align="center">'.t('Monitor No').'</td>
