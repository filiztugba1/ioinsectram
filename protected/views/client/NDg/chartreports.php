...
$monitornames=Monitoringtype::model()->findAll(array("condition"=>" id ".$monitortypexx));
$mtypess=[];
$mtypess[]='xxxxxx';
foreach ($monitornames as $monitornamesx)
{
    $mtypess[]=$monitornamesx->short_code;
}
...
?>
  ...
  <div class="row">
    ...
    <div class="col-lg-12">
      <div class="card">
        <div class="card-block">
          <h4 class="card-title">
            <?=t('Regional Trend Analysis')?>
              <?=$_POST["Monitoring"]["date"];?> /
                <?=$_POST["Monitoring"]["date1"];?> <br>
                 <?php 
            $mtypesxs=[];
						  foreach($monitortype as $monitortypek)
						  {
                $mtypesxs[]=$monitortypek->short_code;
    ...
    <?php 
    $sqll=' and ( monitor like "%xxxxxxx%" ';
   // $haritalar=$_POST["Report"]["clientid"];
    foreach ($mtypess as $mps){
          $sqll.=' or monitor like "%'.$mps.'(%" ';
    }
    $sqll.=')';
    
    $harital=Maps::model()->findall(array( 'condition'=>'client_id='.$_POST["Report"]["clientid"].' '.$sqll.' order by id asc'));
   foreach($harital as $haritalar){
  $adsx=[];
  			for($i=0;$i<count($yeni2);$i++)
							{
          if ($yeni2[$i]['value']>0){
$adsx[]=$yeni2[$i]['key'].'-'.$yeni2[$i]['value'];
...
<center><center><h2>
  <?=$haritalar->map_name?>
  </h2></center><iframe sandbox="allow-same-origin allow-scripts" srcx="/map/heatmap?cbid=<?=$_POST["Report"]["clientid"]?>&mapid=<?=$haritalar->id?>&mnos=<?=$ardi?>&mtypes=<? echo implode(',',$mtypesxs);?>&&rand=<?=uniqid()?>" scrolling="no" frameborder="no" 
style="overflow:hidden;border:0;margin:0;padding:0;width:1074px;height:750px;"></iframe></center>
    </div>
<?php
...
iframes.forEach((iframe, index) => {
  const srcx = iframe.getAttribute('srcx');
  if (!srcx) return;

  setTimeout(() => {
    iframe.src = srcx + (srcx.includes('?') ? '&' : '?') + 'nocache=' + Date.now();
  }, index * 500); // her biri bir öncekinden 500ms sonra başlasın
});
</script>
