...
<?= User::model()->geturl('Client', 'Reports', $_GET['id'], 'client', 0, $ax->branchid); ?>
<div class="card">
    <div class="card-header" style="">
        <ul class="nav nav-tabs">
            <?php if ($ax->clientbranchid != 0) { ?>
                    ...
                    <li class="nav-item">
                        <a class="nav-link"
                            href="<?= Yii::app()->baseUrl ?>/client/branchstaff?id=<?= $getidx; ?>&firmid=<?= $ax->branchid; ?>"><span
                                ...
                                <?php $say = User::model()->findAll(array('condition' => 'clientbranchid=' . $getidx));
                                echo count($say); ?>
                            </span><?= t('Staff'); ?>
                ...
                <li class="nav-item">
                    <a class="nav-link" href="<?= Yii::app()->baseUrl ?>/client/clientqr?id=<?= $getidx; ?>" target="_blank"><span class="btn-effect"
                            style="font-size: 15px;"><i class="fa fa-qrcode"
                ...
                if (($ax->id == 0 || $ax->id == 317 || $ax->id == 588)) { ?>
                    <li class="nav-item">
                        <a class="nav-link" href="<?= Yii::app()->baseUrl ?>/client/maps?id=<?= $getidx; ?>"><span class="btn-effect"
            ...
            <?php } else { ?>
                ...
                <?php if (Yii::app()->user->checkAccess('client.staff.view')) { ?>
                    <li class="nav-item">
                        <a class="nav-link " href="<?= Yii::app()->baseUrl ?>/client/staff/<?= $_GET['id']; ?>"><span
                                class="btn-effect2" style="font-size: 15px;">
                ...
                <?php }
                if ($ax->type == 23 || $ax->type == 13 || $ax->id == 1) { ?>
                    <li class="nav-item">
...
<div class="card">
    ...
    <form id="reports-form" action="<?= Yii::app()->baseUrl ?>/client/reportcreate" method="post" target="_blank">
        <div class="card-content">
            <div class="card-body">

                <input type="hidden" class="form-control" id="basicInput" name="Report[client]"
                    value="<?= $_GET['id']; ?>">
                ...
                <div class="row">
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-4 mb-1">
                        ...
                        <fieldset class="form-group">
                            ...
                            <select class="select2" style="width:100%" name="Report[type]" id="reptype" required>
                                ...
                                <option value="99"><?= t('Servis Formu Raporları') ?></option>
                            </select>
                        </fieldset>
                    </div>
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-4 mb-1">
                        ...
                        <fieldset class="form-group">
                            ...
                            <select class="select2" style="width:100%" id="client" multiple="multiple"
                                name="Report[clientid][]" required onchange="myFunctionClient()">
                                ...
                                foreach ($clientbranchs as $clientbranch) {
                                    ...
                                    if ($ax->clientbranchid != 0) {
                                        ...
                                        if ($return) {
                                            ?>
                                                <option 
                                                    value="<?= $clientbranch->id; ?>"><?= $clientx->name; ?> -> <?= isset($clientbranch->name) ? $clientbranch->name : ''; ?>
                                                </option>
                    ...
                    <div class="col-xl-6 col-lg-6 col-md-6 mb-1">
                        ...
                        <fieldset class="form-group">
                            <input type="date" class="form-control" id="basicInput" name="Monitoring[date1]"
                                value="<?= date("Y-m-d", time()) ?>">
                        </fieldset>
                    </div>
...
<script>
    ...
    function myFunction() {
        ...
        if ($("#typeselect").val() == 0) {
            $.post("<?= Yii::app()->baseUrl ?>/client/tumunugetir?id=" + <?= $_GET["id"] ?>).done(function (data) {
                $('#pointno').html(data);
            ...
            $.post("<?= Yii::app()->baseUrl ?>/client/pointno?department=" + department + "&&subdepartment=" + subdepartment + "&&type=" + type).done(function (data) {
                $('#pointno').html(data);
            });
        }
    }
    ...
    function myType() {
        ...
        var client = <?= $_GET["id"] ?>;
        $.post("<?= Yii::app()->baseUrl ?>/client/pointno?department=" + department + "&&subdepartment=" + subdepartment + "&&type=" + type + "&&clientid=" + client).done(function (data) {
            $('#pointno').html(data);
        });

        //Zararlıları getir
        $.post("<?= Yii::app()->baseUrl ?>/client/peststypes?type=" + type + "&reptype=" + reptype).done(function (data) {
            $('#pests').html(data);
        });
...
*/
    ...
    <?php if ($ax->firmid != 0) { ?>
        ...
        if ($("#firm").length > 0) {
            $.post("<?= Yii::app()->baseUrl ?>/workorder/firmbranch?id=" + document.getElementById("firm").value).done(function (data) {
                $("#branch").prop("disabled", false);
                $('#branch').html(data);
    ...
    function myFunctionClient() {
        ...
        $.post("<?= Yii::app()->baseUrl ?>/conformity/client?id=" + document.getElementById("client").value).done(function (data) {
        ...
        $.post("<?= Yii::app()->baseUrl ?>/conformity/conformitytype?id=" + document.getElementById("client").value + '&&branch=' + document.getElementById("branch").value + '&&firm=' + document.getElementById("firm").value).done(function (data) {
    ...
    function myFunctionDepartment() {
        $.post("<?= Yii::app()->baseUrl ?>/conformity/department?id=" + document.getElementById("department").value).done(function (data) {
            $("#subdepartment").prop("disabled", false);
            $('#subdepartment').html(data);
            //$("#"+$(obj).data('id')+"x").css("background-color", "yellow");
    ...
    function myfirm2() {
        $.post("<?= Yii::app()->baseUrl ?>/workorder/firmbranch?id=" + document.getElementById("firm2").value).done(function (data) {
    ...
    function myFunctionClient2() {
        $.post("<?= Yii::app()->baseUrl ?>/conformity/client?id=" + document.getElementById("client2").value).done(function (data) {
    ...
    function myFunctionDepartment2() {
        $.post("<?= Yii::app()->baseUrl ?>/conformity/department?id=" + document.getElementById("department2").value).done(function (data) {
