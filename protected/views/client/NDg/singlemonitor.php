...
if(!isset($_POST["Monitoring"]["subid"]) || count($_POST['Monitoring']['subid'])==0){  //sub departman yoksa girsin
  if(!in_array("0",$dep))
  {
    ...
    if(is_countable($dep) && count($dep)>1)
    {
...
if(isset($_POST["Monitoring"]["monitors"]) && $_POST["Monitoring"]["monitors"][0]!='null')
{
...
$reports=MobileworkordermonitorsView::model()->findAll(array('select'=>'*,DATE_FORMAT(FROM_UNIXTIME(`checkdate`), "%Y-%m-%d") AS "timestamp"','condition'=>$sql.'checkdate!=0 and isdelete=0 and clientbranchid='.$_POST['Report']['clientid'].' and monitortype '.$monitortypexx.'','order'=>'timestamp asc, monitorid asc'));
...
if(isset($_POST["Monitoring"]["subid"]))
{
    ...
    foreach ($_POST['Monitoring']['subid'] as $item)
    {
		...
		$sql1= $sql1."mobileworkordermonitors_view.subdepartment=".$item." or ";
    }
    $sql=rtrim($sql,"or ");
    $sql= $sql.") and ";
    ...
    $sql1= $sql1.") and ";
		  ...
		  $sql1= $sql1."mobileworkordermonitors_view.subdepartment=".$_POST['Monitoring']['subid'][0]." and ";
...
{
    $sql= $sql." departmentid=".$_POST['Report']['dapartmentid']." and ";
	$sql1= $sql1." mobileworkordermonitors_view.departmentid=".$_POST['Report']['dapartmentid']." and ";
}
if($_POST['Monitoring']['date'])
{
    $sql= $sql." createdtime >=".$midnight." and ";
	$sql1= $sql1." mobileworkordermonitors_view.checkdate >=".$midnight." and ";
}
if($_POST['Monitoring']['date1'])
{
    $sql= $sql." createdtime <=".$midnight2." and ";
	$sql1= $sql1." mobileworkordermonitors_view.checkdate <=".$midnight2." and ";
}
$monitornn='';
if(isset($_POST["Monitoring"]["monitors"]) && $_POST["Monitoring"]["monitors"][0]!="null")
{
...
if(isset($_POST['Report']['pests']))
{
	...
	if(count($_POST['Report']['pests'])>0)
	{
		...
		foreach ($_POST['Report']['pests'] as $item)
		{
			...
			$sqldata1= $sqldata1."mobileworkorderdata_view.petid=".$item." or ";
		...
		$sqldata1=$sqldata1."mobileworkorderdata_view.petid!=0 and mobileworkorderdata_view.isproduct!=1";
		$sqldata1= $sqldata1.") and";
	}
}
 ...
 // exit;
	...
	/*	$tarihbasla=strtotime($tarih);
		...
		*/
		if($secilimi==true)
		{
			$parent= Yii::app()->db->createCommand('SELECT *,sum(value) as total FROM mobileworkorderdata_view where  '.$sqldata.' clientbranchid='.$_POST['Report']['clientid'].' and monitortype '.$monitortypexx.' and isproduct=0 and (checkdate between '.$x1.' and '.$x2.') and monitorid in (SELECT id from monitoring where '.($monitornn!=''?'id='.$monitornn.' and ':'').'   clientid='.$_POST['Report']['clientid'].') ')->queryAll();
			...
			$parent= Yii::app()->db->createCommand('SELECT *,sum(value) as total FROM mobileworkorderdata_view where  clientbranchid='.$_POST['Report']['clientid'].' and monitortype '.$monitortypexx.' and isproduct=0 and (checkdate between '.$x1.' and '.$x2.') and monitorid in (SELECT id from monitoring where '.($monitornn!=''?'id='.$monitornn.' and ':'').' clientid='.$_POST['Report']['clientid'].')')->queryAll();
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-block">
                <h4 class="card-title"><?=t('Single Monitor Report')?> <br> <?=t('Monitor No');?>: <?=$monitornn!=''?(Monitoring::model()->find(array('condition'=>'id='.$monitornn))->mno):'';?>  <br> <?=$_POST["Monitoring"]["date"];?> / <?=$_POST["Monitoring"]["date1"];?><br>   <?php 
                ...
                data:[
					...
					else
					{
						...
						else
						{
							...
							{
								$findpet=Pets::model()->findByPk($item);
								$yazheyvanlari= $yazheyvanlari.t($findpet->name).", ";
							}
							$yazheyvanlari=rtrim($yazheyvanlari,", ");
							$yazheyvanlari= $yazheyvanlari." ";
							$yazheyvanlari= $yazheyvanlari.'"';
	 ...
	 buttons: [
        {
			...
			messageTop:'<?=t('Single Monitor Report')?> <br> <?=t('Monitor No');?>: <?=$monitornn?>  <br> <?=$_POST["Monitoring"]["date"];?> / <?=$_POST["Monitoring"]["date1"];?><br>   <?php 
						  foreach($monitortype as $monitortypek)
						  {
							 echo $monitortypek->name.' - '.t($monitortypek->detailed).', '; 
						  }
        ...
        },
        {
			...
			text:'<?=mb_strtoupper(t('Pdf'));?>',
			  ...
			  title: '<?=t('Single Monitor Report')?> <br> <?=t('Monitor No');?>: <?=$monitornn?>  <br> <?=$_POST["Monitoring"]["date"];?> / <?=$_POST["Monitoring"]["date1"];?><br>   <?php 
						  foreach($monitortype as $monitortypek)
						  {
							 echo $monitortypek->name.' - '.t($monitortypek->detailed).', '; 
						  }
			  ...
			  customize: function(doc) {
				doc.content.splice(0, 1, {
				  text: [{
					text: '<?=t('Single Monitor Report')?> <br> <?=t('Monitor No');?>: <?=$monitornn?>  <br> <?=$_POST["Monitoring"]["date"];?> / <?=$_POST["Monitoring"]["date1"];?><br>   <?php 
						  foreach($monitortype as $monitortypek)
						  {
							 echo $monitortypek->name.' - '.t($monitortypek->detailed).', '; 
						  }
