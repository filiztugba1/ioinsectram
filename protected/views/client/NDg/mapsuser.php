...
<center><center><h2>
  <?=$map->map_name?>
  </h2></center><iframe sandbox="allow-same-origin allow-scripts" srcx="/map/heatmap?cbid=<?=$map->client_id?>&mapid=<?=$map->id?>&rand=<?=uniqid()?>&mnos=1&noact=1" scrolling="no" frameborder="no" 
style="overflow:hidden;border:0;margin:0;padding:0;width:1074px;height:auto;"></iframe></center><br><br><hr><br>
...
iframes.forEach((iframe, index) => {
  ...
  setTimeout(() => {
    ...
    // src yüklendikten sonra boyutlandır
    iframe.addEventListener('load', () => {
      try {
        ...
        console.warn('iframe cross-origin olduğu için yüksekliği alınamadı.');
      }
    });

  }, index * 500); // sırayla yükle
});
</script>
        </section>
