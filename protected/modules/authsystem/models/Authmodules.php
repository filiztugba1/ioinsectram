	...
	public function header2($id)
	{
		...
		foreach($leftmenu as $leftmenux)
		{
			...
			$menu=Authmodules::model()->findAll(array('order'=>'menurow ASC','condition'=>'menuview=1 and parentid='.$leftmenux->id));
					...
					?><li <?if($url==$leftmenux->menuurl){echo 'class="active"';}
					...
					<?		if($leftmenux->name=='Firm')
								{if($ax->type==1)
									{echo Yii::app()->baseUrl.$leftmenux->menuurl;}
									else
									{
										...
										{
											echo Yii::app()->baseUrl.$leftmenux->menuurl.'/staff?type=branch&&id='.$branch->id;
										}
										else
										{
											echo Yii::app()->baseUrl.$leftmenux->menuurl.'/branch?type=firm&&id='.$ax->firmid;
										}
								...
								{if($ax->clientid>0)
									{if($ax->clientbranchid>0)
										{echo Yii::app()->baseUrl.$leftmenux->menuurl.'/branches/'.$ax->clientbranchid;}
										else
											{echo Yii::app()->baseUrl.$leftmenux->menuurl.'/detail/'.$ax->clientid;}
									}
									else
									{echo Yii::app()->baseUrl.$leftmenux->menuurl;}
								}
			...
			foreach($leftmenu as $menu)
			{
				...
				if($issabcategory==1)
				{
					...
					{?>
						...
						<li <?if($url==$menu->menuurl){echo 'class="active"';}?>><a class="nav-item" href="/
							...
							?>
							"><?if($issabcategory==1){?><i class="<?=$menu->menuicon;?>"></i><?}?>
