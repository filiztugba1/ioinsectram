...
class Workorder extends CActiveRecord
{
	...
	public function search()
	{
		...
		$criteria->compare('special_notes',$this->special_notes,true);
	...
	public function monitorupdate($workorderid)
	{
		...
		if ($jobs)
		{
			foreach ($jobs as $model)
			{
				...
				{
					if(!in_array(intval($item),$allmonitors))
					{
						$allmonitors[]=$item;
					}
				}
			}
				foreach ($allmonitors as $item)
				{
					...
					$m->saverid=0;//Yii::app()->user->id;//getuserid
	...
	public function periodcopy($code,$id,$oldcode='')
	{
		...
		$period='';
  //  echo 'sssss'.$date->startfinishdate.' - '.$pdate[0].' ___ '.$pdate[1].'<br>';
      ...
      //  echo '<br>eeeee'.ucwords($day[$i]).' this week<br>';
...
//////echo 'begin'.$sdate.'<br>';
  ...
  //////  echo 'end'.$againnumber.'- '.$type.'<br>';
              ...
              $sonuc = strpos($date->format("Y-m-d"),"970-");
if ($sonuc === false)

    {
}else{
  echo 'tarih hatası oluştu!';
  exit;
}
	...
	public function workorderperiodcopy($id,$date)//perioddan gelen date değerlerini
	{
					...
					$wperiod->special_notes=$copy->special_notes;
	...
	public function workorderList($request)
	{
		...
		if(isset($request['client']) && $request['client']!='' && $request['client']!=0)
		{
					...
					$response=$response->andwhere('c.id in ('.$request['client'].')');

		}
		if(isset($request['clientbranch']) && $request['clientbranch']!='' && $request['clientbranch']!=0)
		{

					$response=$response->andwhere('cb.id in ('.$request['clientbranch'].')');

		}

		if(isset($request['startDate']) && $request['startDate']!='' && $request['startDate']!=0)
		{

					$response=$response->andwhere('w.date>"'.$request['startDate'].'"');
					$response=$response->andwhere('w.date<"'.$request['endDate'].'"');

		}else {
      
					$response=$response->andwhere('w.date>"'.date("Y",strtotime("-1 year"))."-01-01".'"');
					$response=$response->andwhere('w.date<"'.date("Y-m-d",strtotime("+1 year")).'"');

    }

					$response=$response->andwhere('w.clientid>0');
  ...
  public static function toplumonitor($id)
  {
	  ...
	  $language= Translatelanguages::model()->find(array('condition'=>'id="'.$lguser->languageid.'"'))->name;
      ...
      mwd.petid petid,
      mwd.value valuem,
      IF(t.value is not null and t.value!='',t.value,p.name) petname,
      ...
      mwm.monitorno";
		...
		->leftJoin('translates t','t.title=p.name and t.code="'.$language.'"');
  ...
  public function addmonitortotifollow($workorderid=0){
    ...
    if ($workorderid==0){
      		...
      		$workorder=Workorder::model()->findbypk($workorderid);
      if ($workorder->visittypeid==31 || $workorder->visittypeid==62 || $workorder->visittypeid==26 ){
         // echo 'visittype '.$workorderidf.PHP_EOL;
      }else{
       // echo 'visittype değişmiş';
        exit;
      }
				if ($workorder && $workorder->autoaddmonitor==0)
				{
            // echo 'girdim '.$workorder->visittypeid.PHP_EOL;
            $data['Workorder']['workorderid']=$workorderid;
          $data['Workorder']['mavailable']=1;

          
                $veri="clientid=".$workorder->clientid;
                $veri=$veri." and active=1";
                 ...
                 $no=Monitoring::model()->findAll(array(
                  ...
                  foreach($no as $nox)
                  {
                     //  echo 'monitorler burda '.$workorder->visittypeid.PHP_EOL;
                              $data['Workorder']['monitoringno'][]=$nox->id;
                    //$nox->id;
                   //$nox->mno;

                  }
          if ($workorder->visittypeid==62){
              // echo 'visittype ti oldu '.$workorder->visittypeid.PHP_EOL;
               $model=new Departmentvisitedti;
          }else{
             //  echo 'visittype ti değil diğeri follow '.$workorder->visittypeid.PHP_EOL;
               $model=new Departmentvisited;
          }
       
          $model->mavailable=$data['Workorder']['mavailable'];
          $model->workorderid=$data['Workorder']['workorderid'];
          $model->departmentid=Workorderti::model()->msplit($data['Workorder']['departmentid']);
          $model->subdepartmentid=Workorderti::model()->msplit($data['Workorder']['subdepartmentid']);
          $model->monitoringtype=Workorderti::model()->msplit($data['Workorder']['monitoringtype']);
          $model->treatmenttypeid=Workorderti::model()->msplit($data['Workorder']['treatmenttypeid']);
          $model->monitoringno=Workorderti::model()->msplit($data['Workorder']['monitoringno']);
          $model->treatmenttypeid="";
          $model->save();
                 if ($workorder->visittypeid==62){
                   
              // echo 'visittype ti oldu2 '.$workorder->visittypeid.PHP_EOL;
