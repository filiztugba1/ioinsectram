...
class Conformity extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('userid, firmid, branchid, clientid, departmentid, subdepartmentid, type, statusid, priority, isefficiencyevaluation, endofdayemail,workorderid', 'numerical', 'integerOnly'=>true),
			array('date', 'length', 'max'=>15),
			array('definition, suggestion, filesf,filesf2', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, userid, firmid, branchid, clientid, departmentid, subdepartmentid, type, definition, suggestion, statusid, priority, date, isefficiencyevaluation, filesf,filesf2, endofdayemail,workorderid', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('filesf',$this->filesf2,true);
...
$criteria->compare('workorderid',$this->workorderid);
	...
	public function email($senderemail='oneri.uygunsuzluk@insectram.io',$sendername,$subject,$altbody,$msg,$buyeremail,$buyername,$pdf=null)
	{
			...
			$mail->SMTPAuth = true;
			$mail->Username = 'AKIAXKDNZVEJ7TOPVYPO';
			$mail->Password = 'BFGPo5zkfBHd9iXPvpt1tUXKwaR4EkYG62vXIKYDitFu';
			$mail->Port='25';
			$mail->SMTPSecure = "tls";
     //  $mail->SMTPDebug = 2;
...
//$mail->SMTPAutoTLS = false;//
   ...
   // exit;
	...
	public function conformityList($request)
	{
		...
		c.name as clientNames,
		
		IF(c.simple_client=1,CONCAT(c.name,' ( Simple Client )') ,c.name) as clientName,
		...
		IF(ecv.activitydefinition is null,'',ecv.activitydefinition) as etkinlik,
		...
		IF((cn.statusid=4 || cn.statusid=5 || cn.statusid=6 || cn.statusid=2 || cn.statusid=0),'',(IF(ecv.activitydefinition is NULL || ecv.activitydefinition='','','".t("Etkinlik Var")."'))) as etkinlikDurumu,
		cn.numberid as cnumber,
		cn.filesf as filesf,
		cn.filesf2 as filesf2
		")
		->from('conformity cn')
