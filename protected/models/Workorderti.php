 ...
 */
class Workorderti extends CActiveRecord
{
	/**
	...
	public function monitorupdate($workorderid)
	{
				...
				$deletemonitors=Mobileworkordermonitorsti::model()->deleteall(array('condition'=>'workorderid='.$workorderid));
				$deletemonitors1=Mobileworkorderdatati::model()->deleteall(array('condition'=>'workorderid='.$workorderid));
			//	$this->mobileworkorderadd($workorderid);
			
				$jobs=Departmentvisitedti::model()->findall(array('condition'=>'workorderid='.$workorderid));
		if ($jobs)
		{
			foreach ($jobs as $model)
			{
				...
				foreach ($monitors as $item)
				{
					...
					$m=new Mobileworkordermonitorsti;
					...
					if ($m->save())
					{
						...
						{
							$mdata=new Mobileworkorderdatati;
	...
	public function workorderperiodcopy($id,$date)//perioddan gelen date değerlerini
	{
					...
					if ($wperiod->save())
					{
						...
						$departmentvisitedcopy=Departmentvisitedti::model()->findAll(array('condition'=>'workorderid='.$id));
						foreach($departmentvisitedcopy as $vcopy)
						{
							$dvisit=new Departmentvisitedti;
    ...
    if(isset($id) && $id!=null && $id!='')
		{
			...
			$modelD=Mobileworkorderdatati::model()->findByPk($id);
			$check=Mobileworkorderdatati::model()->find(array('condition'=>'petid=49 and workorderid='.$modelD->workorderid.'  and monitorid='.$modelD->monitorid." and value=".$value));
				$modelMon=Mobileworkordermonitorsti::model()->findByPk($modelD->mobileworkordermonitorsid);
      	$date=$checkDate;
      ...
      if($check)
			...
			else
			{
				...
				//$model=Mobileworkordermonitorsti::
			}
  ...
  public static function monitorUpdateData($checkdate,$mobileworkordermonitorsid,$value)
  {
    	// Wdata1[id]: 498      mobileworkorderdata1[value]: 2
			...
			$modelD=Mobileworkorderdatati::model()->findByPk($mobileworkordermonitorsid);
			...
			if($modelD->petid==49)
			{
				...
				$modelM=Mobileworkordermonitorsti::model()->findByPk($modelD->mobileworkordermonitorsid);
			  $modelM->checkdate=$modelM->checkdate==0?$yenitarih:$modelM->checkdate; //// sorulacak Mustafa Beye
				...
				$modelD->value=$value;
				$modelD->update();
       $check=Mobileworkorderdatati::model()->find(array('condition'=>'petid=49 and workorderid='.intval($modelD['workorderid']).'  and mobileworkordermonitorsid='.intval($modelD['mobileworkordermonitorsid'])));
        if(isset($check->id) && $check->id>0)
        {
          $deletemonitors1=Mobileworkorderdatati::model()->deleteall(array('condition'=>'id='.$check->id));
        }
			}
  }
