...
//// eğer firma paket ayarına giriş yapıldıysa default paketindeki teknisyen limitine bakılmayacak
		$firm=Firm::model()->find(array('condition'=>'username="'.$package[1].'"'));
		if(!empty($firm))
		{
			$maxuser=Firmapackages::model()->find(array('condition'=>'firmid="'.$firm->id.'"'));
			if(empty($maxuser))
			{
		...
		foreach($firmBranchs as $firmBranch)
		{
			...
			foreach($baseauthpathfbranch as $baseauthpathfbranchk)
			{
				$isActive=User::model()->findByPk($baseauthpathfbranchk['userid']);
				
				if(intval($isActive->active)==1)
				{
	...
	/*	if(count($package)==2)
		...
		*/
		...
		if(count($package)==3)
		{
			...
			foreach($baseauthpath as $baseauthpathx)
			{
				$isActive=User::model()->findByPk($baseauthpathx['userid']);
			...
			if($say>=$maxuser->maxtech && $maxuser->maxtech!=-1)
				{
					$izin=0;
				}
		}
	...
	/* public function userobjecty($name='')
	{
		 ...
		 if ($user)
		{
			$a=array('id'=> $user->id,
					...
					'logout_revision'=>$user->logout_revision,);
					return  (object)$a;
		}else
		{
	...
	*/
	...
	public function userobjectyxx($name = '')
	{
		/*if (Yii::app()->user->isGuest) {
			Yii::app()->user->setReturnUrl(Yii::app()->request->requestUri); // Geri dönüş adresi
			Yii::app()->controller->redirect(Yii::app()->createUrl('site/login'));
			Yii::app()->end(); // Uygulama burada sonlansın
		}
		*/
	...
	/*		Yii::app()->user->logout(); // Her ihtimale karşı session temizliği
			Yii::app()->controller->redirect(Yii::app()->createUrl('site/login'));
			Yii::app()->end();  */
		}
	}
	...
	public function sqlcompany()
	{
		...
		if($name=="admin")
		{
			 ...
			 $a=array('id'=> $company->id,
				...
				'servicereport_color'=>$company->servicereport_color);
...
*/
	...
	public function getidfromauthname($name,$type)
	{
		...
		if ($type=='client')
		{
			$a=Client::model()->find(array('condition'=>'isdelete=0 and username="'.$name.'"'));
			if ($a){ return $a->id;}
	...
	*/
	...
	public function newdocument($categoryid=0,$cbxid=0)
	{
		...
		else
		{
		 ...
		 $fid=$ax->firmid;
					 $bid=$ax->branchid;
					 $cid=$ax->clientid;
					  $cbid=$ax->clientbranchid;
	    if($cbxid!=0)
		{
		    $cbid=$cbxid;
			$cbdetay=Client::model()->find(array('condition'=>'id=:id','params'=>array('id'=>$cbid)));
		    $fdetay=Firm::model()->find(array('condition'=>'id=:id','params'=>array('id'=>$cbdetay->firmid)));
			$fid=$fdetay->parentid;
    		$bid=$cbdetay->firmid;
    		$cid=$cbdetay->parentid;
    		$user= Yii::app()->db->createCommand('SELECT documents.id as id,documents.categoryid as categoryid FROM documents INNER JOIN userdocumentview ON documents.id = userdocumentview.documentid where userdocumentview.userid='.$ax->id.$categoryid.'  and (select dvf.id from documentviewfirm dvf where dvf.documentid=documents.id and clientbranchid='.$cbid.' limit 1) is not null')->queryAll();
		}
		else
		{
		...
		$document= Yii::app()->db->createCommand('SELECT documents.id as id,documents.categoryid as categoryid,documents.name as name,documents.fileurl as url,documents.type as type,documents.firmid as dfirmid,documents.branchid as dbranchid,documents.clientid as dclientid,documents.clientbranchid as dclientbranchid,documents.createdtime as createdtime,documentviewfirm.createdtime as vcreatedtime FROM documents INNER JOIN documentviewfirm ON documents.id = documentviewfirm.documentid where documentviewfirm.firmid='.$fid.' and documentviewfirm.branchid='.$bid.' and documentviewfirm.clientid='.$cid.' and documentviewfirm.clientbranchid='.$cbid.$categoryid.' GROUP BY documentviewfirm.documentid')->queryAll();
	...
	public function userpagepermission($fid,$bid,$cid,$cbid)
	{
		$ax= User::model()->userobjecty('');
		if($ax->id==1)
		{
		...
		if($ax->type==13) /// firm admin
		{
			if($ax->firmid!==$fid) //// sayfadan atacak
			{
				return 0;
			}
		}
		else if($ax->type==17) /// firm staff
		{
			if($ax->firmid!==$fid) //// sayfadan atacak
			{
				return 0;
		...
		else if($ax->type==19) /// branch staff
		{
			if($ax->branchid!==$bid) //// sayfadan atacak
			{
				return 0;
			}
			
		}
		else if($ax->type==23) /// branch admin
		{
			if($ax->firmid!==$fid) //// sayfadan atacak
			{
				return 0;
		...
		else if($ax->type==24) /// customer staff
		{
			if($ax->clientid!==$cid) //// sayfadan atacak
			{
				return 0;
			}
		}
		else if($ax->type==22) /// customer admin
		{
			if($ax->clientid!==$cid) //// sayfadan atacak
			{
				return 0;
		...
		else if($ax->type==27) /// customer branch staff
		{
			if($ax->clientbranchid!==$cbid) //// sayfadan atacak
			{
				return 0;
			}
		}
		else if($ax->type==26) /// customer branch admin
		{
			if($ax->clientbranchid!==$cbid) //// sayfadan atacak
			{
				return 0;
	...
	{
		return 'id'; // veya birden çok ise ['key1', 'key2']
	}
