...
public function kategoritabloyaz($id,$client) //leftmenu default sayfasındaki listeleme bölümü
{
		...
		foreach ($liste as $deger)
		{
			...
			<?php if (Yii::app()->user->checkAccess('client.branch.department.update') && ($transfer!=1 || $ax->branchid==0 || $ax->branchid==$tclient->firmid||($ax->clientbranchid==0 && $ax->clientid==$tclient->mainclientid))){ ?>	
			
			 <a style='margin-left: 10px;' onclick="openmodalupdate(this)" data-id="<?=$deger->id;?>" data-name="<?=$deger->name;?>" data-parent="<?=$deger->parentid;?>" data-active="<?=$deger->active;?>" class="btn btn-warning btn-sm"><i style="color:#fff;" class="fa fa-edit"></i>
				
				</a>

			 <?}?>
			<?php if (Yii::app()->user->checkAccess('client.branch.department.delete')  && ($transfer!=1 || $ax->branchid==0 || $ax->branchid==$tclient->firmid||($ax->clientbranchid==0 && $ax->clientid==$tclient->mainclientid))){ ?>	

				 <a style='margin-left: 10px;' onclick="openmodaldelete(this)" data-id="<?=$deger->id;?>" data-name="<?=$deger->name;?>" class="btn btn-danger btn-sm"><i style="color:#fff;" class="fa fa-trash"></i>
			...
			<?php if (Yii::app()->user->checkAccess('client.branch.department.create')  && ($transfer!=1 || $ax->branchid==0 || $ax->branchid==$tclient->firmid||($ax->clientbranchid==0 && $ax->clientid==$tclient->mainclientid))){ ?>	
			<?if($deger->parentid==0){?>

				<a style='margin-left: 10px;' onclick="openmodalsubcreate(this)" data-id="<?=$deger->id;?>" data-name="<?=$deger->name;?>" class="btn btn-info btn-sm"><i style="color:#fff;" class="fa fa-plus"></i>
			...
			<?php	if ($altkategorivarmi) //alt kategorisi varmı
				{
						$this->kategoritabloyaz($deger->id,$client);
				}
