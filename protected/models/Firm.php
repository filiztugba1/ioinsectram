...
class Firm extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('name, username, title, taxoffice, taxno, address, landphone, email, package, image,image2,image_footer', 'length', 'max'=>255),
...
$criteria->compare('image_footer',$this->image_footer);
$criteria->compare('image2',$this->image2);
	...
	public function changePackage($oldpackage,$newpackage,$firmusername)
	{
		...
		foreach($olditem as $olditemx)
		{
			...
			$newitem=AuthItem::model()->deleteAll(array('condition'=>'name="'.$olditemx->name.'"'));
			/*$metin  = $newitem->name;
			$eski   = $oldpackage.'.';
			$yeni   = $newpackage.'.';
	...
	public function clientbranch($newpackage,$firmname)
	{
		Yii::app()->getModule('authsystem');
		$clientbranch=Yii::app()->db->createCommand('SELECT clientbranch.name,clientbranch.username as cbusername,client.username as cusername,firmbranch.username as fbusername,firm.username as fusername FROM firm INNER JOIN firm as firmbranch ON firmbranch.parentid=firm.id INNER JOIN client ON client.firmid=firmbranch.id INNER JOIN client as clientbranch ON clientbranch.parentid=client.id WHERE clientbranch.id=20 and firm.username="'.$firmname.'"')->queryall();
		
		for($i=0;$i<count($clientbranch);$i++)
		{
			AuthItem::model()->createitem($newpackage.'.'.$clientbranch[$i]['fusername'].'.'.$clientbranch[$i]['fbusername'].'.'.$clientbranch[$i]['cusername'].'.'.$clientbranch[$i]['cbusername'],0);
		
			AuthItem::model()->generateparentpermission($newpackage.'.'.$clientbranch[$i]['fusername'].'.'.$clientbranch[$i]['fbusername'].'.'.$clientbranch[$i]['cusername'].'.'.$clientbranch[$i]['cbusername']);
			AuthItem::model()->createnewauth($newpackage.'.'.$clientbranch[$i]['fusername'].'.'.$clientbranch[$i]['fbusername'].'.'.$clientbranch[$i]['cusername'],$clientbranch[$i]['cbusername'],'Branch');
			
			
			$clientid=Client::model()->find(array("condition"=>'username="'.$clientbranch[$i]['cbusername'].'"'));
			if($clientid)
			{
				...
				foreach ($users as $user) {
					...
					$authAssignment->itemname=$newpackage.'.'.$clientbranch[$i]['fusername'].'.'.$clientbranch[$i]['fbusername'].'.'.$clientbranch[$i]['cusername'].'.'.$clientbranch[$i]['cbusername'].'.'.$authtypes->authname;
					$authAssignment->userid=$user->id;
					$authAssignment->save();
					}
					catch (Exception $e) {
						/// daha onceden eklenmisse hata vermesin
					}
				}
			}
		}
	...
	public function firmt($newpackage,$firmname)
	{
			...
			if($firmid)
			{
				...
				foreach ($users as $user) {
					$authtypes=Authtypes::model()->find(array('condition'=>'id='.$user->type));
					$isuser=AuthAssignment::model()->findAll(array('condition'=>'userid='.$user->id.' and itemname="'.$newpackage.'.'.$firmname.'.'.$authtypes->authname.'"'));
					if(empty($isuser))
					{
						$authAssignment=new AuthAssignment();
